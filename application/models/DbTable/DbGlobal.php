...
class Application_Model_DbTable_DbGlobal extends Zend_Db_Table_Abstract
{
	...
	function getAllShift(){
		$db = $this->getAdapter();
		$sql = "SELECT shiftCode AS id, title AS name 
				FROM rms_shift 
				WHERE shiftCode IS NOT NULL AND title IS NOT NULL";
	...
	function getAllEnglishLevel($option = null)
	{
		$db = $this->getAdapter();
		$sql = "  SELECT id AS id,title AS name FROM `rms_english_level` 
	...
	public function getRatingSystem($degreeId)
	{
		$db = $this->getAdapter();
		$sql = "SELECT * FROM `rms_rating` order by id desc";
		return $db->fetchAll($sql);
	}
	...
	{
		return array('err' => 1, 'msg' => 'មិនទាន់មានទន្និន័យនូវឡើយទេ!');
	}
	...
	public function caseEdit($id,$urlEdit)
	{
		
		$edit = ', CONCAT("<a href=\"", "'.$urlEdit.'", '.$id.', "\" class=\"btn btn-primary btn-xs\">",
		"<i class=\"fa fa-edit\"></i> ", "'.$this->tr->translate("EDIT").'", "</a>") AS edit';
		
		$string = $edit;
		return $string;
	}
	public function caseView($id,$urlEdit)
	{
		
		$edit = ', CONCAT("<a href=\"", "'.$urlEdit.'", '.$id.', "\" class=\"btn btn-primary btn-xs\">",
		"<i class=\"fa fa-edit\"></i> ", "'.$this->tr->translate("VIEW").'", "</a>") AS edit';
		
		$string = $edit;
		return $string;
	}
	public function caseReturn($id,$url)
	{
		
		$returnbt = 'CONCAT("<a href=\"'.$url.'", '.$id.', "\" class=\"btn btn-primary btn-xs\">",
        "<i class=\"fa fa-undo\"></i> ", "'.$this->tr->translate("RETURN_STOCK").'", "</a>")';

		$returned = 'CONCAT("<a class=\"btn btn-success btn-xs\">",
				"<i class=\"fa fa-check\"></i> ", "'.$this->tr->translate("IS_RETURN").'", "</a>")';

		$string = ",CASE
				WHEN isReturn = 1 THEN $returned
				WHEN isReturn = 0 THEN $returnbt
				END AS returnAction";
	...
	function getUserListbyBranch($data)
	{
		...
		if (!empty($data['branchId'])) {
			$sql .= " AND (FIND_IN_SET('" . $data['branchId'] . "', u.branch_list) )";
	...
	public function getUserInfo()
	{
		...
		if (!empty($session_user->user_id)) {
			...
			$dashboardType = $session_user->dashboardType;
			$dashboardType = empty($dashboardType) ? 0 : $dashboardType;
			
			$amtBranchAccess = $session_user->amtBranchAccess;
			$amtBranchAccess = empty($amtBranchAccess) ? 0 : $amtBranchAccess;
			...
			$info = array(
					...
					, "amtBranchAccess" => $amtBranchAccess
				);
			return $info;
...
//    }
	...
	function getAllLocation($data=[])
	{
		...
		$sql = "
			SELECT 
				b.br_id as id
				, b.$branchLabel as name 
		";
		if(!empty($data["brInfo"])){
			$lang = $this->currentlang();
			$brName = "branch_nameen";
			if ($lang == 1) {// khmer
				$brName = "branch_namekh";
			}
			$sql.="
				,COALESCE(b.abbreviations,b.$brName) AS brNameShort
				,b.branch_namekh
				,b.branch_nameen
				,b.br_address
				,b.branch_tel
		...
		$sql.="
			FROM rms_branch AS b WHERE b.branch_nameen!='' 
			AND  b.status=1  ";
		if(!empty($data["limitRecord"])){
			$sql.=" LIMIT ".$data["limitRecord"];
		}
		return $db->fetchAll($sql);
	}
	...
	{//for fee id only
		...
		$titleQuery = "CONCAT(ac.fromYear,'-',ac.toYear,st.$label,'(',t.generation,')')";
		if (!empty($data['shortTitle'])) {
			$titleQuery = "CONCAT(COALESCE(st.$label,''),'(',generation,')')";
		}

		$sql = "SELECT t.id,
		   	$titleQuery AS name,
		   	CONCAT_WS(',',CONCAT(ac.fromYear,'-',ac.toYear),t.generation) AS years,
			t.generation as feeTitle,
			CONCAT(ac.fromYear,'-',ac.toYear) academicYearTitle,
			st.$label AS termStudy

	   	FROM rms_tuitionfee t 
			LEFT JOIN rms_academicyear ac ON ac.id=t.academic_year
			LEFT JOIN rms_studytype st ON  st.id =t.term_study
		...
		if (!empty($data['academicYear'])) {
			$sql .= " AND t.academic_year =" . $data['academicYear'];
		}
		if (isset($data['isFinished'])) {
			$sql .= " AND t.is_finished=" . $data['isFinished'];
		}

		if (!empty($data['schoolOption'])) {
			$sql .= " AND t.school_option = " . $data['schoolOption'];
		}
		if (!empty($data['degree'])) {
			...
			if (!empty($row['schoolOption'])) {
				$sql .= " AND t.school_option IN (" . $row['schoolOption'] . ") ";
			}
		}
		...
		$level = $user['level'];
		if ($level != 1) {
			$sql .= ' AND t.school_option IN (' . $user['schoolOption'] . ')';
		}

		$sql .= " GROUP BY t.academic_year, t.term_study, t.generation ";
		$order = ' ORDER BY t.academic_year DESC,id DESC';
		...
		if (!empty($data['option'])) {
			...
			if (!empty($result))
				foreach ($result as $value) {
					...
					$options .= '<option value="' . $value['id'] . '" data-datajson="'.$resultPeriodJson.'"  data-academic-year-title="' . $value['academicYearTitle'] . '" data-fee-title="' . $value['feeTitle'] . '" data-term-study="' . $value['termStudy'] . '"  >' . htmlspecialchars($value['name']) . '</option>';
				}
			return $options;
		}
		return $result;
	...
	function getAllGradeList()
	{
		...
		$sql = "
			SELECT i.id,
			  	i.$colunmname AS name,
				i.items_id AS degreeId,
				(SELECT it.$colunmname FROM `rms_items` AS it WHERE it.id = i.items_id LIMIT 1) as degreeName
			  	FROM 
			  		`rms_itemsdetail` AS i
			  	WHERE i.status =1 AND i.items_type=1
			";
	...
	function getAllListStudent($data)
	{
		...
		if(!empty($data['joinGroup'])) {
			...
			$where = " WHERE gds.stu_id = s.stu_id AND gds.itemType = 1 ";
			...
			$data["activeStudent"] = isset($data["activeStudent"]) ? $data["activeStudent"]:1;
			if($data["activeStudent"]==1){
				$where .= " AND (gds.stop_type=0 AND gds.is_current=1)";//active student
			}
			...
			if (!empty($data['academicYear'])) {
				$where .= " AND gds.academic_year=" . $data['academicYear'];
			  }
			  if (!empty($data['grade'])) {
				$where .= " AND gds.grade=" . $data['grade'];
			  }
		...
		if (!empty($data['onlyHasCredit'])){
			$where .= " AND s.walletBalance > 0 ";
		}
	...
	function getStudentinfoGlobalById($stu_id)
	{
		...
		$sql = "
			SELECT 
				s.*
				,s.walletBalance
				,COALESCE(NULLIF(fam.fatherNameKh,''),COALESCE(NULLIF(fam.fatherName,''),'')) AS fatherName
				...
				,COALESCE(NULLIF(fam.motherNameKh,''),COALESCE(NULLIF(fam.motherName,''),'')) AS motherName
				...
				itd.$colunmname AS grade_label,
				i.$colunmname AS degree_label,
				v1.name_kh AS degree_culture,
				v2.name_kh AS studentType,
				v3.$field AS status_student,
				p.$fieldPartList AS parttime_label,
		   		sgd.academic_year,
		   		sgd.is_newstudent,
				sgd.feeId AS fee_id,
				sgd.school_option,
				COALESCE(sgd.group_id,0) AS groupId,
				CONCAT(ac1.fromYear, '-', ac1.toYear) AS academic_year_label,
    			CONCAT(ac2.fromYear, '-', ac2.toYear) AS academicYearEnroll,
				g.group_code          AS group_name,
				el.title AS english_level
	   		FROM 
	   			rms_student AS s LEFT JOIN rms_group_detail_student AS sgd 
					ON s.stu_id=sgd.stu_id AND sgd.itemType = 1 AND sgd.is_current = 1 AND sgd.is_pass = 0 AND sgd.is_maingrade = 1
				LEFT JOIN rms_english_level AS el ON el.id = sgd.englishLevel
				LEFT JOIN rms_family AS fam ON fam.id = s.familyId
				LEFT JOIN rms_itemsdetail AS itd ON itd.id = sgd.grade
				LEFT JOIN rms_items AS i ON i.id = sgd.degree
				LEFT JOIN rms_view AS v1 ON v1.key_code = s.calture AND v1.type = 3
				LEFT JOIN rms_view AS v2 ON v2.key_code = s.studentType AND v2.type = 40
				LEFT JOIN rms_view AS v3 ON v3.key_code = sgd.stop_type AND v3.type = 5 AND sgd.is_maingrade = 1 AND sgd.is_current = 1
				LEFT JOIN rms_parttime_list AS p ON p.id = sgd.partTimeId
				LEFT JOIN rms_academicyear AS ac1 ON ac1.id = sgd.academic_year
				LEFT JOIN rms_academicyear AS ac2 ON ac2.id = s.academicYearEnroll
				LEFT JOIN rms_group AS g ON g.id = sgd.group_id AND sgd.is_maingrade = 1 AND sgd.is_current = 1
			WHERE 
				 s.stu_id=$stu_id 
	   	LIMIT 1 ";
	...
	function getStudentTestinfoById($stu_id)
	{
		...
		$sql = "SELECT 
			...
			i.$colunmname AS degree_label,
			 p.$fieldPartList AS parttime_label,
			t.degree_result AS degree,
			t.grade_result AS grade,
			t.session_result AS session,
			sd.feeId AS fee_id,
			sd.academic_year,
			'' AS english_level,
			'N/A' AS status_student,
			t.id,
			v2.name_kh as session_label,
			'N/A' AS room_label,
			'1' AS is_newstudent,
			CONCAT(ac2.fromYear, '-', ac2.toYear) AS academicYearEnroll
	   	FROM 
			...
			LEFT JOIN rms_family AS fam ON fam.id = s.familyId
			LEFT JOIN rms_view AS v1 ON v1.key_code = s.calture AND v1.type = 3
			LEFT JOIN rms_view AS v2 ON v2.key_code = t.session_result AND v2.type = 4
			
			LEFT JOIN rms_itemsdetail AS itd ON itd.id = t.grade_result
			LEFT JOIN rms_items AS i ON i.id = t.degree_result
			LEFT JOIN rms_academicyear AS ac2 ON ac2.id = s.academicYearEnroll

			LEFT JOIN rms_group_detail_student AS sd ON sd.stu_id = s.stu_id AND sd.test_restult_id = t.id
			LEFT JOIN rms_parttime_list AS p ON p.id = sd.partTimeId
	   	WHERE 
		   	t.is_current=1 AND t.updated_result=1
...
//    // Load the stamp and the photo to apply the watermark to
	...
	function getAllItems($type = null, $branchlists = null, $schooloption = null,$parent = 0, $spacing = '', $cate_tree_array = '',$isparent=null,$optionSelect=null,$data = [])
	{
		$db = $this->getAdapter();
			...
			if(empty($data["isFull"])){
				...
				if (!empty($userSelect['degreeList'])) {
					...
					AND 
						...
						ELSE m.id IN (" . $degreeList . ") OR  COALESCE(m.is_parent,0) = 1
						END
				";
				}
			}
	...
	function getAllItemDetail($data = null)
	{
		...
		$lbString=" CONCAT(i.$colunmname,' (',(SELECT it.$colunmname FROM `rms_items` AS it WHERE it.id = i.items_id LIMIT 1),')')  ";
		if (!empty($data['displayLabel'])) {
			if($data['displayLabel']=="shortcut"){
				$lbString="TRIM(i.shortcut)";
			}
		}
		$sql = "
			SELECT i.id,
	...
	function getAllGradeStudyByDegree($data,$category_id = null, $student_id = null, $is_stutested = null, $groupDetailId = null)
	{
		...
		$sql = "SELECT 
			...
			FROM 
				`rms_itemsdetail` AS i 
				LEFT JOIN rms_items AS ite ON ite.id = i.items_id
			WHERE i.status = 1 and product_type=1 ";

		if(!empty($data['categoryId'])){
			$deptMuti = $data['categoryId'];
			if (!empty($deptMuti)) {
				if (!is_numeric($deptMuti)) {
					$deptMuti = implode(',',(json_decode($deptMuti, true)));
				}
				$sql .= " AND FIND_IN_SET(i.items_id, '".$deptMuti."')";
			}
		}
		if (!empty($data['itemsType'])) {
			$sql .= " AND i.items_type=" . $data['itemsType'];
		...
		if (!empty($data['parentcagetoryId'])) {
			$sql .= " AND  ite.parent=".$data['parentcagetoryId'];
		}
		if (!empty($data['studentId'])) {
		...
		if ($level != 1) {
			...
			$userSelect = $this->getUserProfile();
			if (!empty($userSelect['degreeList']) AND $level!=1) {
				$degreeList = $userSelect['degreeList'];
				$sql .= "
				AND 
					CASE  WHEN i.items_type != 1 THEN '1' 
		...
		$sql.=" OR i.isExtraCourse=1 ";
		$sql .= " GROUP BY i.id  ";//should remove
		$sql .= " ORDER BY i.items_type ASC,COALESCE(ite.ordering,0) ASC, i.ordering ASC , i.items_id DESC ";
		// Application_Model_DbTable_DbUserLog::writeMessageError($sql);
	...
	function getnewStudentId($branch_id, $degree,$extraData=[])
	{//used global
		$db = $this->getAdapter();
		...
		$extraData["branchId"] = empty($extraData["branchId"]) ? $branch_id : $extraData["branchId"];
		$extraData["degree"] = empty($extraData["degree"]) ? $degree : $extraData["degree"];
		$extraData["schoolOption"] = empty($extraData["schoolOption"]) ? $schoolOption : $extraData["schoolOption"];

	
		
		$_dbStCode = new Application_Model_DbTable_DbStudentCode();
		$gapStuCode = $_dbStCode->getCheckGapStuCode($extraData);
		if(!empty($gapStuCode)){
			return $gapStuCode["stuCode"];
		}else{
			$startNum = 0;
			$prefixChar="";
			
			$pre = $_dbStCode->getPrefixCode($extraData);
			...
			if ($option_type == 1) {//auto by branch
				...
				$sql = "
					...
					FROM `rms_student` AS s
						JOIN `rms_group_detail_student` AS gds ON gds.stu_id = s.stu_id AND gds.is_maingrade=1 AND gds.`itemType`=1 AND gds.`is_current`=1 
				...
				*/
				$sql = "
					SELECT 
						COUNT(DISTINCT gds.stu_id)
					FROM `rms_student` AS s
						JOIN `rms_group_detail_student` AS gds ON gds.stu_id = s.stu_id AND gds.is_maingrade=1 AND gds.`is_current`=1
	...
	function getStudentProfileblog($student_id, $data_from = 1, $customer_type = 1)
	{
		...
		if ($customer_type == 1) {//student
			if ($data_from == 1 or $data_from == 3) {//crm,student
		...
		if (!empty($rs)) {
			...
			$studentData = array(
				...
				'walletBalance' => empty($rs['walletBalance']) ? 0 : $rs['walletBalance'],
			);

			
			$groupResult =[];
			if(SHOW_GROUP_INPAYMENT==1){//show group in payment
				//NEW_STU_ID_FROM_TEST==0 should not call group
				$param = array(
					'branch_id' => $branchId,
					'academic_year'=>empty($rs['academic_year'])?'':$rs['academic_year'],
					'degree' => $degree,
					'grade' => $grade,
					'noaddnew' => 1,
					'showGroupOnly'=>1,
			...
			if(NEW_STU_ID_FROM_TEST==1){//setting for new student id from test
			...
			$stuDob = "";
			if(!empty($rs['stuDob'])){
				if(($rs['stuDob']!="0000-00-00") OR $rs['stuDob']!="1970-01-01"){
					$stuDob = date("d-m-Y",strtotime($rs['stuDob']));
				}
			}
			...
			$str = '<div class="col-md-3 col-sm-12 col-xs-12">
						...
						<div class="col-md-9 col-sm-9 col-xs-12">
							<div class="form-group">
								<div class="control-label col-md-5 col-sm-12 col-xs-6">
									' . $tr->translate("STUDENT_CODE") . '
								</div>
								<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_stucode"><a target="_blank" href="' . $link . '">' . $rs["stu_code"] . '</a></span>
								</div>
							</div>
							<div class="form-group">
								<div class="control-label col-md-5 col-sm-12 col-xs-6">
									' . $tr->translate("STUDENT_NAMEKHMER") . '
								</div>
								<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_namekh"><a target="_blank" href="' . $link . '">' . $rs["stu_khname"] . '</a></span>
								</div>
							</div>
							<div class="form-group">
								<div class="control-label col-md-5 col-sm-12 col-xs-6">
									' . $tr->translate("NAME_ENGLISH") . '
								</div>
								<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_nameen" ><a target="_blank" href="' . $link . '">' . $rs["last_name"] . " " . $rs["stu_enname"] . '</a></span>
								</div>
							</div>
							<div class="form-group">
								<div class="control-label col-md-5 col-sm-12 col-xs-6">
								...
								<div class="col-md-7 col-sm-12 col-xs-6">
									<span id="lbl_dob" class="inf-value " >: ' . $stuDob . '</span>
								</div>
							</div>
							<div class="form-group">
								<div class="control-label col-md-5 col-sm-12 col-xs-6">
								...
								<div class="col-md-7 col-sm-12 col-xs-6">:<span id="lbl_phone">&nbsp;' . $rs['tel'] . '</span>
								</div>
							</div>
							<div class="form-group">
								<div class="control-label col-md-5 col-sm-12 col-xs-6">
								...
								<div class="col-md-7 col-sm-12 col-xs-6">:
									<span id="lbl_enroll_year" class="inf-value groupinfo">' . $rs['academicYearEnroll'] . '</span>
								</div>
							</div>
							<div class="form-group">
								<div class="control-label col-md-5 col-sm-12 col-xs-6">
								...
								<div class="col-md-7 col-sm-12 col-xs-6">:
									<span id="lbl_student_type" >' . $rs['studentType'] . '</span>
								</div>
							</div>
							
						</div>';
					$parentPhone = empty($rs["father_phone"]) ? "" : $rs["father_phone"];
					if(!empty($rs["mother_phone"])){
						$parentPhone = ($parentPhone=="") ? $rs["mother_phone"] : $parentPhone." / ".$rs["mother_phone"];
					}
				$str_parentinfo='<div class="form-group">
									<div class="control-label col-md-5 col-sm-12 col-xs-6">
										' . $tr->translate("FATHER_NAME") . '
									</div>
									<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_father">'.$rs['fatherName'] . '</span>
									</div>
								</div>
								<div class="form-group" style="display:none;">
									<div class="control-label col-md-5 col-sm-12 col-xs-6">
										' . $tr->translate("FATHER_PHONE") . '
									</div>
									<div class="col-md-7 col-sm-12 col-xs-6">
										<span id="lbl_fathertel" style="display:none;">' . $rs['father_phone'] . '</span>
										<span id="lbl_mothertel" style="display:none;">' . $rs['mother_phone'] . '</span>
										<span id="lbl_degreeId" style="display:none;">' . $rs['degree'] . '</span>
										<span id="lbl_groupId" class="groupinfo" style="display:none;">' . $rs['groupId'] . '</span>
									</div>
								</div>
								<div class="form-group">
									<div class="control-label col-md-5 col-sm-12 col-xs-6">
										' . $tr->translate("MOTHER_NAME") . '
									</div>
									<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_mother">'. $rs['motherName'] . '</span>
									</div>
								</div>
								<div class="form-group">
									<div class="control-label col-md-5 col-sm-12 col-xs-6">
										' . $tr->translate("PARENT_TEL") . '
									</div>
									<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_parentphone">' . $parentPhone . '</span>
									</div>
								</div>
								<div class="form-group">
									<div class="control-label col-md-5 col-sm-12 col-xs-6">
									...
									<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_culturelevel" class="red bold" >' . $rs['status_student'] . '</span>
									</div>
								</div>';

			$studyInfo = '<div class="form-group">
							<div class="control-label col-md-5 col-sm-12 col-xs-6">
								' . $tr->translate("DEGREE") . '
							</div>
							<div class="col-md-7 col-sm-12 col-xs-6">:
								<span id="lbl_degree">' . $rs['degree_label'] . '</span>
							</div>
						</div>
						<div class="form-group">
							<div class="control-label col-md-5 col-sm-12 col-xs-6">
							...
							<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_grade">' . $rs['grade_label'] . '</span>
							</div>
						</div>
						<div class="form-group">
							<div class="control-label col-md-5 col-sm-12 col-xs-6">
							...
							<div class="col-md-7 col-sm-12 col-xs-6">: <span id="lbl_group" class="inf-value groupinfo">' . $rs['group_name'] . '</span>
							</div>
						</div>
						<div class="form-group">
							<div class="control-label col-md-5 col-sm-12 col-xs-6">
								' . $tr->translate("PART_TIME_LIST") . '
							</div>
							<div class="col-md-7 col-sm-12 col-xs-6">:
								<span id="lbl_part_time">' . $rs['parttime_label'] . '</span>
							</div>
						</div>
						<div class="form-group">
							<div class="control-label col-md-5 col-sm-12 col-xs-6">
								' . $tr->translate("English Level") . '
							</div>
							<div class="col-md-7 col-sm-12 col-xs-6">:
								<span id="english_level">' . $rs['english_level'] . '</span>
							</div>
						</div>';
			...
			if (!empty($rsDiscount)) {
				...
				foreach($rsDiscount as $Discount){
					...
					$bgClass="";
					if(!empty($arrBgLabel[$Discount['discountForId']])){
						$bgClass=$arrBgLabel[$Discount['discountForId']];
					}
					...
					$strDiscountList.= '<li class="media event">
						'.$strListDay.'<div class="media-body">
							...
							<p><small class="'.$bgClass.'">'.$Discount['discountFor'].'</small><small class="blue"> ('.$Discount['discountPeriod'].')</small>
							</p>
						</div>
					</li>';
				}
	...
	function getStudentGroupInfoById($id)
	{
		...
		SELECT 
			...
			,g.expired_date
			,g.group_code
			,(SELECT CONCAT(ac.fromYear,'-',ac.toYear) FROM `rms_academicyear` AS ac WHERE ac.id = g.academic_year LIMIT 1) AS year
		  	,(SELECT CONCAT(ac.fromYear,'-',ac.toYear) FROM `rms_academicyear` AS ac WHERE ac.id = g.academic_year LIMIT 1) AS year_only
		  	,it.$colunmname AS degree
		  	,itd.$colunmname AS grade
		  	,(SELECT pt.$partTimeTitle FROM `rms_parttime_list` AS pt WHERE  pt.`id`=`g`.`session` LIMIT 1) AS session
		  	,(SELECT room_name FROM rms_room WHERE room_id = g.room_id LIMIT 1) AS room
		  	,g.academic_year AS academic_year_id
			,g.degree AS degree_id
			,g.grade AS grade_id
			,g.session AS session_id
			,room_id
			,(SELECT t.teacher_name_kh FROM rms_teacher AS t WHERE t.id=g.teacher_id LIMIT 1) AS teacherNameKh
		  	,(SELECT t.teacher_name_en FROM rms_teacher AS t WHERE t.id=g.teacher_id LIMIT 1) AS teacherNameEng
		  	,it.title AS degree_kh
		  	,it.title_en AS degree_eng
			,itd.title AS grade_kh
		  	,itd.title_en AS grade_eng
		  	,it.isExtraCourse AS isExtraCourse
		  	,it.schoolOption
  		FROM
  			`rms_group` AS g 
	...
	function getAllGroupByBranch($branch_id = null, $forfilterreport = null, $data = array())
	{
		...
		$amtBranchAccess =  empty($userInfo["amtBranchAccess"]) ? 0 : $userInfo["amtBranchAccess"];
		
		$stringGroupName = "CONCAT(g.group_code,' ',COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,'') )";
		//if($amtBranchAccess>1){
			//$stringGroupName = "CONCAT(g.group_code,' ',COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,''),' ',COALESCE(b.$branch,''))";
		//}
		$sql = "SELECT 
					`g`.`id`, 
					TRIM($stringGroupName) AS name
				FROM `rms_group` AS `g` 
		...
		if (!empty($data['branchListJs'])) {
			$data['branchListJs'] = json_decode($data['branchListJs']);
			$branchList = implode(",", $data['branchListJs']);
			$sql.= " AND (FIND_IN_SET(g.branch_id,'" . $branchList . "' )) ";
		}
		$sql .= $this->getAccessPermission('g.branch_id');
		$sql .= $this->getDegreePermission('g.degree');
		$sql .= " ORDER BY it.ordering ASC,itd.ordering ASC,g.degree,g.grade,g.group_code ASC,`g`.`id` DESC ";
		return $db->fetchAll($sql);
	}
	...
	function ordinal_unicode($number) {
		$suffixes = ['ᵗʰ', 'ˢᵗ', 'ⁿᵈ', 'ʳᵈ'];
		$value = $number % 100;
	
		$suffix = isset($suffixes[($value - 20) % 10]) ? $suffixes[($value - 20) % 10] : (
			isset($suffixes[min($value, 3)]) ? $suffixes[min($value, 3)] : $suffixes[0]
		);
	...
	public function getAllAcademicYear($option = null, $ordering = null,$data = [])
	{
		$db = $this->getAdapter();
		$sql = "SELECT id,
	...
	public function getAllChangeGroup($type=null)
...
//   	$stuName = "COALESCE(s.stu_khname,'')";
	...
	{//one calling
		...
		$sql = "SELECT
			  	...
			  	gs.academic_year,
			  	gs.grade AS itemDetailId,
			  	dcs.discountId AS discountType,
			  	dcs.discountValue AS discountValue,
			  	its.$colunmname AS itemLabel,
			  	it.$colunmname AS itemDetaillabel,
			  	it.is_onepayment AS is_onepayment
	  	FROM
	  		rms_student as st
	  		LEFT JOIN `rms_group_detail_student` AS gs ON st.stu_id=gs.stu_id
			LEFT JOIN rms_dis_setting dcs ON dcs.studentId=st.stu_id AND dcs.endDate >='$today'
			LEFT JOIN rms_items its ON its.id=gs.degree
			LEFT JOIN rms_itemsdetail it ON it.id=gs.grade
	...
	public function getAllTermbyItemdetail($data)
	{
		try {
			...
			$sql = "SELECT
				...
				tf.id as feeId
		  	FROM
			  	rms_tuitionfee tf JOIN
				rms_tuitionfee_detail tfd ON tf.id = tfd.fee_id
				LEFT JOIN rms_view v ON v.`type`=6 AND v.key_code =tfd.`payment_term`
				LEFT JOIN rms_itemsdetail it ON it.id=tfd.class_id
		  	WHERE tf.status=1 AND tfd.tuition_fee!=0 ";
			if (!empty($data['itemId'])) {
			...
			if ($data['itemType'] == 1 and !empty($data['feeId'])) {//school fee
				$feeId= $data['feeId'];
				$AcademicYear=0;//must select year by feeId
				$sql.=" AND ((it.isExtraCourse = 1 OR tf.academic_year = $AcademicYear) OR (it.isExtraCourse != 1 AND tf.id = $feeId))";
			}
	...
	{//for payment only
		...
		foreach ($resultItems as $key => $item) {
			...
			$param = array(
				...
				'feeId' => empty($item['feeId'])?$data['feeId']:$item['feeId'],
			);
	...
	function getAllGroupName($data = null)
	{
		...
		$stringGroupName = "CONCAT(g.group_code,' ',COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,'') )";
		$stringGroupName = !empty($data['showGroupOnly'])?'g.group_code':$stringGroupName;//showGroupOnly not include year for user can select group in payment to display group in receipt
		//if($amtBranchAccess>1){
			//$stringGroupName = "CONCAT(g.group_code,' ',COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,''),' ',COALESCE(b.$branch,''))";
		//}
	...
	function getItemAllDetail($data)
	{
		...
		$sql = "SELECT
		  	...
		  	i.$colunmname  AS itemDetaillabel,
			it.$colunmname AS itemLabel,
		  	i.is_onepayment,
		  	i.items_id as itemId,
		  	ds.discountId AS discountType,
  		...
  		FROM `rms_itemsdetail` i 
			LEFT JOIN rms_items it ON it.id=i.items_id
			LEFT JOIN rms_dis_setting ds ON ds.studentId=$studentId AND ds.endDate >='$today'
		WHERE
  			i.status=1 ";
		...
		if (isset($data['isAutopayment']) and $data['isInititilize'] == 1) {
			if ($data['isAutopayment'] != '') {
				if($data['studentType']==1){
					...
					if($data['isAutopayment']==2){
						$sql.=" AND EXISTS (SELECT 1
							FROM rms_student_payment sm
								INNER JOIN rms_student_paymentdetail smp ON sm.id = smp.payment_id
								WHERE sm.student_id = $studentId AND smp.`itemdetail_id`=$refundTestFee LIMIT 1
						)";
					}
	...
	function AddItemToGroupDetailStudent($data)
	{
		try {
			...
			if (empty($resultDetail)) {
				...
				if (!empty($data['changeGroupId'])) {
					$_arr['changeGroupId'] = $data['changeGroupId'];
				}
				if (!empty($data['referenceId'])) {
					$_arr['referenceId'] = $data['referenceId'];
				}
	...
	public function getBackgroundSetting($data)
	{
		$db = $this->getAdapter();
		$sql = " SELECT * FROM `rms_certificate_setting` WHERE  branch_id = " . $data['branch_id']." AND type = ". $data['type'];
		if (!empty($data['dregreeId'])) {
			$sql .= " AND  FIND_IN_SET( " . $data['dregreeId'] . ", degreeList )";
		}
	...
	function getAllPartTimeList($data)
	{
		...
		$sql .= " ORDER BY pt.$columnName ASC ";
		return $db->fetchAll($sql);
	}
	function getAllPartTimeName()
	{
		$db = $this->getAdapter();
		$currentLang = $this->currentlang();
		$columnName = "titleKh";
	...
	function getDiscountListbyStudent($data)
	{
		if (!empty($data['resultList'])) {
			$sql = "
				...
				FROM
					`rms_dis_setting` as ds WHERE ds.status=1 ";
		} else {
			$currentLang = $this->currentlang();
			$colunmname = 'name_en';
			if ($currentLang == 1) {
			...
			$sql = "
				SELECT
					...
					ds.discountCode,
					ds.discountTitle,
					v1.$colunmname AS discountFor,
					v2.$colunmname as discountForType,
					disc.studentId,
					disc.grade,
					ds.discountId AS discountTypeId,
					d.dis_name AS discountName,
					ds.degree,
					ds.DisValueType as DiscountValueType,
					ds.discountValue,
					ds.discountFor AS discountForId,
					...
					CONCAT(COALESCE(v3.$colunmname),'',COALESCE(DATE_FORMAT(ds.startDate,'%d-%m-%Y'),''),'/',COALESCE(DATE_FORMAT(ds.endDate,'%d-%m-%Y'),'')) AS discountPeriod, 
					DATE_FORMAT(ds.endDate,'%d-%b') endDay,
					DATE_FORMAT(ds.endDate,'%y') endYear
				FROM `rms_dis_setting` AS ds
					LEFT JOIN rms_discount_student disc ON ds.`id`=disc.`discountGroupId` $sqlStuId
					LEFT JOIN rms_discount AS d ON d.disco_id = ds.discountId
					LEFT JOIN rms_view v1 ON v1.type=37 AND v1.key_code=ds.discountFor
					LEFT JOIN rms_view v2 ON v1.type=38 AND v2.key_code=ds.discountForType
					LEFT JOIN rms_view v3 ON v3.key_code = ds.discountPeriod AND v3.type = 39
		...
		if (!empty($data['branch_id'])) {
			$sqldiscountId .= " AND ds.branchId=" . $data['branch_id'];
		}
		if (!empty($data['academic_year'])) {
			$sqldiscountId .= " AND ds.academicYear=" . $data['academic_year'];
		}
	...
	function getChildCategoryIncome($arr = array(),$idetity = null){
		$db = $this->getAdapter();
		$parentId = empty($arr["categoryId"]) ? 0 : $arr["categoryId"];
		$sql=" SELECT 
				i.`id` 
			FROM `rms_cate_income_expense` AS i 
			WHERE i.`parent` = $parentId AND i.`status`=1 ";
		$child = $db->fetchAll($sql);
		if(!empty($child)){
			foreach ($child as $va) {
				...
				$arr["categoryId"] = empty($va['id']) ? 0 : $va['id'];
				$idetity = $this->getChildCategoryIncome($arr,$idetity);
			}
		}
		return $idetity;
	}

	function getChildExpenseCategory($arr = array(),$idetity = null){
		$db = $this->getAdapter();
		$parentId = empty($arr["categoryId"]) ? 0 : $arr["categoryId"];
		$sql=" SELECT 
				i.`id` 
			FROM `rms_expense_category` AS i 
			WHERE i.`parentId` = $parentId AND i.`status`=1 ";
		$child = $db->fetchAll($sql);
		if(!empty($child)){
			foreach ($child as $va) {
				if (empty($idetity)){
				...
				$arr["categoryId"] = empty($va['id']) ? 0 : $va['id'];
				$idetity = $this->getChildExpenseCategory($arr,$idetity);
			}
		}
		return $idetity;
	}

	function getAllExpenseCategory($arr = array(),$idetity = null){
		$db = $this->getAdapter();
		$parentId = empty($arr["categoryId"]) ? 0 : $arr["categoryId"];
		
		$sql=" SELECT 
				c.`id` 
			FROM `rms_expense_item` AS c 
			WHERE c.`parentId` = $parentId AND c.`status`=1 ";
		$child = $db->fetchAll($sql);
		if(!empty($child)){
			foreach ($child as $va) {
				if (empty($idetity)){
	...
	public function slqRowButton($data=[])
	{
		$id = empty($data["id"]) ? "" : $data["id"];
		$urlEdit = empty($data["urlEdit"]) ? "#" : $data["urlEdit"];
		$title = empty($data["title"]) ? $this->tr->translate("EDIT") : $this->tr->translate($data["title"]);
		$btnClass = empty($data["btnClass"]) ? "btn btn-primary" : $data["btnClass"];
		$btnIcon = empty($data["btnIcon"]) ? "fa-edit" : $data["btnIcon"];
		
		$edit = 'CONCAT("<a href=\"", "'.$urlEdit.'", '.$id.', "\" class=\"btn '.$btnClass.' btn-xs\">",
		"<i class=\"fa '.$btnIcon.'\"></i> ", "'.$title.'", "</a>")';
		
		$string = $edit;
		return $string;
	}
	
	function getAllPrevSchool($data=[])
	{
		$db = $this->getAdapter();
		$sql = "SELECT 
					pt.id AS `id`
					,pt.schoolName AS `name`
				FROM `rms_previous_school` AS pt
				WHERE pt.`status` =1 
			";
		if(!empty($data["id"])){
			$sql.=" OR pt.id = ".$data["id"];
		}
		$sql .= " ORDER BY pt.schoolName ASC ";
		return $db->fetchAll($sql);
	}
}
?>