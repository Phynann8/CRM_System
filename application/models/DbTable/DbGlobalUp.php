...
class Application_Model_DbTable_DbGlobalUp extends Zend_Db_Table_Abstract
{
	...
	public function getAllStudentsList($data){
		...
		if (!empty($data['joinGroup'])) {
			$sql .= "
				,CASE 
					WHEN gds.`is_setgroup` =1 
						THEN (SELECT CONCAT(COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,'')) FROM `rms_academicyear` AS ac WHERE ac.id =g.`academic_year` LIMIT 1) 
					ELSE (SELECT CONCAT(COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,'')) FROM `rms_academicyear` AS ac WHERE ac.id =gds.`academic_year` LIMIT 1) 
				END AS academicYear
				,(SELECT i.$degree FROM rms_items AS i WHERE i.id=gds.degree AND i.type=1 LIMIT 1) AS degreeTitle
				,(SELECT idd.$grade FROM rms_itemsdetail AS idd WHERE idd.id=gds.grade AND idd.items_type=1 LIMIT 1) AS gradeTitle
				,gds.`degree`
				,gds.`grade`
				,gds.`group_id` AS groupId
				,gds.`programType`
			...
			if (!empty($data['academicYear'])) {
				$sql .= " AND gds.academic_year=" . $data['academicYear'];
			}
			if (!empty($data['programType'])) {
				$sql .= " AND gds.programType=" . $data['programType'];
			}
	...
	public function getAllFamilyList($data){
		...
		$concate=" 
			CONCAT(
				fam.familyCode,' ',
				COALESCE(NULLIF(fam.fatherNameKh,''), fam.fatherName, ''),
				' / ',
				COALESCE(NULLIF(fam.motherNameKh,''), fam.motherName, '')
			)
		";
		...
		$sql = " 
			SELECT 
				...
				,fam.`familyCode`
				,fam.`fatherName`
				,COALESCE(NULLIF(fam.fatherNameKh,''), fam.fatherName) AS fatherNameKh
				,fam.`fatherPhone`
				,fam.`fatherPhoto`
				,fam.`motherName`
				,COALESCE(NULLIF(fam.motherNameKh,''), fam.motherName) AS motherNameKh
				,fam.`motherPhone`
				,fam.`motherPhoto`
				,fam.`guardianName`
				,fam.`guardianNameKh`
				...
				,fam.`communeId`
				,fam.`villageId`
				,COALESCE(v.$labelShortCut,'N/A') AS familyTypeTitle
				,COALESCE(fam.`houseNo`,'N/A') AS houseNo
				,COALESCE(fam.`street`,'N/A') AS street
				,COALESCE(p.`province_en_name`,'N/A') AS provinceName
				,COALESCE(d.$districtCol,'N/A') AS districtName
			
		";
		$fromStatment = " FROM `rms_family` AS fam  
			LEFT JOIN rms_view AS v ON v.type =41 AND v.key_code = fam.`familyType`
			LEFT JOIN rms_province p on p.province_id = fam.provinceId
			LEFT JOIN ln_district d on d.dis_id = fam.districtId
		";
