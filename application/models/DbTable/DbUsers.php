...
class Application_Model_DbTable_DbUsers extends Zend_Db_Table_Abstract
{
	...
	{		
		if (!empty($user_id)){	
			...
			'(SELECT isSuperUser FROM `rms_acl_user_type` WHERE user_type_id=rms_users.user_type LIMIT 1) as isSuperUser'
			,'(SELECT dashboardType FROM `rms_acl_user_type` WHERE user_type_id=rms_users.user_type LIMIT 1) as dashboardType'
			,'(SELECT COUNT(b.br_id) FROM `rms_branch` AS b WHERE b.status=1 AND FIND_IN_SET(b.`br_id`,rms_users.`branch_list`) LIMIT 1) AS amtBranchAccess'
			))
			->where('id=?',$user_id);
	...
	function getUserList($search=null){
		...
		u.`active` as status,
		(SELECT GROUP_CONCAT(b.abbreviations) FROM `rms_branch` AS b WHERE b.status=1 AND FIND_IN_SET(b.br_id,u.branch_list) LIMIT 1) AS branchList
		FROM `rms_users` AS u
		WHERE u.is_system =0 ";
		$orderby = " ORDER BY u.user_type DESC";
...
// 		$where.= $_db->getAccessPermission('branch_id');
		...
		if (!empty($search['branch_id'])) {
            // Check if branch_id is in the user's branch_list using FIND_IN_SET
            $branchId = (int)$search['branch_id'];
            $where .= " AND FIND_IN_SET($branchId, u.branch_list)";
        }
