...
class Accounting_Model_DbTable_DbRequestProduct extends Zend_Db_Table_Abstract
{
    ...
    function getAllRequest($search=null){
		...
		$urlEdit = $base_url."/stock/requestproduct/edit/id/";
		$urlReturn = $base_url."/stock/requestproduct/return/id/";
		...
		$sql.=$dbp->caseReturn($id,$urlReturn);
    	...
    	$sql.=" FROM rms_request_order  WHERE 1 ";
	    	...
	    	if($search['request_status']==1 OR $search['request_status']==0){
	    		$where.=" AND isReturn =".$search['request_status'];
	    	}
	    	if(!empty($search['request_for'])){
	    		$where.=" AND request_for=".$search['request_for'];
	    	}
	    	if(!empty($search['for_section'])){
    ...
    public function addRequest($data){
			...
			if($data['identity']!=""){
				...
				foreach ($ids as $i)
				{
					...
					$param = [
						...
						'movement_type' => 6, // 6= Request Product,
						'quantity'      => -$data['receive_qty'.$i],
 	...
 	function updateRequest($data){
		...
		try{
			...
			$dbmv->deleteMovementStock($data['id'], 6); // delete movement stock
	...
	function returnStock($data){
		...
		try{
			$arr=array(
				"returnDate" =>date("Y-m-d",strtotime($data['returnDate'])),
				"user_id"    =>$this->getUserId(),
				"isReturn"   => 1,
			);
			$this->_name="rms_request_order";
			...
			if($data['identity']!=""){
				...
				{
					$data_item= array(
						'qty_return'	=>  $data['qty_return'.$i],
					);
					$this->_name='rms_request_orderdetail';
					$where=" request_id = ".$data['id']." AND pro_id= ".$data['product_name_'.$i];
					$this->update($data_item, $where);
					
					$rows=$this->getProQtyByLocation($data['branch_id_'.$i], $data['product_name_'.$i]); 
					if(!empty($rows)){
						$datatostock= array(
								'pro_qty' 	=> $rows["pro_qty"]+$data['qty_return'.$i],
								'date'		=> date("Y-m-d H:i:s"),
								'user_id'	=> $this->getUserId()
						);
						$this->_name="rms_product_location";
					...
					$param = [
						'branch_id'     => $data['branch_id_'.$i],
						'product_id'    => $data['product_name_'.$i],
						'movement_type' => 4, // 4= Return Product,
						'quantity'      => $data['qty_return'.$i],
						'reference'     => $data['id'],
	...
	function getRequestDetail($id){
		...
		$sql="SELECT
			...
			FROM rms_request_orderdetail AS rd
				LEFT JOIN rms_itemsdetail AS p ON p.`id` = rd.`pro_id`
				LEFT JOIN `rms_product_location` AS pl ON  (pl.`branch_id` = rd.`branch_id` AND pl.`pro_id` = rd.`pro_id` )
