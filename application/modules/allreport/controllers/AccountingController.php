...
class Allreport_AccountingController extends Zend_Controller_Action {
	...
	public function rptExpenseDetailReportAction(){
		$frm = new Application_Form_FrmGlobal();
		try{
			if($this->getRequest()->isPost()){
				$search=$this->getRequest()->getPost();
			...
			$db = new Allreport_Model_DbTable_DbRptIncomeExpense();
			$this->view->row = $db->getAllexpenseDetail($search);
			$this->view->search = $search;
	...
	function rptPaymentdetailbytypeAction(){
		...
		$form=$frm->FormSearchPaymentDetailReport();
		Application_Model_Decorator::removeAllDecorator($form);
		$this->view->form_search=$form;
	...
	public function rptstudentnearlyendserviceAction(){
		try{
			if($this->getRequest()->isPost()){
				...
				$serviceTypeParam = $this->getRequest()->getParam('service_type');
				if(!empty($serviceTypeParam)){
					$search["service_type"]=$serviceTypeParam;
				}
				$nearlyPaymetySortParam = $this->getRequest()->getParam('nearlyPaymetySort');
				if(!empty($nearlyPaymetySortParam)){
					$search["nearlyPaymetySort"]=$nearlyPaymetySortParam;
				}
	...
	public function rptExpenseBycateAction(){
		...
		try{
			...
			$this->view->rowMethod = $db->getAllExpenseCateByMethod($search);
			$this->view->search = $search;
			$branch_id = empty($search['branch_id'])?null:$search['branch_id'];
			$frm = new Application_Form_FrmGlobal();
			$this->view-> rsheader = $frm->getLetterHeaderReport($branch_id);
		...
		$form=new Application_Form_FrmCombineSearchGlobal();
		$form->FormFilterExpensebyType($search);
	...
	public function rptCreditmemoAction(){
		try{
			...
			$this->view->all_memo= $db->getAllStudentWalletBalance($formdata);
...
// 			$this->view->rsheader = $frm->getLetterHeaderReport($branch_id);
	...
	public function rptCreditTransactionAction(){
		try{
    		if($this->getRequest()->isPost()){
    			$formdata=$this->getRequest()->getPost();
    		}
    		...
    		$_db = new Allreport_Model_DbTable_DbAccountReport();
			$this->view->search = $formdata;
			$this->view->result = $_db->getCreditTransaction($formdata);
			...
			$form = new Application_Form_FrmCombineSearchGlobal();
			$frm = $form->FormSearchCreditTransaction($formdata);
			Application_Model_Decorator::removeAllDecorator($frm);
			$this->view->frm_search = $frm;
	...
	public function rptCreditExpiredAction(){
		try{
    		if($this->getRequest()->isPost()){
    			$formdata=$this->getRequest()->getPost();
    		}
    		...
    		$_db = new Allreport_Model_DbTable_DbAccountReport();
			$this->view->search = $formdata;
			$this->view->result = $_db->getCreditTransactionExpired($formdata);
	...
	public function confirmExpiredAction(){
		try{
			if($this->getRequest()->isPost()){
				$data=$this->getRequest()->getPost();

				$_db = new Accounting_Model_DbTable_DbCreditmemo();
				$_db->issueExpiredCredit($data);
				
				Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/allreport/accounting/rpt-credit-expired");
				exit();
			}
		}catch (Exception $e){
			Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
			Application_Form_FrmMessage::Sucessfull('INSERT_FAIL', "/allreport/accounting/rpt-credit-expired");	
    		
    	}
	}
	
	public function rptReferralAction(){
		...
		$form = new Application_Form_FrmCombineSearchGlobal();
    	$frm = $form->FormSearchReferralReport();
    	Application_Model_Decorator::removeAllDecorator($frm);
    	$this->view->form_search = $frm;
		
		
		$dbAcc= new Allreport_Model_DbTable_DbAccountReport();
		$this->view->rs = $dbAcc->getReferralReport($search);
		$this->view->subRs = $dbAcc->getSubReferral($search);
		$this->view->search=$search;
	...
	function updatereferredAction(){
		$db = new Allreport_Model_DbTable_DbAccountReport();
     	if($this->getRequest()->isPost()){
    		$_data = $this->getRequest()->getPost();
     		try{
    			$result = $db->updateReferred($_data);
    			print_r(Zend_Json::encode($result));
    			exit();
     		} catch (Exception $e) {
     			Application_Form_FrmMessage::message("UPDATE_FAIL");
