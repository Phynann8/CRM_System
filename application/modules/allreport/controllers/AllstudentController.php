...
class Allreport_AllstudentController extends Zend_Controller_Action {
	...
	public function printIdcardAction(){//using
		...
		$cardSetting=$this->getRequest()->getParam('cardSetting');
		$this->view->cardSetting = empty($cardSetting)?1:$cardSetting;
	...
	public function rptStudentcardAction(){//using
		...
		$this->view->bgCardSetting = $dbc->getAllStudentCard();
	}
	public function studentGroupAction()
	{
		...
		$this->view->controlReport = $frm->reportControlWhite($ctrOpt);
		
		$arr=array(
			"footerLeftContent" => "",
		);
		$this->view->printFooter = $arr;
	...
	public function rptStudentListAction()
	{
		...
		$forms=$form->FormSearchStudentListReport();
		Application_Model_Decorator::removeAllDecorator($forms);
		$this->view->form_search=$form;
	
		$db = new Application_Model_DbTable_DbGlobal();
		$this->view->study_type = $db->getViewByType(5,0);
		$this->view->branchInfo = $db->getBranchInfo($branchId);
	...
	public function rptAllAmountStudentAction(){
		...
		$form=$frm->FormSearchStudentReport();
	...
	public function rptStudentAction(){
		...
		$textTran = "initial";
		$qrReport = empty($search['qrReport']) ? 0 : $search['qrReport'];
		$arr=array(
			"footerLeftContent" => "",
			"footerLeftTextTransform" => $textTran,
			"printFunction" => ($qrReport==1) ? "doPrintQr()" : "doPrint()",
		);
		//"printFunction" => "doPrint('#viewer')",
		$this->view->printFormat = $frm->getPrintPageFormat($arr);
		$this->view->controlReport = $frm->reportControl($arr);
	}
	...
	public function rptStudentDropAction(){
		...
		$form=new Application_Form_FrmCombineSearchGlobal();
		$forms=$form->FormSearchStuDropReport();
		Application_Model_Decorator::removeAllDecorator($forms);
		$this->view->form_search=$form;
	...
	public function rptStudentChangeBranchAction(){
		if($this->getRequest()->isPost()){
			$search=$this->getRequest()->getPost();
		}else{
		...
		$group= new Allreport_Model_DbTable_DbRptAllStudent();
		$this->view->rs = $group->getStudentChangeBranch($search);
		$this->view->search=$search;
