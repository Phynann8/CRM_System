...
class Allreport_ScoreController extends Zend_Controller_Action
{
	...
	function getLunadateAction()
	{
		if ($this->getRequest()->isPost()) {
			$data = $this->getRequest()->getPost();

			if (!empty($data['lunarDate'])) {
				$lunar = new Application_Model_DbTable_DbLunaCalendar($data['lunarDate']);

				$lnDate = $lunar->toLunarDate('ថ្ងៃW dN  ខែm ឆ្នាំa e ព.ស. b');
				$lnYear = $lunar->toLunarDate('ព.ស. b');
