...
class Allreport_Model_DbTable_DbMistakeCertificate extends Zend_Db_Table_Abstract
{
	public function getStudentInfoMistake($group_id,$stu_id){
		...
		$sql="SELECT 
					...
					gsd.group_id AS groupId,
					,gsd.academic_year AS academicYearId
    ...
    function getAttendenceBydate($semester,$group_id,$stu_id){
		...
		$where.=" GROUP BY 
				  CASE  
					WHEN sta.type = 1 THEN sta.id
					ELSE sade.id
				  END
		ORDER BY sta.`date_attendence` ASC,sade.attendence_status DESC ";
		*/
		
		$where.=" GROUP BY 
				  CASE  
					WHEN sta.type = 1 THEN sta.date_attendence
					ELSE sade.id
				  END
		ORDER BY sta.`date_attendence` ASC,sade.attendence_status DESC ";
	...
	function getAttendenceBydateNew($semester,$data=[]){
    	$db = $this->getAdapter();
		$academic_year = empty($data['academic_year']) ? 0 : $data['academic_year'];
		$stu_id = empty($data['stu_id']) ? 0 : $data['stu_id'];
    	$sql="SELECT 
					sade.*,
    				sta.`date_attendence`,
					sta.`group_id`,
					...
					(SELECT st.`for_session` FROM `rms_student_attendence` AS st WHERE st.id = sade.`attendence_id` LIMIT 1) AS `for_session`
				FROM 
					rms_student_attendence_detail AS sade JOIN `rms_student_attendence` AS sta ON sta.`id` = sade.`attendence_id`
					LEFT JOIN rms_group as g ON g.id = sta.`group_id`
				WHERE  1 
					AND sta.status = 1
				";
    	$where = " AND sta.for_semester=".$semester;
    	$where.=" AND sade.`stu_id`=$stu_id AND g.`academic_year`=$academic_year ";
		$where.=" GROUP BY 
				  CASE  
					WHEN sta.type = 1 THEN sta.id
					ELSE sade.id
