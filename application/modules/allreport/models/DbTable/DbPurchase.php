...
class Allreport_Model_DbTable_DbPurchase extends Zend_Db_Table_Abstract
{
    ...
    function getPurchaseCodeSuplier($search=null){
    	...
    	$sql="SELECT 
			    ...
			    WHERE sp.supplierId=s.id  ";//AND sp.status=1
    	...
    	if($search['supplier_id']>0){
    		$where.=" AND sp.supplierId=".$search['supplier_id'];
    	}
    ...
    function getPurchaseName($id){
    	$db=$this->getAdapter();
    	$sql="SELECT sp.purchaseNo,s.sup_name,
		       ...
		       WHERE s.id=sp.supplierId AND sp.supplierId=$id LIMIT 1";
    	return $db->fetchRow($sql);
    }
    	...
    	$db=$this->getAdapter();
    			$sql=" SELECT  
			            	...
			            	spd.cost,spd.amount,spd.date,
		       				...
		       				WHERE sp.id=spd.supproduct_id AND s.id=sp.supplierId AND spd.supproduct_id=$pro_id";
    			$where="";
    ...
    function  getAllPurchaseSumarryReport($search=null){
    	$db=$this->getAdapter();
    	...
    	$sql=" SELECT  
		           ...
		           (SELECT $branch FROM rms_branch WHERE rms_branch.br_id=sp.branch_id LIMIT 1) AS brand_name ,
					...
					spd.date,
       				...
       				FROM 
       					rms_purchase AS sp
	       				INNER JOIN rms_purchase_detail AS spd ON sp.id=spd.supproduct_id AND sp.status=1
	       				LEFT JOIN rms_supplier AS s ON s.id=sp.supplierId
	       				LEFT JOIN rms_itemsdetail AS pro ON pro.id=spd.pro_id
       				WHERE 1 ";
    ...
    function getSuplierPuchaseBalance($search=null){
    	...
    	if($search['paymentstatus']>-1){
			$where .= " AND sp.is_paid =".$search['paymentstatus'];
    	}
    	if($search['supplier_id']>0){
    		$where.=" AND sp.supplierId=".$search['supplier_id'];
    	}
