...
//      		$where .=" and (SELECT $item FROM `rms_items` WHERE rms_items.id=".$search['service'].")";
	...
	function getAllStudentNearlyEndService($search){
    	...
    	$sql="SELECT 
				...
				,CASE 
					WHEN vSt.primaryPhone = 1 THEN vSt.`tel`
					WHEN vSt.primaryPhone = 2 THEN COALESCE(NULLIF(f.fatherPhone,''),vSt.tel)
					WHEN vSt.primaryPhone = 3 THEN COALESCE(NULLIF(f.motherPhone,''),vSt.tel)
					WHEN vSt.primaryPhone = 4 THEN COALESCE(NULLIF(f.guardianPhone,''),vSt.tel)
				END AS tel
				...
				,v.paidamount
				,CASE 
					WHEN v.paymentTerm !=6 THEN (SELECT title FROM `rms_startdate_enddate` WHERE id=v.academicFeeTermId LIMIT 1)
			...
			FROM v_stu_record_info AS vSt
				...
				LEFT JOIN rms_family AS f ON f.id = vSt.familyId
		...
		} elseif ($search['nearlyFilterType'] == 3) {
			$toDatePmt = (empty($search['end_date']))? '1': "v.`endDate` >= '".$search['end_date']." 23:59:59'";
			$sql.=" AND  
						CASE 
							WHEN (NULLIF(v.`endDate`,'') IS NOT NULL OR v.`endDate` !='0000-00-00')
		...
		if(!empty($search['branchList'])){
			...
			$where.= " AND (FIND_IN_SET(vSt.branchId,'" . $branchList . "' ) ) ";
		}
     	if(!empty($search['study_year'])){

			$where.=" AND ( 
				(vSt.`itemType` = 1 AND vSt.`academicYear` = ".$search['study_year'].")
				OR
				(vSt.`itemType` <> 1 AND  v.academicYear = ".$search['study_year'].")
		   )";
		}
		if(!empty($search['academic_year'])){

			$where.=" AND ( 
				(vSt.`itemType` = 1 AND vSt.`academicYear` = ".$search['academic_year'].")
				OR
				(vSt.`itemType` <> 1 AND  v.academicYear = ".$search['academic_year'].")
		   )";
		}
		if(!empty($search['group'])){

			$where .= " AND (
							(vSt.`itemType` = 1 AND vSt.`groupId` = ".$search['group'].") 
								OR 
							(vSt.`itemType` <> 1 AND dg.group_id = ".$search['group'].")
						) ";
		}
		if(!empty($search['multiDegree'])){
			$multiDegree = implode(",",$search['multiDegree']);
			$where .= " AND (
					(vSt.`itemType` = 1 AND vSt.`degree` IN (".$multiDegree.")) 
				OR 
					(vSt.`itemType` <> 1 AND dg.degree IN (".$multiDegree."))
			  ) ";
		}
		if(!empty($search['grade_all'])){

			$where .= " AND (
				(vSt.`itemType` = 1 AND vSt.`grade` = ".$search['grade_all'].") 
			OR 
				(vSt.`itemType` <> 1 AND dg.grade = ".$search['grade_all'].")
		  ) ";
		}
		$where.=" GROUP BY vst.`detailId` ";
    	$order=" ORDER BY vSt.`degree` ASC, vSt.`grade` ASC,vSt.`groupId`,vSt.`stuCode` ASC ";
