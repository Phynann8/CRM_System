...
class Allreport_Model_DbTable_DbRptSummaryStock extends Zend_Db_Table_Abstract
{
	...
	function getSummaryStockReport($search){
		...
		$where = ' WHERE 1 AND p.is_productseat=0  AND items_type =3 ';
		...
		if(!empty($search['product'])){
			$where .= " AND cd.proId = " . $search['product'];
		}
		if(!empty($search['product_type'])){
		...
		if (!empty($results)) {
			foreach ($results as $key => $result) {
				...
				$qtyReturn = $this->getReturnQtyClosingEntry($param);
				if (!empty($qtyUsage)) {
					$records[$key]['qtyReturn'] = $qtyReturn;
				}
	...
	function getReturnQtyClosingEntry($data)
	{ //usage and sale
		$sql = "SELECT 
					SUM(req_d.`qty_return`) 
				FROM
					rms_request_order AS req,
					rms_request_orderdetail AS req_d 
				WHERE 
					req.id = req_d.request_id 
					AND req.status=1  AND req.`isReturn` = 1 ";
		if (!empty($data['start_date'])) {
			$from_date = (empty($data['start_date'])) ? '1' : " req.returnDate  >= '" . $data['start_date'] . " 00:00:00'";
			$to_date = (empty($data['end_date'])) ? '1' : " req.returnDate  <= '" . $data['end_date'] . " 00:00:00'";
			$sql .= " AND " . $from_date . " AND " . $to_date;
		}
    ...
    public function getAllStudentProduct($search,$new=null){
    	try{
			...
			$branch = $dbp->getBranchDisplay();
    		$stuname=" CONCAT(s.last_name,' ',s.stu_enname)";
    		if ($currentLang==1){
	...
	function studentGetProduct($search){
    	...
    	try{
			...
			(SELECT pl.pro_qty FROM `rms_product_location` AS pl  WHERE pl.pro_id = sd.pro_id AND pl.branch_id = p.branch_id LIMIT 1) AS current_qty,
			sd.qty, 
			sd.qty_after,
			(SELECT CONCAT(COALESCE(u.last_name,''),' ',COALESCE(u.first_name,'')) FROM rms_users AS u WHERE u.id = p.user_id LIMIT 1) AS user_enter
			FROM `rms_saledetail` AS sd JOIN `rms_student_payment` AS p ON (sd.payment_id=p.id AND p.status=1 AND p.is_void=0 )
				LEFT JOIN `rms_student` AS s ON s.stu_id = p.student_id 
    ...
    function getAllCutStockForClose($search){
    	$db = $this->getAdapter();
	    	try{
	    		...
	    		pp.*,
	    		(SELECT CONCAT(COALESCE(last_name,''),' ',COALESCE(first_name,'')) FROM rms_users as u where u.id = pp.closed_by LIMIT 1) as user_close,
	    		(SELECT CONCAT(COALESCE(last_name,''),' ',COALESCE(first_name,'')) FROM rms_users as u where u.id = pp.user_id LIMIT 1) as user_enter
	    		FROM 
		    		`rms_cutstock` AS pp,
		    		`rms_student` AS s,
    ...
    function getProductSold($search){
		...
		$sql="SELECT
    			...
    			FROM 
    				...
    				JOIN rms_saledetail sd ON sp.id = sd.payment_id AND sp.status = 1 AND sp.is_void = 0
					LEFT JOIN rms_itemsdetail AS i ON i.id = sd.pro_id AND i.items_type = 3
    			WHERE 1
    		";
		...
		if(!empty($search['user_id'])){
    		$where.= " AND sp.user_id= ".$db->quote($search['user_id']);
    	}
    	$dbp = new Application_Model_DbTable_DbGlobal();
    	$where.=$dbp->getAccessPermission('sp.branch_id');
