...
class Api_Model_DbTable_DbActions extends Zend_Db_Table_Abstract
{
			...
			if ($row['status']){
				if(!empty($row['value'])){
					...
					if($disableAccount==1){
						...
						$condictionArray['mobileToken'] = empty($_data['mobileToken'])? "":$_data['mobileToken'];
	...
	public function newsAction($search){
		...
		if ($row['status']){
			$arrResult = array(
					"result" => $row['value'],
					"newsList" => empty($row['newsList']) ? array() : $row['newsList'],
					"totalRecord" => empty($row['totalRecord']) ? 0 : $row['totalRecord'],
					"code" => "SUCCESS",
			);
		}else{
			$arrResult = array(
	...
	public function setReadNotificationAction($search){
		try{
			$search['studentId'] = empty($search['studentId'])?0:$search['studentId'];
			$search['mobileToken'] = empty($search['mobileToken'])?"":$search['mobileToken'];
	...
	public function getAchievementDetailAction($_data){
		try{
			$db = new Api_Model_DbTable_DbApi();
			$row = $db->getStudentAchievementDetail($_data);
			if ($row['status']){
				$arrResult = array(
					"result" => $row['value'],
					"code" => "SUCCESS",
	...
	public function getStudentCriteriaScoreAction($_data){
		try{
			...
			$unreadrRs = $db->getCountingUnreadCriteriaByStudent($_data);
			if ($row['status']){
				$countingUnread = empty($unreadrRs["countingUnread"]) ? "0" : $unreadrRs["countingUnread"]; 
				$arrResult = array(
					"result" => $row['value'],
					"countingUnread" => $countingUnread,
					"totalRecord" => empty($row['totalRecord']) ? 0 : $row['totalRecord'],
					"code" => "SUCCESS",
				);
			}else{
	...
	public function changePasswordBusAction($_data){
		try{
			$db = new Api_Model_DbTable_DbApi();
			$_data['oldPassword'] = empty($_data['oldPassword'])?0:$_data['oldPassword'];
			$_data['userId'] = empty($_data['userId']) ? 0:$_data['userId'];
			$row = $db->checkChangePasswordBus($_data);
			if (empty($row)){
				$arrResult = array(
					"code" => "FAIL",
					"message" => "INVALID_OLD_PASSWORD",
				...
				$_data["driverId"] = empty($row["driverId"]) ? 0 : $row["driverId"];
				$rowRs = $db->changePasswordBus($_data);
				if (!$rowRs){
					$arrResult = array(
						"code" => "FAIL",
						"message" => "UNABLE_TO_CHANGE_PASSWORD",
	...
	public function getStPmtValidationInfoAction($_data){
		try{
			
			$search['studentId'] = empty($search['studentId'])?0:$search['studentId'];
			$search['currentLang'] = empty($search['currentLang'])?1:$search['currentLang'];
			$db = new Api_Model_DbTable_DbApi();
			$displayPmtValidate="0";
			$rsSetting = $db->getMobileKeySetting("displayPmtValidate");
			if(!empty($rsSetting)){
				$displayPmtValidate = empty($rsSetting["keyValue"]) ? "0" : $rsSetting["keyValue"];
			}
			if($displayPmtValidate=="1"){
				$row = $db->getStPmtValidationInfo($_data);
				if ($row['status']){
					$arrResult = array(
						"result" => $row['value'],
						"totalRecord" => empty($row['totalRecord']) ? 0 : $row['totalRecord'],
						"displayPmtValidate" => $displayPmtValidate,
						"code" => "SUCCESS",
					);
				}else{
					$arrResult = array(
	...
	public function updateTokenStatusAction($search){
		try{
			$search['studentId'] = empty($search['studentId'])?0:$search['studentId'];
			$search['mobileToken'] = empty($search['mobileToken'])?"":$search['mobileToken'];
			$db = new Api_Model_DbTable_DbApi();
			$row = $db->updateTokenStatus($search);
			if ($row['status']){
				$arrResult = array(
						"result" => $row['value'],
						"code" => "SUCCESS",
