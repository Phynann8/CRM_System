...
// 		$db->beginTransaction();
	...
	//s.stu_khname,
			$sql ="SELECT
						...
						CONCAT(COALESCE(s.last_name,''),' ',COALESCE(s.stu_enname,'')) as studentNameEn,
						s.stu_khname AS studentNameKh,
						s.stu_enname AS firstName,
    	...
    	try{
		    	...
		    	$sqlSelect="SELECT
					...
					,CASE
						...
						END AS isRead
		    	";
		    	$sqlFrom=" FROM `mobile_news_event` AS act  ";
				
				$stuId = empty($search['studentId'])?0:$search['studentId'];
				$sqlFrom.=" LEFT JOIN `mobile_news_event_read` AS re ON act.`id` = re.newsId  AND re.`stuId` =$stuId ";
				
		    	$sqlWhere=" WHERE act.`status` =1 ";
		    	$sqlOrder= "  ORDER BY act.publish_date DESC,act.`id` DESC";
		    	
		    	if (!empty($search['limit'])){
		    		$sqlOrder.= "  LIMIT ".$search['limit'];
		    	}
				
				//New Added
				if(!empty($search['LimitStart'])){
					$sqlOrder.=" LIMIT ".$search['LimitStart'].",".$search['limitRecord'];
				}else if(!empty($search['limitRecord'])){
					$sqlOrder.=" LIMIT ".$search['limitRecord'];
				}
				if(!empty($search['isFeature'])){
					$sqlWhere.=" AND act.`is_feature` =".$search['isFeature'];
				}
				...
				$merchResult = array('feature_news'=>array(),'normal_news'=>$row);
		   
			$sqlCount="SELECT  COUNT(act.id) AS totalRecord ";
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$totalRecord = $db->fetchOne($sqlCount);
...
//     }
    		...
    		try{
				...
				$sqlSelect="
					SELECT
						...
						,br.branch_namekh AS branchNameKh
						,br.branch_nameen AS branchNameEn
						,g.group_code AS groupCode
						,(SELECT CONCAT(ac.fromYear,'-',ac.toYear) FROM `rms_academicyear` AS ac WHERE ac.id = g.academic_year LIMIT 1) AS academicYear
							...
							'[',
								COALESCE(
									GROUP_CONCAT(
										...
										ORDER BY vdst.`dateAttendance` DESC
									),
									''
								),
							']'
				...
				$sqlFrom="
					FROM
						...
						LEFT JOIN `rms_branch` AS br  ON br.br_id = sat.branch_id 
					";
				...
				$where="";
				if($type==2){
					$where.=" AND vdst.`studentId` = $studentId ";
				}else{
					$where.=" AND sat.group_id IN (".$groupList.")";
				}
				if(!empty($search['month'])){
					$where.=" AND DATE_FORMAT(sat.`date_attendence`,'%m') = ".$search['month'];
				}
				if(!empty($search['forSemester'])){
					$where.=" AND sat.for_semester = ".$search['forSemester'];
				}
				if(!empty($search['academicYear'])){
				...
				$groupBy=" GROUP BY sat.`group_id` ,DATE_FORMAT(sat.`date_attendence`,'%Y%m') ";
				$groupBy.=" ORDER BY DATE_FORMAT(sat.`date_attendence`,'%Y%m') DESC ";
				
				
				if(!empty($search['LimitStart'])){
					$groupBy.=" LIMIT ".$search['LimitStart'].",".$search['limitRecord'];
				}else if(!empty($search['limitRecord'])){
					$groupBy.=" LIMIT ".$search['limitRecord'];
				}
				$row = $db->fetchAll($sql.$where.$groupBy);
				
				$sqlCount="SELECT 
						COUNT(DISTINCT DATE_FORMAT(sat.`date_attendence`,'%Y%m')) AS totalRecord
						";
				$sqlCount.=$sqlFrom;
				$sqlCount.=$sqlWhere;
				$sqlCount.=$where;
		    	...
		    	if(empty($search['newVersion'])){
					...
					$sql="
						SELECT 
							...
							,br.branch_namekh AS branchNameKh
							,br.branch_nameen AS branchNameEn
							,br.school_nameen AS schoolNameen
							,br.photo AS branchLogo
						...
						FROM 
							...
							JOIN  rms_group AS g ON g.id =sch.group_id
								...
								LEFT JOIN `rms_branch` AS br  ON br.br_id = g.branch_id 
					...
					";
					$sql.="
					FROM `vapi_study_schedule` AS sch 
						LEFT JOIN `rms_academicyear` AS ac ON ac.id = sch.`academicYearId`
						LEFT JOIN rms_branch AS b ON b.br_id=sch.`branchId`
						LEFT JOIN rms_group_detail_student AS grd ON grd.group_id =sch.groupId AND grd.`itemType` = 1
						LEFT JOIN `rms_room` AS r ON r.`room_id` = sch.`roomId`
					WHERE 
						1
						AND grd.`stu_id` = $studentId 
					";
					$sql.=" GROUP BY sch.`branchId`,sch.`academicYearId` ,sch.`groupId` ";
					$sql.=" ORDER BY sch.`academicYearId` DESC ,sch.`scheduleId` DESC ,sch.`groupId` ";
					$row = $db->fetchAll($sql);
				}
				$result = array(
					'status' =>true,
	...
	public function getStudentScore($search){
		...
		try{
			...
			$sqlSelect="
				SELECT
					s.*
					,DATE_FORMAT(s.date_input,'%Y%m%d') AS dateInputValue
					,s.$titleScore AS title_score
					,st.`stu_id`
					,g.`branch_id`
					,b.$branch AS branchName
					,b.photo AS branchLogo
					,b.school_namekh AS schoolNameKh
					,b.school_nameen AS schoolNameEng
				
					,g.`group_code` AS groupCode
					,`g`.`degree` as degreeId
					,CONCAT(ac.fromYear,'-',ac.toYear) AS academicYearTitle
					,ite.$degree AS degreeTitle
					,ited.$grade AS gradeTitle
					
					,`g`.`semester` AS `semester`
					,`r`.`room_name` AS roomName
					,pt.$sessionTitle AS session
					
					,t.$teacherName AS teacherName
					,t.teacher_name_kh AS teacherNameKh
					,t.teacher_name_en AS teaccherNameEng
					,t.signature AS teacherSigature
					,t.tel AS teacherTel
					...
					,CASE 
						...
						ELSE (SELECT 
								...
								WHERE mst.id = mstd.metion_score_id
									...
									AND mst.degree = g.degree 
									AND mst.status = 1 
									AND ((sm.total_avg/g.max_average)*100) >= mstd.max_score
								ORDER BY mstd.max_score DESC LIMIT 1
							)
			...
			";
			$sqlFrom= "FROM 
							`rms_score` AS s JOIN `rms_score_monthly` AS sm ON s.`id`=sm.`score_id` AND sm.type = 1 
							LEFT JOIN `rms_student` AS st ON st.`stu_id`=sm.`student_id`
							...
							LEFT JOIN `rms_items` AS ite ON `ite`.`id`=`g`.`degree` AND `ite`.`type`=1
							LEFT JOIN `rms_itemsdetail` AS ited ON ited.`id`=`g`.`grade` AND ited.`items_type`=1
			...
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$sqlCount.=" AND st.stu_id=".$studentId;
			$sqlCount.=$where;
			$totalRecord = $db->fetchOne($sqlCount);
			$totalRecord = empty($totalRecord) ? 0 : $totalRecord;
	...
	public function getScoreInformation($search){
		...
		try{
			...
			$sql="SELECT
				...
				,COALESCE((SELECT vSt.`keyValue` FROM `rms_setting` AS vSt WHERE vSt.`keyName`='mobileShareTranscript' LIMIT 1),0) AS isShareTranscript
			...
			FROM
				...
				LEFT JOIN rms_teacher AS t ON t.id = g.teacher_id 
				LEFT JOIN rms_parttime_list AS pt ON pt.id=g.session 
				LEFT JOIN `rms_items` ite ON ite.`id`=`g`.`degree` AND ite.`type`=1 
				LEFT JOIN `rms_itemsdetail` AS ited ON ited.`id`=`g`.`grade` AND ited.`items_type`=1 
		...
		try{
			...
			$sqlSelect=" 
				SELECT 
	    			...
	    			,CONCAT((SELECT CONCAT(COALESCE(fromYear,''),'-',COALESCE(toYear,'')) FROM rms_academicyear WHERE rms_academicyear.id=tf.academic_year LIMIT 1),'(',COALESCE(tf.generation,''),')') AS academicYear
			...
			$sqlFrom=" 
				 FROM 
    				rms_student AS s JOIN rms_student_payment AS sp ON s.stu_id=sp.student_id 
					LEFT JOIN rms_tuitionfee AS tf ON tf.id=sp.academic_year
					LEFT JOIN `vapi_pmtdetail` AS ptd ON ptd.`paymentId` = sp.`id` 
				WHERE 
					sp.is_void = 0
			";
	    	...
	    	if(!empty($search['academicYear'])){
	    		$sqlWhere.=" AND tf.academic_year=".$search['academicYear'];
	    	}
			if(!empty($search['paymentMethod'])){
	    		$sqlWhere.=" AND sp.payment_method=".$search['paymentMethod'];
	    	}
			$sqlWhere.=" AND sp.student_id = ".$studentId;
			...
			if(!empty($search['unreadRecord'])){
				if($search['unreadRecord']==1){ //unread
					$sqlWhereUn.=" AND sp.`isRead` =0 ";
				}else if($search['unreadRecord']==2){ //read
					$sqlWhereUn.=" AND sp.`isRead` =1 ";
				}else if($search['unreadRecord']==3){ //all
					$ordering=" ORDER BY sp.`isRead` ASC,sp.create_date DESC";
				}
				$sql=$sqlSelect.$sqlFrom.$sqlWhere.$sqlWhereUn.$ordering.$limit;
				return  $db->fetchAll($sql);
			}
			$sql=$sqlSelect.$sqlFrom.$sqlWhere.$sqlWhereUn.$ordering.$limit;
			$row = $db->fetchAll($sql);
			
			$sqlCount="SELECT  COUNT(sp.id) AS totalRecord ";
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$totalRecord = $db->fetchOne($sqlCount);
			$totalRecord = empty($totalRecord) ? 0 : $totalRecord;
	...
	public function getAllMobileNotification($search){
    		...
    		try{
				...
				if($studentId>0){
					...
					$whereIsRead = " AND ntf_r.studentId = '$studentIdValue' AND ntf_r.type = 1 ";
					...
					$joinTableWhere=" 
						AND CASE 
							...
							ELSE ntf.student
						END IN ($studentId)  ";
					$joinTableWhere.=" AND ntf.opt_notification != 5 ";
						
				}else if($teacherId>0){
					$studentId = "0,".$studentId;
					$whereIsRead = " AND ntf_r.studentId = '$teacherId' AND ntf_r.type = 3 ";
					$joinTableWhere=" AND ntf.opt_notification IN (1,5)";
				}else{
					$joinTableWhere=" AND ntf.opt_notification = 1 ";
				}
				
    			$sqlSelect=" SELECT ntf.id, 
							ntf_d.title
							,ntf_d.title AS announcementTitle
							
							";
							$sqlSelect.=$joinTableQuery;
							$sqlSelect.="
							,ntf.group,
							ntf.date AS publicDate,
							ntf.date AS publishDate,
							COALESCE(DATE_FORMAT(ntf.date, '%Y%m%d'),'') AS publicDateFmt,
							ntf_d.description,
				...
				$sqlFrom="FROM `mobile_notice` AS ntf 
							JOIN `mobile_notice_detail` AS ntf_d ";
				$sqlFrom.=$joinTable;	
				
				$sqlWhere=" WHERE 
							ntf_d.notification_id = ntf.id 
							AND ntf_d.lang = ".$currentLang;
				$sqlWhere.=$joinTableWhere;	
				
				
				if(!empty($isCounting)){
					$dateToken = empty($tokenInfo['date'])?"2023-01-01 00:00:00":$tokenInfo['date'];
					if($studentIdValue==0){
						$sqlWhere.=" AND ntf.date >= '$dateToken' ";
					}
					$sqlWhere.=" AND COALESCE((SELECT ntf_r.isRead FROM mobile_notify_read AS ntf_r WHERE ntf_r.notification_id = ntf.id ".$whereIsRead." ORDER BY ntf_r.isRead DESC LIMIT 1),0) =0 ";
				}
				
				$sqlOrder=" ORDER BY ntf.date DESC ";
				if(!empty($search['LimitStart'])){
					$sqlOrder.=" LIMIT ".$search['LimitStart'].",".$search['limitRecord'];
				}else if(!empty($search['limitRecord'])){
					$sqlOrder.=" LIMIT ".$search['limitRecord'];
				...
				$sqlCount="SELECT  COUNT(ntf.id) AS totalRecord ";
				$sqlCount.=$sqlFrom;
				$sqlCount.=$sqlWhere;
	...
	function checkIsRead($studentId,$notificationId,$type=0){
		$db = $this->getAdapter();
		$sql="SELECT 
				ntf_r.* FROM mobile_notify_read AS ntf_r 
			WHERE 
				ntf_r.studentId = $studentId 
				AND ntf_r.notification_id=$notificationId 
				AND ntf_r.type=$type 
			LIMIT 1 ";
		return $db->fetchRow($sql);
	}
	...
	function getReadAndUnreadCompateTokenUser($_data){
		$db = $this->getAdapter();
		...
		$sql="SELECT 
				...
				,b.`isRead`
			FROM  
				`mobile_notify_read` AS b 
				LEFT JOIN `mobile_notify_read` AS bb ON bb.`studentId` = b.`studentId` 
					AND bb.`mobileToken` = '$mobileToken'  
					AND bb.`notification_id` = b.`notification_id`
					AND bb.`type` = b.`type`
			WHERE b.`mobileToken` != '$mobileToken' 
			AND bb.`mobileToken` IS NULL
			AND b.`studentId` = $studentId 
			
			GROUP BY b.`notification_id`
		";
		return $db->fetchAll($sql);
	}
				...
				if($readType=="markAllRead"){ 
					...
					if(!empty($rowNotify['value'])){
						foreach($rowNotify['value'] as $rs){
							...
							if($studentId>0){
								$check = $this->checkIsRead($userId,$rs['id'],$type);
								if(empty($check)){
									$this->insert($_arr);
								}
							}else{
						...
						if(!empty($readyByToken)){
							foreach($readyByToken as $read){
								if($userId>0){
									$check = $this->checkIsRead($userId,$read['notification_id'],$type);
									if(empty($check)){
										$_arr=array(
											'studentId'			=> $userId,						
											'type'				=> $type,						
						...
						if(!empty($userId) && !empty($mobileToken) ){
							$arr = array(
								'mobileToken' => $mobileToken,
								'studentId' => $userId,
							);
							$userRead = $this->getReadAndUnreadCompateTokenUser($arr);
							if(!empty($userRead)){
								foreach($userRead As $row){
									$_arr=array(
					...
					if($notificationId>0){ 
						...
						$this->_name = "mobile_notify_read";
						if($studentId>0){
							$check = $this->checkIsRead($userId,$notificationId,$type);
							if(empty($check)){
								$this->insert($_arr);
							}
						}else{
	...
	*/
	...
	function getAllGroupStudyByStudent($_data){
		...
		$sql="SELECT 
				...
				,CONCAT(COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,'')) AS academicYear
			FROM 
				`rms_group_detail_student` AS gds JOIN `rms_group` AS g ON g.id = gds.group_id AND gds.`itemType` = 1
				LEFT JOIN `rms_academicyear` AS ac ON ac.id = g.`academic_year` 
		...
		$sql .=' ORDER BY g.`academic_year` DESC,g.degree DESC,gds.is_current DESC';	
		return $db->fetchAll($sql);
	}
	...
	public function getAllStudentCreditMemo($search){
		...
		try{
			...
			$sqlSelect=" SELECT 
					...
					END as isExpired
				";
			$sqlFrom=" FROM  rms_creditmemo AS credit ";
			$sqlWhere=" WHERE  credit.status = 1 ";
			//$expireDate =" credit.end_date >= '".date("Y-m-d")." 23:59:59'";
			//$sql.= " AND ".$expireDate;
			$sqlWhere.=" AND credit.student_id = ".$studentId;

			$sqlOrder=" ORDER BY credit.id DESC";
			$limit=" ";
			if(!empty($search['LimitStart'])){
				$limit.=" LIMIT ".$search['LimitStart'].",".$search['limitRecord'];
			}else if(!empty($search['limitRecord'])){
			...
			$sqlCount="SELECT  COUNT(credit.id) AS totalRecord ";
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$totalRecord = $db->fetchOne($sqlCount);
			$totalRecord = empty($totalRecord) ? 0 : $totalRecord;
							...
							COALESCE(
								(SELECT 
									...
									WHERE mst.id = mstd.metion_score_id
										...
										AND mst.degree = g.degree 
										AND mst.status = 1 
										AND ((sd.`score`/ (gdSub.semester_max_score*COALESCE(gdSub.amount_subject_sem,1)) )*100) >= mstd.max_score
									ORDER BY mstd.max_score DESC LIMIT 1
								),''
							)
						ELSE 
							COALESCE(
								(SELECT 
									...
									WHERE mst.id = mstd.metion_score_id
										...
										AND mst.degree = g.degree 
										AND mst.status = 1 
										AND ((sd.`score`/ (gdSub.max_score*COALESCE(gdSub.amount_subject,1)) )*100) >= mstd.max_score
									ORDER BY mstd.max_score DESC LIMIT 1
								),''
							)
					END AS mentionGradeTitle
	...
	public function getTotalStudentAttendance($search){
		...
		try{
			...
			$groupInfo = $this->getGroupInfoById($search);
			$row = array(
	...
	function getStudentRequestPermission($_data){
		...
		try{
			...
			if(!empty($_data['degreeId'])){
				$sqlWhere.=" AND g.degree = ".$_data['degreeId'];	
			}
			if(!empty($_data['requestStatus'])){
				...
				$sqlWhere.=" AND srq.requestStatus = ".$requestStatus;	
			}
			$sqlOrder=" ORDER BY srq.id DESC ";
			...
			$sqlCount="SELECT  COUNT(DISTINCT srq.id) AS totalRecord ";
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$totalRecord = $db->fetchOne($sqlCount);
			$totalRecord = empty($totalRecord) ? 0 : $totalRecord;
	...
	function getStudentAchievement($_data){
		...
		try{
			...
			$sql=$sqlQuery.$sqlFrom.$sqlWhere.$sqlOrder;
			$row = $db->fetchAll($sql.$limit);
			
			
			$sqlCount="SELECT  COUNT(ac.id) AS totalRecord ";
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$totalRecord = $db->fetchOne($sqlCount);
			$totalRecord = empty($totalRecord) ? 0 : $totalRecord;
	...
	function getStudentAchievementDetail($_data){
		$db = $this->getAdapter();
		try{
	...
	public function getStudentCriteriaScore($search){
		...
		try{
			...
			$sqlSelect="SELECT
					...
					,COALESCE(grdTmpD.`isRead`,'0') AS isRead
			...
			if(!empty($search['academicYear'])){
				$sqlWhere.=" AND g.academic_year = ".$search['academicYear'];
			}
			if(!empty($search['groupId'])){
				$sqlWhere.=" AND g.id = ".$search['groupId'];
			}
			if(!empty($search['forSemester'])){
				$sqlWhere.=" AND grdTmp.forSemester = ".$search['forSemester'];
			}
			if(!empty($search['examType'])){
				$sqlWhere.=" AND grdTmp.examType=".$search['examType'];
				if($search['examType']==1){
					if(!empty($search['month'])){
						$sqlWhere.=" AND grdTmp.forMonth=".$search['month'];
					}	
				}
			}
			if(!empty($search['criteriaId'])){
				$sqlWhere.=" AND grdTmp.`criteriaId` = ".$search['criteriaId'];
			}
			if(!empty($search['subjectId'])){
				$sqlWhere.=" AND grdTmp.`subjectId` = ".$search['subjectId'];
			}
			...
			$sql = $sqlSelect.$sqlFrom.$sqlWhere.$ordering.$limit;
			$row = $db->fetchAll($sql);
			
			
			$sqlCount="SELECT  COUNT(grdTmp.id) AS totalRecord ";
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$totalRecord = $db->fetchOne($sqlCount);
			$totalRecord = empty($totalRecord) ? 0 : $totalRecord;
	...
	public function setReadCriteria($_data){
		$db = $this->getAdapter();
		try{
			$readType 	= empty($_data['readType'])? "1" : $_data['readType'];
			$mobileToken 	= empty($_data['mobileToken'])?0:$_data['mobileToken'];
			$studentId 		= empty($_data['studentId'])?0:$_data['studentId'];
			...
			$countingUnread = $this->getCountingUnreadCriteriaByStudent($_data);
			$result = array(
					'status' =>true,
					'value' =>$countingUnread,
			);
			return $result;
		}catch(Exception $e){
			Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
	...
	function getCountingUnreadCriteriaByStudent($_data){
		$db = $this->getAdapter();
		$studentId = empty($_data['studentId'])?0:$_data['studentId'];
		$currentLang = empty($_data['currentLang'])?1:$_data['currentLang'];
		...
		$sql="
			SELECT 
				COUNT(grdTmpD.`id`) AS countingUnread 
			FROM 
				rms_grading_detail_tmp AS grdTmpD 
			WHERE grdTmpD.`isRead` = 0 AND grdTmpD.`studentId` = $studentId 
			";
		$sql .=" LIMIT 1 ";	
		return $db->fetchRow($sql);
	}
	
	function getGroupInfoById($_data){
		$db = $this->getAdapter();
		$groupId = empty($_data['groupId'])?0:$_data['groupId'];
		$currentLang = empty($_data['currentLang'])?1:$_data['currentLang'];
		...
		$sql="
			SELECT 
				...
				,(SELECT t.teacher_name_kh FROM rms_teacher AS t WHERE t.id=g.teacher_id LIMIT 1) AS teacherNameKh
				,(SELECT t.teacher_name_en FROM rms_teacher AS t WHERE t.id=g.teacher_id LIMIT 1) AS teacherNameEn
			FROM 
				rms_group AS g 
			WHERE g.id = $groupId 
			";
	...
	function checkChangePasswordBus($_data){
		$db = $this->getAdapter();
		try{
			...
			$sql.= " AND ".$db->quoteInto('dri.password=?', md5($_data['oldPassword']));
			$row = $db->fetchRow($sql);
	...
	function changePasswordBus($_data){
		$db = $this->getAdapter();
		try{
			$_arr=array(
					'password'	  	=> md5($_data['newPassword']),
			...
			Application_Model_DbTable_DbUserLog::writeMessageError($where);
			$this->update($_arr, $where);
			return true;
		}catch(Exception $e){
			Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
	...
	public function getStPmtValidationInfo($search){
		$db = $this->getAdapter();
		try{
			$currentLang = empty($search['currentLang'])?1:$search['currentLang'];
			$studentId = empty($search['studentId'])?0:$search['studentId'];
			...
			SELECT 
				...
				,vSt.`grade`
				,vSt.`stopType`
				,CASE WHEN (NULLIF(vSt.`endDate`,'') IS NOT NULL OR vSt.`endDate` !='0000-00-00') THEN vSt.`startDate` WHEN (NULLIF(v.`endDate`,'') IS NOT NULL OR v.`endDate` !='0000-00-00') THEN v.`startDate` ELSE 1 END AS startDate 
				,CASE WHEN (NULLIF(vSt.`endDate`,'') IS NOT NULL OR vSt.`endDate` !='0000-00-00') THEN vSt.`endDate` WHEN (NULLIF(v.`endDate`,'') IS NOT NULL OR v.`endDate` !='0000-00-00') THEN v.`endDate` ELSE '' END AS endDate 
				,v.`receiptNo` 
				...
				,CASE 
					...
					ELSE g.group_code 
				END AS groupCode 
				,(SELECT CONCAT(COALESCE(ac.fromYear,''),'-',COALESCE(ac.toYear,'')) FROM `rms_academicyear` AS ac WHERE ac.id =vSt.`academicYear` LIMIT 1) AS academicYearTitle
				,CASE 
					WHEN v.paymentTerm !=6 THEN (SELECT stda.title FROM `rms_startdate_enddate` AS stda WHERE stda.id=v.academicFeeTermId LIMIT 1)
					ELSE (SELECT $label FROM `rms_view` WHERE type=6 AND key_code=v.paymentTerm LIMIT 1) 
				END AS paymentTerm
				,vSt.$item AS categoryName
				,vSt.$itemDetail AS serviceTitle
				,COALESCE(vSt.`balance`,0) AS balance
				,'0' isExpired
			";
			
			$sqlFrom=" 
				FROM v_stu_record_info AS vSt  
					LEFT JOIN rms_group_detail_student dg ON dg.stu_id = vSt.`studentId` AND dg.is_maingrade = 1 AND dg.itemType = 1 AND dg.is_current=1 
					LEFT JOIN v_stu_item_pmtinfo v ON v.paymentId = ( SELECT paymentId FROM v_stu_item_pmtinfo fa WHERE fa.itemDetailId = vSt.grade AND fa.studentId = vSt.`studentId` AND fa.itemDetailId = vSt.`grade` ORDER BY `paymentId` DESC LIMIT 1 ) 
						AND v.`studentId` = vSt.`studentId` AND v.itemDetailId = vSt.`grade` 
					LEFT JOIN `rms_group` AS g ON g.id = dg.group_id 


				WHERE 
					vSt.`isCurrent`=1
					AND vSt.`isMainGrade` =1 
					AND vSt.`itemType` =1 
					AND vSt.`studentId` = $studentId 
			";
			
			$sqlWhere='';
			
			$ordering=" ORDER BY vSt.`itemType` ASC ";
			$limit=" ";
			$sql = $sqlSelect.$sqlFrom.$sqlWhere.$ordering.$limit;
			$row = $db->fetchAll($sql);
			
			
			$sqlCount="SELECT  COUNT(vSt.`studentId`) AS totalRecord ";
			$sqlCount.=$sqlFrom;
			$sqlCount.=$sqlWhere;
			$totalRecord = $db->fetchOne($sqlCount);
			$totalRecord = empty($totalRecord) ? 0 : $totalRecord;
	...
	public function updateTokenStatus($_data){
		$db = $this->getAdapter();
		try{
			if(!empty($_data['mobileToken'])){
				$_arr=array(
					'notifyStatus'	=> $_data['notifyStatus'],						
					'last_active'	  	=> strtotime("now"),
				);
				$where = " token='".$_data['mobileToken']."'";
				$this->_name = "mobile_mobile_token";
				$this->update($_arr,$where);
				$result = array(
					'status' =>true,
