...
class Foundation_GroupstudentchangegroupController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$collumns = array("BRANCH","FROM_GROUP","GRADE","TYPE","TO_GROUP","GRADE","SESSION","MOVING_DATE","NOTE","STATUS","BY","ROLLBACK");
			$link=array(
	...
	public function rollbackAction(){
		...
		if (empty($permission)){
			Application_Form_FrmMessage::Sucessfull("NO_PERMISSION_TO_ROLLBACK","/foundation/groupstudentchangegroup/index");
			exit();
		}else{
			$db= new Foundation_Model_DbTable_DbGroupStudentChangeGroup();
			$result = $db->getAllGroupStudentChangeGroupById($id);
			...
			if($result["status"]==0){
				Application_Form_FrmMessage::Sucessfull("ALREADY_DEACTIVE","/foundation/groupstudentchangegroup/index");
				exit();
			}else if($result["change_type"]==3){
				$dataChecking = [
					"changeGroupId" =>$id,
				];
				$rerurn = $db->getCheckingPmtInfo($dataChecking);
				if(!empty($rerurn)){
					if($rerurn["paymentId"]>0){
						Application_Form_FrmMessage::Sucessfull("UNABLE_TO_ROLLBACK_SOME_STUDENT_HAS_SOME_PAYMENT","/foundation/groupstudentchangegroup/index");
						exit();
					}
				}
			}
			...
			$rerurn = $db->rollBackGroupStudentChangeGroup($data);
			if($rerurn){
				Application_Form_FrmMessage::Sucessfull("SUCCESSFULLY_ROLLBACK","/foundation/groupstudentchangegroup/index");
				exit();
			}else{
				Application_Form_FrmMessage::Sucessfull("SAVE_FAIL","/foundation/groupstudentchangegroup/index");
				exit();
			}
		}
	}
