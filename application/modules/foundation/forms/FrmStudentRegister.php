...
Class Foundation_Form_FrmStudentRegister extends Zend_Dojo_Form {
	...
	public function FrmStudentRegister($data=null)
	{
		...
		$province_opt = array(
				0=>$tr->translate('SELECT_PROVINCE'),
		);
		...
		$selectiveSubject = new Zend_Dojo_Form_Element_FilteringSelect("selectiveSubject");	
		$selectiveSubject->setAttribs(array(
				'dojoType'=>$this->filter,
				'required'=>false,
				'class'=>'fullside',
				'autoComplete'=>'false',
		...
		$selectiveSubject->setMultiOptions($_db->getViewByType(45,1));

		$englishLevel = new Zend_Dojo_Form_Element_FilteringSelect("englishLevel");	
		$englishLevel->setAttribs(array(
				'dojoType'=>$this->filter,
				'required'=>false,
				'class'=>'fullside',
				'autoComplete'=>'false',
		...
		$englishLevel->setMultiOptions($_db->getAllEnglishLevel(1));
		...
		$type_opt = $_db->getViewByType(44,1);
		unset($type_opt['']);
		$goHomeType->setMultiOptions($type_opt);
		$goHomeType->setValue(2);
		...
		$lastestAca = $_dbgb->getLatestAcadmicYear();
		if(!empty($lastestAca["id"])){
			$academicYearEnroll->setValue($lastestAca["id"]);
		}
		...
		if($data!=null){
			...
			$selectiveSubject->setValue($data['selectiveSubject']);
			$englishLevel->setValue($data['englishLevel']);
