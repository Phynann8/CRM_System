...
Class Foundation_Form_FrmStudentReturn extends Zend_Dojo_Form {
	...
	public function FrmStudentReturn($data=null){
		...
		$_is_new=  new Zend_Dojo_Form_Element_FilteringSelect('is_new');
		$_is_new->setAttribs(array(
			'dojoType'=>$this->filter,
			'class'=>'fullside',
			'required'=>true,
		));
		$__is_new_opt = array(
				1=>$this->tr->translate("NEW_ID"),
				2=>$this->tr->translate("OLD_ID"),
			);
		$_is_new->setMultiOptions($__is_new_opt);
		...
		if($data!=null){
			...
			$_degree->setValue($data['degreeReturn']);
