...
class Foundation_Model_DbTable_DbFamily extends Zend_Db_Table_Abstract
{
	...
	public function updateFamily($_data){
		...
		try{	
			...
			if (!empty($name)){
				...
				if($photopj!=""){
					$oldFatherPhoto = empty($_data['oldFatherPhoto']) ? "" : $_data['oldFatherPhoto'];
					if( $oldFatherPhoto!=""){
						$thisFile = $part.$oldFatherPhoto;
			...
			if (!empty($motherPhoto)){
				...
				if($photoM!=""){
					$oldMotherPhoto = empty($_data['oldMotherPhoto']) ? "" : $_data['oldMotherPhoto'];
					if( $oldMotherPhoto!=""){
						$thisFileMotherPhoto = $part.$oldMotherPhoto;
						if (file_exists($thisFileMotherPhoto)){
							if (is_file($thisFileMotherPhoto)){
								unlink($thisFileMotherPhoto);
							}
						}
					}
				}
			...
			if (!empty($guardianPhoto)){
				...
				if($photoG!=""){
					$oldGuardianPhoto = empty($_data['oldGuardianPhoto']) ? "" : $_data['oldGuardianPhoto'];
					if( $oldGuardianPhoto!=""){
						$thisFileGuardianPhoto = $part.$oldGuardianPhoto;
						if (file_exists($thisFileGuardianPhoto)){
							if (is_file($thisFileGuardianPhoto)){
								unlink($thisFileGuardianPhoto);
							}
						}
					}
				}
	...
	function getAllFamily($search){
		...
		FROM `rms_family` AS f 
			WHERE 1
		";
		$from_date = (empty($search['start_date'])) ? '1' : "f.`createDate` >= '" . $search['start_date'] . " 00:00:00'";
		$to_date = (empty($search['end_date'])) ? '1' : "f.`createDate` <= '" . $search['end_date'] . " 23:59:59'";
	...
	function checkFatherHasInFamilyId($fatherNameKh){
		...
		";
		$sql.= $this->getAdapter()->quoteInto("AND TRIM(s.fatherNameKh)=?", TRIM($fatherNameKh));
		$sql.=" LIMIT 1 ";
		$rs = $db->fetchRow($sql);
		return $rs;
	}
	...
	function checkMotherKhHasInFamilyId($motherNameKh){
		...
		";
		$sql.= $this->getAdapter()->quoteInto("AND TRIM(s.motherNameKh)=?", TRIM($motherNameKh));
		$sql.=" LIMIT 1 ";
		$rs = $db->fetchRow($sql);
		return $rs;
	}
		...
		if(!empty($rs)){
			foreach($rs as $row){
				...
				if(!empty($check)){
					...
					$where= $this->getAdapter()->quoteInto("father_khname=?", $row["father_khname"]);
					...
					$_arr= array(
						...
						'fatherName'	=>trim($row['father_enname']),
						'fatherNameKh'	=>trim($row['father_khname']),
						...
						'motherName'	=>trim($row['mother_enname']),
						'motherNameKh'	=>trim($row['mother_khname']),
						...
						'guardianName'	=>trim($row['guardian_enname']),
						'guardianNameKh'=>trim($row['guardian_khname']),
		...
		if(!empty($rsMother)){
			foreach($rsMother as $row){
				...
				if(!empty($check)){
					...
					$this->_name = "rms_student";
					$where= $this->getAdapter()->quoteInto("mother_khname=?", $row["mother_khname"]);
		...
		if(!empty($rsEn)){
			foreach($rsEn as $row){
				...
				if(!empty($check)){
					...
					$this->_name = "rms_student";
					$where= $this->getAdapter()->quoteInto("father_enname=?", $row["father_enname"]);
					$where.= " AND (`father_khname` = '' OR `father_khname` IS NULL)";
					$this->update($_arr, $where);	
				}else{
		...
		if(!empty($rsGuard)){
			foreach($rsGuard as $row){
				...
				if(!empty($check)){
					...
					$this->_name = "rms_student";
					$where= $this->getAdapter()->quoteInto("guardian_khname=?", $row["guardian_khname"]);
					$where.= "  AND (`father_khname` = '' OR `father_khname` IS NULL) AND (`father_enname` = '' OR `father_enname` IS NULL)";
					$this->update($_arr, $where);	
				}else{
					...
					$_arr= array(
						...
						'fatherName'	=>trim($row['guardian_khname']),
						'fatherNameKh'	=>trim($row['guardian_khname']),
						'fatherPhone'	=>$row['father_phone'],
						'fatherDob'		=>empty($row['father_dob']) ? "" : $row['father_dob'],
						'fatherNation'	=>$row['father_nation'],
						'fatherJob'		=>$row['father_job'],
