	...
	public function getTeacherinfoById($data){//use
		...
		$sql = "
			SELECT g.*, 
				...
				(SELECT DISTINCT ch.`schedule_setting` AS sttId FROM `rms_group_schedule` AS ch INNER JOIN `rms_group_reschedule` AS  rch ON ch.id =  rch.main_schedule_id WHERE rch.`techer_id` = g.id AND ch.`academic_year`=".$academic_year." LIMIT 1) AS schSettingId
