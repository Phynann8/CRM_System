...
class Global_Form_FrmItems extends Zend_Dojo_Form
{
    ...
    function FrmAddDegree($data,$frmType=1){
		...
		$_arr = [
			0=>$this->tr->translate("NORMAL_COURSE"),
			1=>$this->tr->translate("EXTRA_COURSE")
		];
    	$courseType = new Zend_Dojo_Form_Element_FilteringSelect("course_type");
    	$courseType->setMultiOptions($_arr);
    	$courseType->setAttribs(array(
    			'dojoType'=>'dijit.form.FilteringSelect',
    			'required'=>'true',
    			'missingMessage'=>'Invalid Module!',
    			'class'=>'fullside'));
    	...
    	if(!empty($data)){
    		...
    		$courseType->setValue($data["isExtraCourse"]);
    	}
