...
class Global_Form_FrmParttimeList extends Zend_Dojo_Form
{
	...
	function FrmAddPartimeList($data)
	{
		...
		$rsShift = $_dbgb->getAllShift();
		if (!empty($rsShift)) {
			foreach ($rsShift AS $row) {
				$_arr_opt[$row['id']] = $row['name']; // shiftCode = value, title = text
			}
		}

		$shiftStr = new Zend_Dojo_Form_Element_FilteringSelect("shiftStr");
		$shiftStr->setMultiOptions($_arr_opt);
		$shiftStr->setAttribs(array(
			'dojoType'       => 'dijit.form.FilteringSelect',
			'autoComplete'   => 'false',
			'queryExpr'      => '*${0}*',
			'missingMessage' => 'Invalid Shift!',
			'class'          => 'fullside',
		));
		...
		if (!empty($data)) {
			...
			$shiftStr->setValue($data["shiftStr"]);
