<?php class Home_Model_DbTable_DbCRM extends Zend_Db_Table_Abstract{
			...
			if (!empty($_data['identity'])){
				...
				foreach ($ids as $i){
					...
					$array = array(
							...
							'crm_program_type'	=>$_data['program_type_'.$i],
							'age'				=> $_data['age_'.$i],
	...
	public function getCRMById($id){
		...
		$sql="SELECT st.*
				...
				,pre.schoolName AS oldSchoolName
				,(SELECT GROUP_CONCAT(COALESCE(v.$label,'')) FROM `rms_view` AS v WHERE v.type=22 AND FIND_IN_SET(v.key_code, st.prev_concern)  LIMIT 1 ) AS prevConcernTitle
		FROM `rms_crm` AS st 
			LEFT JOIN `rms_previous_school` AS pre ON pre.id = st.old_school 
			
		WHERE st.id = $id 
		";
		$sql.=$dbGb->getAccessPermission('st.branch_id');
	...
	public function getCRMDetailById($id){
		...
		$sql="SELECT 
				...
				WHEN  st.sex = 2 THEN '".$tr->translate("FEMALE")."'
				END AS sextitle,
				COALESCE((SELECT pt.title FROM `rms_program_type` AS pt WHERE pt.id = st.crm_program_type  LIMIT 1),'".$tr->translate("General")."') AS programtype_title,
	...
	public function addCrmContactHistory($_data){
		...
		try{
			...
			//update CRM
			$followupStatus = $_data['followup_status'];
			$followupStatus = empty($_data['proccess']) ? 0 : $followupStatus; 
			$_arr=array(
