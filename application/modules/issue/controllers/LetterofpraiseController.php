...
class Issue_LetterofpraiseController extends Zend_Controller_Action {
	...
	public function indexAction()
	{
		try{
			...
			$collumns = array("BRANCH","ACADEMIC_YEAR","GROUP_CODE","EXAM_TYPE","RESULT","ISSUE_DATE","TYPE","USER","STATUS");
			$link=array(
					'module'=>'issue','controller'=>'letterofpraise','action'=>'edit',
			);
			$this->view->list=$list->getCheckList(1, $collumns, $rs_rows,array('branch_name'=>$link,'academic_year'=>$link,'group_code'=>$link));
	...
	function getStudentResultAction(){
		if($this->getRequest()->isPost()){
			$data=$this->getRequest()->getPost();
			$db = new Issue_Model_DbTable_DbLetterofpraise();
			$scoreId= $data['scoreId'];
			$student = $db->getStundetScoreById($scoreId);
