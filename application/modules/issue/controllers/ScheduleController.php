...
class Issue_ScheduleController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			//$this->view->list=$list->getCheckList(1, $collumns, $rs_rows,array('branch_name'=>$link,'group_code'=>$link,'years'=>$link));
	...
	function publicschAction(){
		if($this->getRequest()->isPost()){
			$data = $this->getRequest()->getPost();
			$_dbmodel = new Issue_Model_DbTable_DbSchedule();
			$result=$_dbmodel->publicSchedule($data);
			print_r(Zend_Json::encode($result));
			exit();
		}
	}
