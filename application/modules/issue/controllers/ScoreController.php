...
class Issue_ScoreController extends Zend_Controller_Action
{
	...
	public	function addAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			...
			try {
				if($_data['exam_type']==4 || $_data['exam_type']==5){ // exam as term for kinder katen
					$rs =  $db->addStudentTermScore($_data);
				}else{
	...
	public	function editAction()
	{
		if ($this->getRequest()->isPost()) {
			...
			try {
				...
				if (isset($_data['save_close'])) {
					...
					if($_data['exam_type']==4 || $_data['exam_type']==5 ){ // for term kiderkaten
						 $dbs->updateStudentScoreTerm($_data);
					}else{
						 $dbs->updateStudentScore($_data);
					}
					$notify = array(
						"optNotification" => 2,
		...
		$this->view->student = $_model->getStundetScoreEdit($id, $sortType);
		...
		$this->view->subjectAndCriteria= $_model->getSubjectAndCriteriaByGroup($row['group_id'], null, $row['exam_type']);
	...
	function getStudenttermAction()
	{
		if ($this->getRequest()->isPost()) {
			$data = $this->getRequest()->getPost();
			$db = new Issue_Model_DbTable_DbScore();
			$data['sortStundent'] = empty($data['sortStundent']) ? 0 : $data['sortStundent'];
			$data = $db->getStudentTermByGroup($data['groupId'], $data);
			print_r(Zend_Json::encode($data));
			exit();
		}
	}
	...
	function getSubjectcriteriaAction()
	{
		if ($this->getRequest()->isPost()) {
			$data = $this->getRequest()->getPost();
			$db = new Issue_Model_DbTable_DbScore();
			$data = $db->getSubjectAndCriteriaByGroup($data['groupId'], null, $data['exam_type']);
			print_r(Zend_Json::encode($data));
			exit();
		}
	}
