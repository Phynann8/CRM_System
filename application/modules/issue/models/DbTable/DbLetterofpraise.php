...
class Issue_Model_DbTable_DbLetterofpraise extends Zend_Db_Table_Abstract
{
    ...
    function getAllIssueCertification($search){
    	...
    	$sql = "
			SELECT 
				...
				,(SELECT v.name_kh FROM `rms_view` AS v WHERE v.type=19 AND v.key_code =s.exam_type LIMIT 1) AS examType
				,s.title_score
				,c.issue_date
				,CASE
    	...
    	$sql.=" FROM `rms_issue_letterpraise` AS c 
				LEFT JOIN `rms_score` AS s ON s.`id` = c.`score_id`
				WHERE 1 ";
    	$where ='';
	...
	public function getStundetScoreById($id)
	{ // លទ្ធផលប្រឡង//
		...
		$sql = "SELECT 
				...
				vs.`group_id`,
				vs.`total_avg`,
				vs.`overallAssessmentSemester`,
				(
					SELECT COUNT(DISTINCT 
						CASE 
							WHEN vs.`exam_type` = 1 THEN vs2.`total_avg`
							ELSE vs2.`overallAssessmentSemester`
						END
					) + 1
					FROM `v_score_ft_scoredetail` AS vs2
					WHERE 
						(vs2.score_id= vs.`score_id` AND  vs.`exam_type` = 1 AND vs2.`total_avg` > vs.`total_avg`) OR 
            			(vs2.score_id= vs.`score_id` AND  vs.`exam_type` <> 1 AND vs2.`overallAssessmentSemester` > vs.`overallAssessmentSemester`)
				) AS rank
			FROM `v_score_ft_scoredetail` AS vs
			LEFT JOIN `rms_student` AS st ON st.`stu_id` = vs.`student_id` ";
		$where = " WHERE  vs.`score_id` = $id ";
		$order = " ORDER BY 
				CASE 
					WHEN vs.`exam_type` = 1 THEN vs.`total_avg`
					ELSE vs.`overallAssessmentSemester`
				END DESC ";
		return $db->fetchAll($sql . $where . $order);
	}
}