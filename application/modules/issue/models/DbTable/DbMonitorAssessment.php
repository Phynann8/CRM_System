...
class Issue_Model_DbTable_DbMonitorAssessment extends Zend_Db_Table_Abstract
{
	...
	function getAllGroups($search){
		...
		$sql = "
			SELECT 
				...
				,CASE
					...
					WHEN s.exam_type = 4 THEN CONCAT('Term ',s.`for_term` )
					WHEN s.exam_type = 5 THEN 'Final'
		...
		if($forPSISSensok==1){
			$where.=' AND (s.exam_type =1 OR s.exam_type =4 OR s.exam_type =5 )';
			//$where.=' AND g.academic_year > 7 ';
		}
	...
	function getAssessmentDetailList($assessmentId){
		...
		$sql="
			SELECT 
				...
				,assd.`assessmentId`
				,assd.`studentId`
				,(SELECT astd.teacherComment FROM rms_studentassessment AS ast JOIN rms_studentassessment_detail AS astd ON ast.id = astd.assessmentId WHERE  astd.assessmentId =  assd.assessmentId  AND astd.studentId= assd.`studentId` AND astd.teacherComment!='' ORDER BY astd.teacherComment DESC LIMIT 1) AS teacherComment
   ...
   function getAssessmentDetailListByStudent($data){
	...
	function unlockAss($_data){
		$db = $this->getAdapter();
		$db->beginTransaction();
		try{
			$assessmentID = empty($_data["assId"]) ? 0 : $_data["assId"];
			$this->_name='rms_studentassessment';
			$_arr = array(
					'isLock'	 	=>0,
					'lockBy'		=>$this->getUserId(),
