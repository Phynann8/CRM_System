...
class Issue_Model_DbTable_DbStudentAttendance extends Zend_Db_Table_Abstract
{
    ...
    function getAllAttendence($search=null){
    	...
    	$sql="
			SELECT 
				...
				,sa.`date_attendence`
				,CASE 
					WHEN sa.for_session =1 THEN '".$tr->translate('MORNING')."'
					WHEN sa.for_session =2 THEN '".$tr->translate('EVENING')."'
					ELSE '".$tr->translate('FULL_DAY')."'
				END AS shiftTime
		...
		try{
			...
			if(empty($id)){
				...
				if(!empty($scheduleTime)) {
					$_arr["scheduleId"] = empty($scheduleTime[0]["mainScheduleId"]) ? 0 : $scheduleTime[0]["mainScheduleId"];
				}
				$id=$this->insert($_arr);
			}
   ...
   public function updateStudentAttendence($_data){
	...
	function getScheduleTimeStudty($data){
		...
		$groupId = empty($data['group']) ? 0 : $data['group'];
		$sessionType = empty($data['session_type']) ? 0 : $data['session_type'];
		...
		$sql = "
			SELECT 
				...
				gSchD.id AS id,
				gSchD.main_schedule_id AS mainScheduleId,
		...
		if($sessionType!=3){
			if($sessionType==1){
				$sql.=" AND (gSchD.from_hour > 6 AND gSchD.from_hour < 13 ) ";//Morning
			}else if($sessionType==2){
				$sql.=" AND (gSchD.from_hour > 12.59 AND gSchD.from_hour < 18 ) ";//Afternoon
			}
		}
	...
	function getStudentInfoWithPermissionRequest($data){
		...
		$sql = "
			SELECT 
				...
				,CASE 
					WHEN attD.attendence_status IS NOT NULL 
						THEN COALESCE(attD.attendence_status,'')
					ELSE COALESCE(MIN(pre.`type`),2)
			...
			FROM `rms_student` AS s 
				...
				LEFT JOIN `rms_scan_pre_att` AS pre ON pre.`studentId` = s.stu_id AND DATE_FORMAT(pre.`createDate`,'%Y-%m-%d') = '".$attendenceDate."' 
	...
	function getThisGroupInfo($data=[]){
		$db = $this->getAdapter();
		$groupId = empty($data["groupId"]) ? 0 : $data["groupId"];
		//,(SELECT GROUP_CONCAT(gds.`stu_id`) FROM `rms_group_detail_student` AS gds WHERE gds.`itemType`=1 AND gds.group_id = g.id AND gds.`is_current`=1 AND gds.`stop_type`=0 LIMIT 1 ) AS studentList
		$sql = "
			SELECT
				g.`id` AS groupId
	...
	function getInitialStudentAtt($data){
		
		$db = $this->getAdapter();
		$groupId = empty($data["groupId"]) ? 0 : $data["groupId"];
		$initilaize = empty($data["initilaize"]) ? 0 : $data["initilaize"];
		$sessionType = empty($data["session_type"]) ? 0 : $data["session_type"];
		$date = new DateTime($data['attendenceDate']);
		$attendenceDate =  $date->format("Y-m-d");
		
		$groupInf = $this->getThisGroupInfo($data);
		$sessionId = empty($groupInf["sessionId"]) ? 0 : $groupInf["sessionId"];
		
		$groupShiftValue = empty($groupInf['groupShiftStr']) ? "-1" : $groupInf['groupShiftStr'];
		$groupShiftStrExp = explode(",", $groupShiftValue);
		if(count($groupShiftStrExp)>1){
			$groupShiftValue = 0; //assign as Full Day
		}
		$moreCodiction="";
		if($groupShiftValue>0){
			if($groupShiftValue==1){//Morning
				$moreCodiction= " AND pre.shift = $groupShiftValue ";
			}else if($groupShiftValue==2){//Afternoon
				$moreCodiction= " AND pre.shift = $groupShiftValue ";
			}
		}else{
			if($sessionType!=3){
				$moreCodiction= " AND pre.shift = $sessionType ";
			}
		}
		$tr = Application_Form_FrmLanguages::getCurrentlanguage();
		$sql = "
			SELECT 
				...
				,CASE
					...
					WHEN  s.sex = 2 THEN '".$tr->translate("FEMALE")."'
				END AS genderTitle
				,gd.`group_id` AS groupId
				,COALESCE(attD.description,'') as reason
				,COALESCE(attD.id,'0') AS permissionRecordId
				,CASE 
					WHEN attD.attendence_status IS NOT NULL THEN attD.attendence_status
					ELSE COALESCE(MAX(pre.`attendanceStatus`),2)
				END AS attendence_status
			FROM (`rms_group_detail_student` AS gd JOIN `rms_student` AS s ON s.stu_id = gd.stu_id )
				LEFT JOIN `rms_scan_pre_att` AS pre ON pre.`studentId` = gd.stu_id AND DATE_FORMAT(pre.`createDate`,'%Y-%m-%d') = '".$attendenceDate."'  $moreCodiction
				LEFT JOIN rms_student_attendence_detail AS attD ON attD.type=2 AND gd.stu_id = attD.stu_id AND DATE_FORMAT(attD.attendanceDate,'%Y-%m-%d')  = '".$attendenceDate."' 
		";
		$sql.= "
			WHERE 
			gd.`is_current` = 1
			AND gd.`itemType`=1
		";
		$sql.= " AND gd.group_id = $groupId ";
		
    	$sql.= " GROUP BY s.stu_code ASC ";
    	$fullResult = $db->fetchAll($sql);
		
		$strFilter =  (string) 1; 
		$resultOFStudent = array_filter($fullResult, function ($item) use ($strFilter,$initilaize) {
			
			if(!empty($initilaize)){
				if ($item['attendence_status'] != $strFilter) {
					return $item;
				}
			}else{
				if (($item['attendence_status'] != $strFilter) AND $item['attendence_status'] != 2) {
					return $item;
				}
			}
			return;
		...
		return array_values($resultOFStudent);
	}
}