...
class Issuesetting_Model_DbTable_DbItemsScoreEng extends Zend_Db_Table_Abstract
{
    ...
    function getAllItesmScoreEn($search=null){
    	...
    	$sql = " SELECT en.id,
						...
						en.title_en,
						case
							when en.criteriaType = 0 then 'Attendance'
							when en.criteriaType = 1 then 'General'
							when en.criteriaType = 2 then 'Exam'
						else ''
						end as criteriaType,
						en.short_cut,
						en.rank,
    ...
    public function addItemsScoreEng($_data){
    	...
    	try{
    		$_arr = array(
				...
				'criteriaType'=>$_data['criteriaType'],
    			'short_cut'=>$_data['short_cut'],
    			'rank'=>$_data['rank'],
	    		'note'=>$_data['note'],
    			'status'=>1,
