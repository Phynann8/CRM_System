...
Class Application_Form_FrmSearchGlobal extends Zend_Dojo_Form {
	...
	public function FrmSearch($_data=null){
		...
		$optRsChange = $_dbgb->getAllChangeGroup(); // 1=ប្តូរក្រុម
		...
		$_arr = array(
			0=>$this->tr->translate("SELECT_TERM_EXAM"),
		...
		$_for_term->setAttribs(array(
				'dojoType'=>'dijit.form.FilteringSelect',
				'required'=>'false',
				'placeholder'=>$this->tr->translate("SELECT_TERM_EXAM"),
				'missingMessage'=>'Invalid Module!',
				'class'=>'fullside height-text',));
		$_for_term->setValue($request->getParam("for_term"));
		...
		* */
		...
		$isPassClass->setAttribs(array('dojoType'=>$this->filter,
				'class'=>'fullside',
				'placeholder'=>$this->tr->translate("SERVIC"),
				'autoComplete'=>"false",
				'queryExpr'=>'*${0}*',
		...
		$isPassClass->setMultiOptions($opt);
		$isPassClass->setValue($request->getParam("isPassClass"));
		
		$isReturnStudy = new Zend_Dojo_Form_Element_FilteringSelect('isReturnStudy');
		$isReturnStudy->setAttribs(array('dojoType'=>$this->filter,
				'class'=>'fullside',
				'placeholder'=>$this->tr->translate("SERVIC"),
				'autoComplete'=>"false",
				'queryExpr'=>'*${0}*',
		...
		$opt = array(
				-1=>$this->tr->translate("ALL"),
				0=>$this->tr->translate("STOP"),
				1=>$this->tr->translate("returnStudy"),
				);
		$isReturnStudy->setMultiOptions($opt);
		$isReturnStudy->setValue($request->getParam("isReturnStudy"));
		
		$_dropReturnType=  new Zend_Dojo_Form_Element_FilteringSelect('dropReturnType');
		$_dropReturnType->setAttribs(array(
			'dojoType'=>$this->filter,
			'class'=>'fullside',
			'required'=>true,
		));
		$_type_opt = array(
				...
				1=>$this->tr->translate("RETURN_TO_CRM"),
				3=>$this->tr->translate("RETURN_TO_TEST"),
			);
		$_dropReturnType->setMultiOptions($_type_opt);
