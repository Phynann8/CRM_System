...
{        
    ...
    public function getCheckList($delete=0, $columns,$rows,$link=null,$additionalOption=array())
    {
    	...
    	if($delete== 1 || $delete== 2 ) {
    		$col_str .= '<th class="tdheader tdcheck"><input type="checkbox" onchange="checkall();" class="checkbox" id="check_all"  name="check_all" ></td>';
    	}
		  				...
		  				if($i==0) {
		  					...
		  					if($delete==2){
				    			$row_str .= '<td><input type="radio" onclick="setValue('.$temp.')" name="copy" id="copy" value="'.$temp.'" /></td>';
		  						...
		  						$row_str .= '<td><input type="checkbox" name="del[]" class="del" value="'.$temp.'" /></td>';
		  					}
							$stringAdditional='';
							...
							if($key=="titleRecord"){
								...
								if($key!=$columnSubTitle AND $key!=$processingBg ){
									$smallInfo = empty($row[$key."SmallInfo"]) ? "" : $row[$key."SmallInfo"];
									$row_str.='<td '.$clisc.'>';
										$row_str.=$read;
										if($smallInfo!=""){
											$row_str.='<small class="subtitle-row text-secondary">'.$smallInfo.'</small>';
										}
									$row_str.='</td>';
									//$row_str.='<td '.$clisc.' >'.$read.'</td>';
								}
													...
													<ul class="dropdown-menu" role="menu">';
														foreach($actionLink as $rowAction){
															...
															if ($linkType == "javascript") {
																...
																$jsFunc = $rowAction["link"];
																$href = "javascript:void(0);";
																$typefunction = "onclick=\"".$jsFunc."('".$recordId."'); return false;\"";

															} else if ($linkType=="inframe") {
