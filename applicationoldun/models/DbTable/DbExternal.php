...
class Application_Model_DbTable_DbExternal extends Zend_Db_Table_Abstract
{
	...
	public function getGroupDetailByIDExternal($id, $getTeacherId = null)
	{
		...
		$sql = "SELECT
				g.*,
				CONCAT(b.branch_nameen) AS branch_name,
				b.branch_namekh AS branchNameKh,
				b.branch_nameen AS branchNameEn,
				...
				di.$degree AS degree,
				gi.$grade AS grade,
				viewSession.name_en AS session,
				r.room_name AS room_name,
				t.teacher_name_en,
				t.teacher_name_kh,
				viewStatus.name_en AS status,
				grading.title AS gradingSystem,
			...
			FROM
				...
				LEFT JOIN rms_items AS di ON di.id = g.degree AND di.type = 1
				LEFT JOIN rms_itemsdetail AS gi ON gi.id = g.grade AND gi.items_type = 1
				LEFT JOIN rms_view AS viewSession ON viewSession.key_code = g.session AND viewSession.type = 4
				LEFT JOIN rms_room AS r ON r.room_id = g.room_id
				LEFT JOIN rms_teacher AS t ON t.id = g.teacher_id
				LEFT JOIN rms_view AS viewStatus ON viewStatus.key_code = g.status AND viewStatus.type = 1
				LEFT JOIN rms_scoreengsetting AS grading ON grading.id = g.gradingId AND grading.schoolOption = 1
	...
	function getGradingCriteriaItems($data)
	{
		...
		$sql = "SELECT 
				s.*,
				cri.criteriaType,
				es.title AS criterialTitle,
				es.title_en AS criterialTitleEng
			FROM 
				rms_scoreengsettingdetail AS s
			LEFT JOIN rms_exametypeeng AS cri ON cri.id = s.criteriaId
			LEFT JOIN rms_exametypeeng AS es ON es.id = s.criteriaId
			WHERE s.score_setting_id = $gradingId AND s.subjectId = $subjectId ";
		...
		$sql = "SELECT 
					s.*,
					exam.criteriaType,
					exam.title AS criterialTitle,
					exam.title_en AS criterialTitleEng
				FROM 
					rms_scoreengsettingdetail AS s
				LEFT JOIN rms_exametypeeng AS exam ON exam.id = s.criteriaId
				WHERE 
					s.score_setting_id = $gradingId 
					AND s.subjectId = 0 ";
	...
	function getClassSubjectScoreById($gradingId, $fullControlID)
	{
		...
		$sql = "SELECT 
				...
				,t.signature AS mainTeacherSigature
				,t.teacher_name_kh AS mainTeaccherNameKh
				,t.teacher_name_en AS mainTeaccherNameEng

				,te.signature AS teacherSigature
				,te.teacher_name_kh AS teaccherNameKh
				,te.teacher_name_en AS teaccherNameEng

				,CASE
					WHEN grd.examType = 2 THEN gsjd.semester_max_score
					ELSE gsjd.max_score
				END AS maxSubjectscore
				,sj.$subjectTitle AS subjectTitle
				,sj.subject_titlekh AS subjectTitleKh
				,sj.subject_titleen AS subjectTitleEng
				,sj.subject_titleen AS subjectTitleEng
				,CONCAT(acad.fromYear,'-',acad.toYear) AS academicYear
				,$criterialList,
		...
		$sql .= " FROM  rms_grading AS grd
					...
					LEFT JOIN rms_teacher AS t ON t.id = g.teacher_id
					LEFT JOIN rms_teacher AS te ON te.id = grd.teacherId
					LEFT JOIN rms_view AS v ON v.type=19 AND v.key_code=grd.examType
					LEFT JOIN `rms_month` m ON m.id=grd.forMonth
					LEFT JOIN `rms_academicyear` AS acad ON acad.id = g.academic_year
					LEFT JOIN rms_subject AS sj ON sj.id = grd.subjectId
					LEFT JOIN rms_group_subject_detail AS gsjd ON g.id = gsjd.group_id AND gsjd.subject_id=grd.subjectId
	...
	function getStudentGradingScore($data = array())
	{
		...
		$sql = " SELECT 
				...
				(SELECT vm.metion_grade FROM `v_mention_grade` AS vm 
					WHERE vm.academic_year= ".$data['academicYearId']."
					AND vm.degree = ".$data['degreeId']."
					AND (gt.totalAverage/gt.maxScore*100) >=vm.max_score 
					ORDER BY vm.max_score DESC
				LIMIT 1 ) AS mentionScore
			FROM 
				rms_grading_total AS gt
				LEFT JOIN rms_student AS s ON s.stu_id = gt.studentId
				LEFT JOIN v_grading_detail AS vgd ON vgd.gradingId = gt.gradingId AND vgd.studentId = gt.studentId
				LEFT JOIN (SELECT 
					studentId, 
					gradingId,FIND_IN_SET(totalAverage,(SELECT GROUP_CONCAT(totalAverage ORDER BY totalAverage DESC) FROM rms_grading_total WHERE gradingId = ".$data['gradingId']." )
					) AS rank
				FROM rms_grading_total WHERE gradingId = ".$data['gradingId']."
	...
	function getGradingByCriterial($data)
	{
		...
		if (!empty($data['forTerm'])) {
			$sql .= " AND gt.`forTerm`=" . $data['forTerm'];
		}
		if (!empty($data['examType'])) {
	...
	function getCriterialByGrading($data)
	{
		...
		$sql = "SELECT 
				...
				,(SELECT cri.rank FROM `rms_exametypeeng` cri WHERE cri.id= s.criteriaId LIMIT 1) rank
		...
		$sql .= " AND (SELECT crit.criteriaType FROM `rms_exametypeeng` AS crit WHERE crit.id = s.criteriaId LIMIT 1)
			...
			AND sttDi.subjectId =  " . $data['subjectId'] . " ORDER BY sttDi.isNotEnteryCri DESC LIMIT 1 ),'0') =1 
					...
					ELSE '0'
			END  ";
		$sql .= " ORDER BY rank ASC, criteriaType ASC, s.criteriaId ASC, s.`subjectId` DESC ";
		$db = $this->getAdapter();
		$row = $db->fetchAll($sql);
	...
	function getAllClassForIssueScore($search)
	{
		...
		$sql = "SELECT 
			...
			,CASE
				...
				WHEN sett.examType = 4 THEN sett.forTerm
		...
		FROM 
			...
			JOIN `rms_score_entry_setting` AS sett ON ( FIND_IN_SET(g.degree,sett.degreeId) AND sett.status = 1 AND sett.branchId = g.branch_id )
