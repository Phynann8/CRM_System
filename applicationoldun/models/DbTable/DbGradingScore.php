...
class Application_Model_DbTable_DbGradingScore extends Zend_Db_Table_Abstract
{
	...
	function getAllGradingScore($search=null){
		...
		if($search['for_term']>0){
			$where.= " AND grd.forTerm =".$search['for_term'];
		}
   ...
   public function UpdateGradingScore($_data){
		...
		try{
			...
			if($_data['criteriaType']==2 ){//submit final score
				$_arr = array(
						'branchId'			=>$_data['branch_id'],
						'gradingSettingId'	=>$gradingSettingId,
						'gradingTmpId'		=>$recordId,
			...
			if(!empty($_data['identity'])){
				...
				if(!empty($ids))foreach ($ids as $i){
					...
					if(!empty($criterial)) foreach($criterial AS $rowCri){
						...
						if($_data['criteriaType']==2 AND $totalScoreAverage>0 AND $old_studentid!=$_data['student_id'.$i]){//submit final score
						...
						if(!empty($rowCri['subjectId'])){
							if(!empty($rowCri['subCriterialTitleKh'])){
								...
								foreach ($subCriterial AS $keyV => $subCriTitle){
									...
									$score = $_data['score_'.$i.'_'.$criterialId.'_'.$indexSub];
									$gradingDetailId = $_data['gradingDetailId_'.$i.'_'.$criterialId.'_'.$indexSub];
									...
									if($criterialId==$_data['criteriaId']){
										...
										$this->_name='rms_grading_detail_tmp';
										$this->insert($arr);
									}elseif($rowCri['criteriaType']!=0){
										$_arr = array(
											'note'				=> $_data['note_'.$i],
											'totalGrading'		=> $score,
										);
										$this->_name='rms_grading_detail_tmp';
										$where='id='.$gradingDetailId;
										$this->update($_arr, $where);
									}
							...
							if(!empty($idCriterials)) foreach($idCriterials as $innerId){
								$score = $_data['score_'.$i.'_'.$criterialId.'_'.$innerId];//problem or not ?score_2_11score_1_1
								$gradingDetailId = $_data['gradingDetailId_'.$i.'_'.$criterialId.'_'.$innerId];//problem or not ?score_2_11score_1_1
				...
				if($totalScoreAverage>0 AND $_data['criteriaType']==2){//submit final score
					$arr=array(
							'gradingId'		=> $idGrading,
							'gradingTmpId'	=> $recordId,
							'studentId'		=> $_data['student_id'.$i],
   		...
   		$sql="
			...
			WHERE gt.id=gtd.gradingId ";
   			...
   			if(!empty($_data['examType'])){
   				...
   				}else if($_data['examType']==2){
   					if(!empty($_data['forSemester'])){
   						$sql.=" AND forSemester=".$_data['forSemester'];
   					}
   				}else if($_data['examType']==4){
   					if(!empty($_data['forTerm'])){
   						$sql.=" AND forTerm=".$_data['forTerm'];
   					}
   				}
   			}
   ...
   function getGradingScoreById($gradingId,$fullControlID){
   	...
   	$sql="SELECT
		   ...
		   grd.settingEntryId,
		   	...
		   	,CASE
		   	WHEN grd.examType = 2 THEN grd.forSemester
		   	WHEN grd.examType = 4 THEN grd.forTerm
		   	ELSE (SELECT $month FROM `rms_month` WHERE id=grd.forMonth  LIMIT 1)
		   	END AS forMonthTitle
		   	,g.group_code AS  groupCode
   ...
   function getStudentForGradingScore($data){//single entry by criteria
   	...
   	if(!empty($data['criteriaId']) AND $data['criteriaId']==EXAM_CRITERIA_ID AND $data['examType']!=4 ){
   ...
   function getStudentForGradingScoreEdit($data){//single entry by criteria

		$db=$this->getAdapter();
		$dbExternal = new Application_Model_DbTable_DbExternal();
		$students = $dbExternal->getStudentByGroupExternal($data);
		
		if(!empty($data['criteriaId']) AND $data['criteriaType']==2 AND $data['examType']!=4){
			unset($data['criteriaId']);
			$data['getExistingData']=1;
		}
		...
		// Application_Model_DbTable_DbUserLog::writeMessageError(json_encode($criterial));
		//	return $criterial;
				...
				if(!empty($rowCri['subjectId'])){
					...
					for ($x = 1; $x <= $count; $x++) {
						...
						if($rowCri['criteriaType']==0){//att
							...
							$resultScore = empty($resultEntry[$x-1]['avgTotalGrading'])?0:$resultEntry[$x-1]['avgTotalGrading'];//check more
							$gradingDetailId = empty($resultEntry[$x-1]['gradingDetailId'])?0:$resultEntry[$x-1]['gradingDetailId'];//check more
						}
						

						$string.='<div class="'.$classCol.'">';
						$string.='<input '.$readonly.' value="'.$resultScore.'" data-dojo-props="constraints:{min:0,max:'.$maxSubjectScore.'},'.$invalidesms.'" required="1" class="fullside" dojoType="dijit.form.NumberTextBox" type="text" onKeyup="calculateAverage('.$keyIndex.')" name="score_'.$keyIndex.'_'.$criterialId.'_'.$x.'"  id="score_'.$keyIndex.'_'.$criterialId.'_'.$x.'" />';
						$string.='<input value="'.$gradingDetailId.'"  dojoType="dijit.form.TextBox" class="fullside" name="gradingDetailId_'.$keyIndex.'_'.$criterialId.'_'.$x.'"  id="gradingDetailId_'.$keyIndex.'_'.$criterialId.'_'.$x.'"  type="hidden" >';
						$string.='</div>';
