...
class Accounting_Model_DbTable_DbFee extends Zend_Db_Table_Abstract
{
				...
				if(!empty($_data['identity_term'])){
					...
					foreach ($id_st_term as $t){
						$arr = array(
								...
								'periodId'		=>empty($_data['term_'.$t]) ? 0 : $_data['term_'.$t],
								'installmentOrdering'	=>empty($_data['installmentOrdering'.$t]) ? 0 : $_data['installmentOrdering'.$t],
    ...
    public function getFeeById($id){
    	$db = $this->getAdapter();
    	$sql = "SELECT t.* FROM rms_tuitionfee AS t WHERE t.id = ".$id;
    	$dbp = new Application_Model_DbTable_DbGlobal();
