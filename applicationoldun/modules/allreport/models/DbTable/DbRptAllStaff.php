...
class Allreport_Model_DbTable_DbRptAllStaff extends Zend_Db_Table_Abstract
{
	...
	function getTeacherScheduleGroupAndStudent($search){
    	...
    	$sql =" SELECT 
			...
			FROM rms_group_reschedule AS schDetail
				...
				JOIN `rms_group_detail_student` AS gsd ON gsd.group_id = sch.group_id AND gsd.stop_type=0 AND gsd.is_current=1
    	...
    	$order=" 
				GROUP BY sch.group_id,schDetail.techer_id,schDetail.subject_id,gsd.stu_id 
				ORDER BY schDetail.techer_id,COALESCE(ite.ordering,'0'),COALESCE(itd.ordering,'0'),g.group_code ASC,schDetail.subject_id ASC
			";
	...
	function getAllTeachingClass($search){
    	$db = $this->getAdapter();
    	$tr = Application_Form_FrmLanguages::getCurrentlanguage();
    	...
    	$sql = "SELECT 
			    	...
			    	END AS staff_type_title
			    	,gsc.id
					,gsc.`teacher` AS teacherId
					,gsc.`subject_id` AS subjectId
					,gsc.`group_id`
					,g.`academic_year`
					
					,(SELECT CONCAT(ac.fromYear,'-',ac.toYear) FROM `rms_academicyear` AS ac WHERE ac.id = g.academic_year LIMIT 1) AS academicYearTtitle
					,g.`group_code` AS groupCode
					,subj.`shortcut`
					,CASE 
						WHEN subj.`subject_lang` = 1 THEN subj.`subject_titlekh`
						ELSE subj.`subject_titleen`
					END AS subjectTitle
					,(SELECT COUNT(DISTINCT(gSb.subject_id)) FROM (`rms_group_subject_detail` AS gSb  JOIN `rms_group` AS g2 ON g2.`id` = gSb.`group_id`) WHERE g2.academic_year = g.`academic_year` AND gSb.teacher  = gsc.teacher AND COALESCE(gSb.amount_subject,0) > 0 ) AS amtSubject
					,(SELECT COUNT(DISTINCT(gSb.group_id)) FROM (`rms_group_subject_detail` AS gSb  JOIN `rms_group` AS g2 ON g2.`id` = gSb.`group_id`) WHERE g2.academic_year = g.`academic_year` AND gSb.teacher  = gsc.teacher AND COALESCE(gSb.amount_subject,0) > 0 ) AS amtGroup

				FROM  (`rms_group_subject_detail` AS gsc  JOIN `rms_group` AS g ON g.`id` = gsc.`group_id`)
						LEFT JOIN `rms_teacher` AS t ON t.id  = gsc.teacher
						LEFT JOIN rms_subject AS subj ON subj.id  = gsc.subject_id
						LEFT JOIN rms_items AS ite ON ite.id  = g.degree
						LEFT JOIN rms_itemsdetail AS itd ON itd.id  = g.grade
    			WHERE 
    				COALESCE(gsc.amount_subject,0) > 0
    		";
    	...
    	if(!empty($search['adv_search'])){
			...
			$s_where[] = " REPLACE(subj.`subject_titleen`,' ','') LIKE '%{$s_search}%'";
			$s_where[] = " REPLACE(subj.`subject_titlekh`,' ','') LIKE '%{$s_search}%'";
			$s_where[] = " REPLACE(subj.`shortcut`,' ','') LIKE '%{$s_search}%'";
			$where .=' AND ('.implode(' OR ',$s_where).')';
		}
		if(!empty($search['academic_year'])){
    	...
    	$order_by=" GROUP BY gsc.`teacher`,gsc.`group_id`,gsc.`subject_id` 
ORDER BY gsc.`teacher`,gsc.`subject_id`,COALESCE(ite.ordering,'0'),COALESCE(itd.ordering,'0'),g.`group_code` ";
