...
class Allreport_Model_DbTable_DbRptFee extends Zend_Db_Table_Abstract
{
	...
	function getAllStudentCurrentService($search = array()){
		...
		$sql="SELECT
				...
				,CONCAT(aca.fromYear,'-',aca.toYear)  AS academicYearTitle
				,vs.`stuCode`
				,vs.`stuNameKh`
				,vs.`stuNameEn`
				,vs.`tel`
				...
				,vs.`groupCode`
				,vs.`photo`
				,(SELECT v.shortcut FROM rms_view AS v where v.type=2 AND v.key_code=vs.sex LIMIT 1) AS sexTitle
		...
		$sql.="
			FROM (`rms_group_detail_student` AS gds  JOIN `v_stu_study_info` AS vs  ON vs.`studentId` = gds.`stu_id` AND vs.`itemType` =1)
				...
				LEFT JOIN rms_academicyear aca ON aca.id=vs.`academicYear` 
		";
		$sql.="WHERE 1 
			AND gds.`itemType` !=1 
			AND gds.`stop_type` = 0
