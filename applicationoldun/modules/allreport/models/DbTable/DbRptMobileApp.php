...
class Allreport_Model_DbTable_DbRptMobileApp extends Zend_Db_Table_Abstract
{
	...
	{ //for card list
		...
		$where .=" ORDER BY s.disableDate DESC ";
	...
	public function enableStudentAccount($_data)
	{
		$db = $this->getAdapter();
		$db->beginTransaction();
		try {
			...
			if (!empty($_data['identity'])) {
				...
				foreach ($ids as $rs) {
					...
					$isDisbleAccount = empty($_data['isDisbleAccount' . $rs]) ? 1 : 0;
					$_arr = array(
						'isDisbleAccount'	 	=> $isDisbleAccount,
					);
					$where = " stu_id = " . $rs;
