...
class Allreport_Model_DbTable_DbRptPayment extends Zend_Db_Table_Abstract
{
    ...
    public function getStudentPaymentDetail($search,$order_no){//rpt-paymentdetailbytype & rpt-studentpaymentdetail
    	$db = $this->getAdapter();
    	$_db  = new Application_Model_DbTable_DbGlobal();
    	$lang = $_db->currentlang();
    	...
    	$sql=" SELECT 
					...
					tf.generation as academic_type, 
					v1.$label AS payment_term, 	
					v2.$label as void_status,	  
					ds.dis_name as discount_type,
					d.items_id,
					COALESCE(d.shortcut,d.$title) AS service_name,
					u.first_name as user,
					itd.$grade as major_name,
					g.group_code as group_name,
					it.$degree as category
					FROM 
					    rms_student_payment AS sp join rms_student_paymentdetail AS spd ON sp.id=spd.payment_id 
						JOIN rms_student AS s ON s.stu_id = sp.student_id
					LEFT JOIN rms_itemsdetail AS itd ON itd.id=sp.grade
					LEFT JOIN rms_itemsdetail AS d ON  d.id = spd.itemdetail_id
					LEFT JOIN rms_items AS it ON it.id = d.items_id
					LEFT JOIN rms_group AS g ON g.id=sp.group_id
    	...
    	if(!empty($search['item'])){
			...
			$condiction = $_dbGb->getChildItems($arrCon);
			if (!empty($condiction)){
				$where.=" AND  d.items_id IN ($condiction)";
			}else{
				$where.=" AND  d.items_id=".$categoryId;
			}
    	}
    	if($search['pay_term']!=''){
    	...
    	if(!empty($search['study_year'])){
    		$where .= " and sp.academicYearId = ".$search['study_year'];
    	}
		if(!empty($search['academic_year'])){
    		$where .= " and sp.academicYearId = ".$search['academic_year'];
    	}
    	if($search['degree']>0){
		...
		if(!empty($search['receiptStatus'])){
			if($search['receiptStatus']==1){
				$where.= " AND sp.is_void = 0 ";
			}else if($search['receiptStatus']==2){
				$where.= " AND sp.is_void = 1 ";
			}
		}
		if(!empty($search['user_id'])){
			$where.= " AND sp.user_id = ".$search['user_id'];
		}
    	if($order_no==1){
    		...
    		$order=" ORDER BY sp.branch_id ASC,d.items_type ASC,d.items_id ASC,d.ordering ASC ,sp.id DESC  ";
    	}
    ...
    function getDiscountSetting($search = '',$type=null){
		...
		$sql = "SELECT ds.id, 
					...
					discountCode,
					discountTitle,
					v.$colunmname discountForOption,
					v2.$colunmname AS discountForText,
					dis.dis_name discName,
					(
						SELECT GROUP_CONCAT($strDegree)
						FROM rms_items
						WHERE FIND_IN_SET(id, ds.degree)
					) AS degreeList,
					CONCAT(ds.discountValue, 
						CASE ds.DisValueType 
							WHEN 1 THEN '%' 
							WHEN 2 THEN '$' 
						END
					) AS DisValueType,
					COALESCE(stuCounts.total, 0) AS StuAmount,
					COALESCE(stuCounts.used, 0) AS StuAmountUsed,
					COALESCE(stuCounts.stopped, 0) AS AmountStopUsed,
					CASE 
					WHEN ds.discountPeriod = 2 THEN 
						CONCAT(
							COALESCE(DATE_FORMAT(ds.startDate,'%d/%m/%y'), ''),
							'-',
							COALESCE(DATE_FORMAT(ds.endDate,'%d/%m/%y'), '')
						)
					ELSE v3.name_en
		...
		$sql .= " FROM rms_dis_setting AS ds
						LEFT JOIN rms_branch b ON b.br_id=ds.branchId
						LEFT JOIN rms_academicyear ac ON ac.id=ds.academicYear
						LEFT JOIN `rms_view` v ON v.type=38 AND v.key_code=ds.discountForType
						LEFT JOIN rms_discount dis ON dis.disco_id=ds.discountId
						LEFT JOIN rms_users u ON u.id = ds.userId
						LEFT JOIN rms_view v2 ON v2.type = 37 AND v2.key_code = ds.discountFor
						LEFT JOIN rms_view v3 ON v3.type = 39 AND v3.key_code = ds.discountPeriod
						LEFT JOIN (SELECT 
									discountGroupId,
									COUNT(studentId) AS total,
									SUM(CASE WHEN isCurrent = 1 THEN 1 ELSE 0 END) AS used,
									SUM(CASE WHEN isCurrent = 0 THEN 1 ELSE 0 END) AS stopped
								FROM rms_discount_student
								GROUP BY discountGroupId
							) AS stuCounts ON stuCounts.discountGroupId = ds.id
					 ";
	...
	function getScholarshipsDiscountValue($search = '',$type=null){
		$db = $this->getAdapter();
		$dbp = new Application_Model_DbTable_DbGlobal();
		...
		$sql ="SELECT 
				b.$branch AS branch,
				d.dis_name AS DiscountType,
				v.DisCode AS DisCode,
				v.discountTitle,
				CONCAT(v.discountValue, 
						CASE 
						WHEN v.DisValueType = 1 THEN '%'
						WHEN v.DisValueType = 2 THEN '$'
						END
				) AS DiscountValue,

				COUNT(DISTINCT v.studentId) AS NumberOfStudent,
				ROUND(SUM(v.discount_amount), 2) AS TotalValue

				FROM v_dis_student_fee v
				LEFT JOIN rms_discount d ON d.disco_id = v.discountId
				LEFT JOIN rms_branch b ON b.br_id = v.branchId ";
		$groupby =" GROUP BY v.branchId,v.id, v.discountId, v.discountValue ";
		$order =" ORDER BY	b.$branch ASC, d.dis_name ASC, v.discountValue ASC ";
		$where =" WHERE 1";
		...
		if (!empty($search['title'])) {
			$s_where = array();
			$s_search = addslashes(trim($search['title']));
			$s_where[] = " v.DisCode LIKE '%{$s_search}%'";
			$s_where[] = " v.discountValue LIKE '%{$s_search}%'";
			$where .= ' AND ( ' . implode(' OR ', $s_where) . ')';
		}
		if (!empty($search['branch_id'])) {
			$where .= ' AND v.branchId=' . $search['branch_id'];
		}
		if (($search['academic_year'])>0) {
			$where .= ' AND v.academicYear=' . $search['academic_year'];
		}
		if (!empty($search['discountId'])) {
			$where .= ' AND v.discountId=' . $search['discountId'];
		}
		$where .= $dbp->getAccessPermission('v.branchId');
    ...
    public function getStudentPayment($search){
		...
		$where = " AND ".$from_date." AND ".$to_date;
			$sql="SELECT
					...
					FROM
						...
						rms_student as s
					WHERE
						sp.student_id=s.stu_id AND sp.status=1 ";//AND is_void=0 
			$order=" ORDER BY id DESC";
    	...
    	if(!empty($search['study_year'])){
			...
			$where .= " and COALESCE((SELECT academic_year FROM rms_tuitionfee WHERE rms_tuitionfee.id = sp.academic_year LIMIT 1),0) = ".$search['study_year'];
    	}
		if(!empty($search['academic_year'])){
			$where.= " AND COALESCE((SELECT academic_year FROM rms_tuitionfee WHERE rms_tuitionfee.id = sp.academic_year LIMIT 1),0) = ".$search['academic_year'];
		}
		if(!empty($search['user_id'])){
			$where.= " AND sp.user_id = ".$search['user_id'];
		}
...
//     	}
		...
		$_dbGb  = new Application_Model_DbTable_DbGlobal();
		$where.=$_dbGb->getAccessPermission('sp.branch_id');
    	return $db->fetchAll($sql.$where.$order);
    }
...
//     	echo $sql.$where.$order;
    ...
    public function getStudentPaymentbyDegree($search,$order_no){
		...
		$branch_id = $_dbGb->getAccessPermission("sp.branch_id");
		$sql=" SELECT
			    	spd.id,
...
//     	}
    ...
    function getTotalOtherIncomeByDate($search){
    	try{
    		$_db = new Application_Model_DbTable_DbGlobal();
    		$branch_id = $_db->getAccessPermission('ic.branch_id');
    		$db=$this->getAdapter();
...
//     		}
    ...
    public function getDiscountsetById($id)
	{
		...
		$currentLang = $dbp->currentlang();
		$branch = $dbp->getBranchDisplay(); // e.g., name_en or name_kh depending on implementation
		$colunmname = 'name_en';
		$strDegree = 'title_en';
		...
		$sql = "SELECT  
					...
					ds.discountCode,
					ds.discountTitle,
					vf.$colunmname AS discountForText,
					vo.$colunmname AS discountForOption,
					GROUP_CONCAT(di.$strDegree) AS degreeList,
					d.dis_name AS discName,
					CONCAT(ds.discountValue, 
						(CASE WHEN ds.DisValueType=1 THEN '%' WHEN ds.DisValueType=2 THEN '$' END)
					) AS DisValueType,
					COUNT(dc.studentId) AS StuAmount,
					COUNT(IF(dc.isCurrent = 1, 1, NULL)) AS StuAmountUsed,
					COUNT(IF(dc.isCurrent = 0, 1, NULL)) AS AmountStopUsed,
					CONCAT(
						COALESCE(vp.$colunmname, ''), '', 
						COALESCE(DATE_FORMAT(ds.startDate,'%d-%m-%Y'), ''), ' ',
						COALESCE(DATE_FORMAT(ds.endDate,'%d-%m-%Y'), '')
					) AS discountPeriod,
			...
			LEFT JOIN `rms_branch` AS b ON b.br_id = ds.branchId
			LEFT JOIN `rms_academicyear` AS ac ON ac.id = ds.academicYear
			LEFT JOIN `rms_view` AS vf ON vf.type = 37 AND vf.key_code = ds.discountFor
			LEFT JOIN `rms_view` AS vo ON vo.type = 38 AND vo.key_code = ds.discountFor
			LEFT JOIN `rms_view` AS vp ON vp.type = 39 AND vp.key_code = ds.discountPeriod
			LEFT JOIN `rms_discount` AS d ON d.disco_id = ds.discountId
			LEFT JOIN `rms_users` AS u ON u.id = ds.userId
			LEFT JOIN `rms_items` AS di ON FIND_IN_SET(di.id, ds.degree)
			LEFT JOIN `rms_discount_student` AS dc ON dc.discountGroupId = ds.id
			WHERE ds.id = " . $db->quote($id);
		$sql .= $dbp->getAccessPermission('ds.branchId');
		$sql .= " GROUP BY ds.id LIMIT 1";
		$row = $db->fetchRow($sql);
		return $row;
	}
