...
class Allreport_Model_DbTable_DbRptStudentScore extends Zend_Db_Table_Abstract
{
	...
	{ // លទ្ធផលប្រចាំខែលម្អិតតាមមុខវិជ្ជា//
		...
		$semesterTotalAverage = empty($param['semesterTotalAverage']) ? 50 : $param['semesterTotalAverage'];
		$max_average = empty($param['max_average']) ? 100 : $param['max_average'];
		...
		$sql = "SELECT
		  ...
		  ";
		  if($exam_type==1){
				$sql .= " ,(SELECT CONCAT(vm.metion_grade,',',vm.metion_in_khmer,',',vm.mention_in_english)
					FROM v_mention_grade AS vm 
					WHERE 
						 vm.academic_year= $for_academic_year
						AND vm.degree = $degree
						AND (sml.total_avg/$max_average *100) >=vm.max_score 
						ORDER BY vm.max_score DESC
				...
				$sql .= "	
				,(SELECT vt.total_avg FROM  v_avg_term_score AS vt WHERE 
					vt.`for_term` = 1 
					AND vt.student_id = sml.`student_id`
					AND vt.group_id = $group LIMIT 1) AS avgTerm1
				,(SELECT vt.total_avg FROM  v_avg_term_score AS vt WHERE 
					vt.`for_term` = 2
					AND vt.student_id = sml.`student_id`
					AND vt.group_id = $group LIMIT 1) AS avgTerm2
				,(SELECT vt.total_avg FROM  v_avg_term_score AS vt WHERE 
					vt.`for_term` = 3
					AND vt.student_id = sml.`student_id`
					AND vt.group_id = $group LIMIT 1) AS avgTerm3
				
				,(SELECT CONCAT(vm.metion_grade,',',vm.metion_in_khmer,',',vm.mention_in_english)
					FROM v_mention_grade AS vm 
					WHERE 
						vm.academic_year= $for_academic_year
						AND vm.degree = $degree
				...
				$sql .= " ,(SELECT CONCAT(vm.metion_grade,',',vm.metion_in_khmer,',',vm.mention_in_english)
					...
					WHERE 
						vm.academic_year= $for_academic_year
						AND vm.degree = $degree
						AND (sml.overallAssessmentSemester/$semesterTotalAverage *100) >=vm.max_score 
						ORDER BY vm.max_score DESC
					LIMIT 1  ) AS mentionScore
					";
			}
		...
		}else if($exam_type == 4 || $exam_type == 5){
			$order .= " sml.total_avg  DESC ";
		} else {
	...
	{ // សម្រាប់លទ្ធផលប្រចាំខែ មិនលម្អិត/outstanding photo and no photo
		...
		$lang = $_db->currentlang();
		if ($lang == 1) { // khmer
			$grade = "itemsdetail.title";
			$degree = "items.title";
			$branch = "b.branch_namekh";
		} else { // English
			$grade = "itemsdetail.title_en";
			$degree = "items.title_en";
			$branch = "b.branch_nameen";
		}
		
		$tr = Application_Form_FrmLanguages::getCurrentlanguage();
		$Male = $tr->translate("MALE_SHORT");
		$FeMale = $tr->translate("FEMALE_SHORT");
		...
		$sql = "SELECT
				...
				t.signature AS teacher_signature,
				CONCAT(COALESCE(st.last_name,''),' ',COALESCE(st.stu_enname,'')) AS stu_enname,
				st.`stu_id`,
				st.`sex`,
				CASE 
					WHEN st.sex=1 THEN '.$Male.' 
					ELSE '$FeMale' 
				END AS gender,
				st.photo,
				st.`stu_code`,
		...
		if ($search['exam_type'] == 2) {
			...
			,(SELECT CONCAT(sd.metion_grade,',',sd.metion_in_khmer,',',sd.mention_in_english)
				...
				WHERE s.id = sd.metion_score_id
					AND s.academic_year= smd.for_academic_year
					AND s.degree = `g`.`degree`
					AND (smd.total_avg/g.max_average*100) >=sd.max_score 
					ORDER BY sd.max_score DESC
				LIMIT 1  ) AS mentionScore
			";
		}else if($search['exam_type'] == 4 OR $search['exam_type'] == 5){
			$sql .= "
			,(SELECT CONCAT(sd.metion_grade,',',sd.metion_in_khmer,',',sd.mention_in_english)
				FROM `rms_metionscore_setting_detail` AS sd,
					`rms_metionscore_setting` AS s
		...
		$sql .= " FROM
		   ...
		   LEFT JOIN rms_items AS items ON items.id = g.degree
		   LEFT JOIN rms_itemsdetail AS itemsdetail ON itemsdetail.id = g.grade
		   LEFT JOIN rms_teacher AS t ON t.id = g.teacher_id
		   LEFT JOIN rms_month as m on m.id=smd.for_month
		...
		if (!empty($search['academic_year'])) {
			$where .= " AND smd.for_academic_year =" . $search['academic_year'];
		}
		if (!empty($search['group'])) {
		...
		if (!empty($search['exam_type'])) {
			...
			}else if($search['exam_type'] == 4){
				if (!empty($search['for_term'])) {
					$where .= " AND smd.for_term =" . $search['for_term'];
				}
	...
	public function getSubjectScoreGroup($group_id, $teacher_id = null, $exam_type = 1)
	{
		...
		if($exam_type == 4 || $exam_type == 5){
			return $db->getSubjectAndCriteriaByGroup($group_id,$teacher_id,$teacher_id);
		}else{
	...
	function getScoreExamByID($score_id)
	{
		...
		$sql = "SELECT
			...
			g.max_average,
			CONCAT(ac.fromYear,'-',ac.toYear) AS academic_year,
			m.month_kh AS forMonth,
			t.teacher_name_kh as teacher,
			t.signature as teacher_sigature
		 FROM `rms_score` AS s
			LEFT JOIN rms_group AS g ON g.id=s.group_id
			LEFT JOIN rms_academicyear AS ac ON ac.id=s.for_academic_year
			LEFT JOIN rms_month AS m ON m.id=s.for_month
			LEFT JOIN rms_teacher as t on t.id=g.teacher_id
			WHERE s.id = $score_id 
			LIMIT 1";
		return $db->fetchRow($sql);
	...
	function getAllStudentPassed($search)
	{
		...
		$sql = "SELECT
				...
				FROM
					...
					JOIN (`rms_group_detail_student` AS gds JOIN rms_student AS st ON gds.stu_id=st.stu_id AND gds.itemType=1  ) ON  gds.`group_id` = gscg.`to_group` AND gds.`old_group` = gscg.`from_group`
					LEFT JOIN rms_group AS g ON gscg.to_group=g.id
				   	
				WHERE gscg.change_type=2
	   		";
	...
	{ //using
		...
		if (!empty($search['for_term'])) {
			$where .= ' AND s.for_term=' . $search['for_term'];
		}
	...
	{ //using
		...
		if ($search['exam_type'] == 1) {
			$totalScore = "sm.total_avg";
			$totalMaxScore = "(SELECT  g.max_average FROM `rms_group` AS g WHERE g.id=s.group_id LIMIT 1 )";
		}else if($search['exam_type'] == 4 || $search['exam_type'] == 5){
	...
	{ //using
		...
		if (!empty($search['for_term'])) {
			$where .= ' AND sj.for_term=' . $search['for_term'];
		}
		if (!empty($search['sort_degree'])) {
			$where .= ' AND g.degree in(' . $search['sort_degree'] . ')';
		}
		...
		if(!empty($search['group_by'])) {
			if($search['group_by']==1){
				$orderBy = " ORDER BY  sj.teacher_id,sj.subject_id,g.degree,g.grade,g.group_code  ASC ";
			}else{
				$orderBy = " ORDER BY  sj.subject_id,g.degree,g.grade,g.group_code  ASC ";
			}
		}
	...
	function getMentoinGradeSetting($search){   /// Funtion Get Score Setting
		...
		WHERE m.status = 1 AND m.`degree`= " . $search['degree'] . " AND m.`academic_year`= " . $search['academic_year'] . "  ORDER BY md.`max_score` DESC";
	...
	function getAllAchievementList($data){
    	$db = $this->getAdapter();
		...
		$where.=" AND FIND_IN_SET(ac.id, '" . $achievementId . "') ";	
	...
	function getAchievementTemplateList($data){
		
		$rs = $this->getAllAchievementList($data);
		...
		if(!empty($rs)) foreach($rs AS $key => $row){
			...
			$row["indexKey"] = $key;
			$template.= $frm->getAchievementTemplate($row);
		}
	...
	function getGroupInfo($data)
	{
		...
		if($data['examType']==1){
			$amountSubject = 'vgs.amount_subject';
		}else{
			$amountSubject = 'vgs.amount_subject_sem';
		}
		...
		$subjectList ='
			CONCAT(
				...
				CONCAT(
					"{",
					"\"subjectId\":", vgs.subject_id, ",",
					"\"subjectTitleEn\":\"", vgs.subjectTitleEn, "\",",
					"\"subjectTitleKh\":\"", vgs.subjectTitleKh, "\",",
					"\"subjectShortcut\":\"", vgs.subjectShortcut, "\",",
					"\"amountSubject\":\"",'.$amountSubject.', "\",",
					"\"subjectLang\":\"", vgs.subjectLang, "\"",
					"}"
				)
				ORDER BY vgs.subjectLang, vgs.subject_id ASC
				),
				"]"
			) AS subjectList
		';
		...
		$sql .= " FROM rms_group AS g
				LEFT JOIN `vapi_group_subject_info` AS vgs ON g.id = vgs.group_id ";
		$where = ' WHERE 1 ';
		if (!empty($data['groupId'])) {
			$where .= ' AND g.id=' . $data['groupId'];
		}
		if (!empty($data['gradingId'])) {
			$where .= ' AND g.gradingId=' . $data['gradingId'];
		}
		if (!empty($data['examType'])) {
			if($data['examType']==1){
				$where .= ' AND vgs.amount_subject > 0 ';
			}else{
				$where .= ' AND vgs.amount_subject_sem > 0 ';
			}
		}
		$groupby = ' GROUP BY g.id ';
		$limit = ' LIMIT 1 ';
		//echo $sql . $where.$groupby.$limit; exit();
		return $db->fetchRow($sql . $where.$groupby.$limit);
	}
	public function getStundetGradingTotal($param=null)
	{ // លទ្ធផលប្រចាំខែលម្អិតតាមមុខវិជ្ជា//
		$db = $this->getAdapter();

		$group = empty($param['groupId']) ? 0 : $param['groupId'];
		
		$sql = "SELECT
		    gsd.`stu_id`,
			s.`stu_code`,
			s.stu_khname,
			CONCAT(COALESCE(s.last_name,''),' ',COALESCE(s.stu_enname,'')) AS stu_enname,
			s.`sex`,
			vgt.`jsonGradingTotal`  ";
		$sql .= " FROM `rms_group_detail_student` AS gsd
					LEFT JOIN `rms_student` AS s ON gsd.`stu_id` = s.`stu_id`
					LEFT JOIN `v_grading_gradingtotal` AS vgt ON  vgt.`studentId` = gsd.`stu_id` 
   			WHERE 1 
			AND gsd.`itemType` = 1 
			AND gsd.`is_pass` = 0
			AND gsd.`stop_type` = 0 ";
		$sql .= " AND gsd.group_id  = $group ";
	
		$where = '';
		if (!empty($param['examType'])) {
			$where .= ' AND vgt.examType=' . $param['examType'];
			if ($param['examType'] == 1) {
				if (!empty($param['for_month'])) {
					$where .= " AND vgt.forMonth =" . $param['for_month'];
				}
			} else if ($param['examType'] == 2) {
				if (!empty($param['for_semester'])) {
					$where .= " AND vgt.forSemester =" . $param['for_semester'];
				}
			}
		}
