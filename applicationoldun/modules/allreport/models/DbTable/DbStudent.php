...
class Allreport_Model_DbTable_DbStudent extends Zend_Db_Table_Abstract
{
	...
	function getAllCRM($search=array()){
		...
		$sql="SELECT c.*,
			...
			(SELECT COALESCE(i.shortcut,i.$colunmname) FROM `rms_items` AS i WHERE i.id = ds.degree AND i.type=1 AND ds.is_maingrade=1 LIMIT 1) AS degree,
			(SELECT COALESCE(idd.shortcut,idd.$colunmname) FROM `rms_itemsdetail` AS idd WHERE idd.id = ds.grade AND idd.items_type=1 AND ds.is_maingrade=1 LIMIT 1) AS grade,
			
			(SELECT COALESCE(v.shortcut,v.$label) FROM `rms_view` AS v WHERE v.type=2 AND v.key_code = c.sex LIMIT 1) AS sexTitle,			
			...
			c.prev_concern,
			(SELECT GROUP_CONCAT(COALESCE(v.$label,'')) FROM `rms_view` AS v WHERE v.type=22 AND FIND_IN_SET(v.key_code, c.prev_concern)  LIMIT 1 ) AS prevConcernTitle,
	...
	function getAllStudentTest($search=null){
		try{
			...
			$sql=" SELECT 
					st.*,
					(SELECT b.$branch FROM `rms_branch` AS b WHERE b.br_id=st.branch_id LIMIT 1) AS branchName,
