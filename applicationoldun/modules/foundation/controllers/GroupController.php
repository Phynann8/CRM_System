...
class Foundation_GroupController extends Zend_Controller_Action
{
	...
	public function indexAction()
	{
		try {
			...
			$collumns = array("BRANCH", "GROUP_CODE", "YEARS", "SESSION", "ROOM_NAME", "TEACHER_ROOM", "TIME","CLASS_STATUS", "BY", "STATUS");
			$link = array(
			...
			$this->view->list = $list->getCheckList(10, $collumns, $rs_rows, array('branch_name' => $link, 'titleRecord' => $link, 'academicYear' => $link, 'degree' => $link, 'grade' => $link),$additionalOption);
		} catch (Exception $e) {
	...
	function getGroupInfoAction()
	{
		if ($this->getRequest()->isPost()) {
			$data = $this->getRequest()->getPost();
			$db = new Foundation_Model_DbTable_DbGroup();
			$row = $db->getGroupInfo($data['groupId']);
