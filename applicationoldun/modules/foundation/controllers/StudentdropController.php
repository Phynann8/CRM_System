...
class Foundation_StudentdropController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$collumns = array("BRANCH","STUDENT","SEX","GROUP","SESSION","TYPE","STOP_DATE","REASON","returnStudy","USER","STATUS");
			$link=array(
					'module'=>'foundation','controller'=>'studentdrop','action'=>'edit',
			);
			$this->view->list=$list->getCheckList(10, $collumns, $rs_rows,array('branch_name'=>$link,'titleRecord'=>$link,'academic'=>$link,'stu_id'=>$link,'student_name'=>$link,'sex'=>$link));
	...
	public function editAction(){
		try{	
			...
			if ($row['status']==0){
			 	Application_Form_FrmMessage::Sucessfull("UNABLE_TO_EDIT_DEACTIVE_RECORD","/foundation/studentdrop/index");
				exit();
			 }
			 if ($row['isReturn']==1){
