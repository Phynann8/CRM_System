<?php class Global_Model_DbTable_DbItems extends Zend_Db_Table_Abstract{
	...
	function getAllItems($search = '',$type=null){
		 ...
		 $labelExtraCourse= Application_Form_FrmLanguages::getCurrentlanguage()->translate("EXTRA_COURSE");
		$db = $this->getAdapter();
		$sql = " SELECT 
					d.id,
					d.title,
					...
					d.shortcut,
					CASE 
						WHEN type=1 AND d.isExtraCourse=1 THEN '$labelExtraCourse'
					END AS extraCourse,
					d.ordering,
	...
	public function AddDegree($_data){
		...
		try{
			...
			$_arr=array(
					...
					'isExtraCourse'	=> empty($_data['course_type']) ? 0 : $_data['course_type'],
