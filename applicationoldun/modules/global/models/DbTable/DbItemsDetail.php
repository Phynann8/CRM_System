<?php class Global_Model_DbTable_DbItemsDetail extends Zend_Db_Table_Abstract{
	...
	public function getItemsDetailById($degreeId,$type=null,$is_set=null,$extraCondiction=[]){
		$db = $this->getAdapter();
		$sql=" SELECT ide.*,
		...
		if(!empty($extraCondiction["forCheckIssuePayment"])){
			//Skip when addRegister In registra
		}else{
	...
	public function AddItemsDetail($_data){
		...
		try{
			...
			$isExtraCourse = !empty($itemsinfo['isExtraCourse'])?$itemsinfo['isExtraCourse']:0;
	...
	public function AddProduct($_data){
		...
		try{
	    	...
	    	if (!empty($_data['selector'])) foreach ( $_data['selector'] as $rs){
	    		if (empty($gradeList)){
	    			$gradeList = $rs;
	    		}else{ $gradeList = $gradeList.",".$rs;
	    		}
	    	}
			$isProductSet = empty($_data['isProductSet']) ? 0 : 1;
	...
	public function AddInitizeqty($_data){
		...
		try{
			...
			foreach ($ids as $i){
				...
				$param = array(
					'branch_id'     =>$_data['branch_id'],
					'product_id'	=>$_data['product_name_'.$i],
					'movement_type' =>1, // 1= Initizeqty
					'quantity'		=>$_data['qty_'.$i],
					'reference'		=>$_data['branch_id'],
	...
	function getAllProduct($search = '',$items_type=null){
		...
		$urlEdit = $base_url."/stock/product/edit/id/";
		
		$strBr = " (SELECT GROUP_CONCAT(b.$branch) FROM `rms_branch` AS b WHERE  FIND_IN_SET(b.br_id,GROUP_CONCAT(pl.`branch_id`)) LIMIT 1 ) ";
		$sql = " 
				SELECT 
				ide.id
				,ide.code
				,$grade AS titleRecord
				,(SELECT $degree FROM `rms_items` AS it WHERE it.id = ide.items_id LIMIT 1) AS degree
				,CASE    
					WHEN  ide.is_productseat = 1 THEN '".$tr->translate("PRODUCTSET")."'
		...
		$sql.=",CONCAT('Available at : <strong>',COALESCE($strBr,''),'</strong>') AS subTitleRecord";
		$sql.=",COALESCE(GROUP_CONCAT(pl.branch_id),'') AS branchIdList";
		$sql.=" FROM 
					`rms_itemsdetail` AS ide 
					LEFT JOIN rms_product_location AS pl ON pl.pro_id = ide.id 
				WHERE 1  ";//AND ide.is_productseat = 0
		$where = "";
		...
		if($search['status']>-1 AND $search['status']!=''){
			$where.= " AND ide.status = ".$db->quote($search['status']);
		}
		if($search['is_onepayment']>-1 AND $search['is_onepayment']!=''){
			$where.= " AND ide.is_onepayment = ".$db->quote($search['is_onepayment']);
		}
		if(!empty($search['product_type'])){
			$where.= " AND ide.product_type = ".$db->quote($search['product_type']);
		}
		if($search['isCountStock']>-1){
			$where.= " AND ide.isCountStock = ".$db->quote($search['isCountStock']);
		}
		if($search['isProductSet']>-1){
			$where.= " AND ide.is_productseat = ".$db->quote($search['isProductSet']);
		}
		if(empty($search['branch_id'])){
			if(!empty($search['inLocationStatus'])){
				if($search['inLocationStatus']==1){
					$where.= " AND pl.branch_id IS NOT NULL ";
				}elseif($search['inLocationStatus']==2){
					$where.= " AND pl.branch_id IS NULL ";
				}
			}
		}
		$orderby = " GROUP BY ide.id ";
		if(!empty($search['branch_id'])){
			$strHaving= " HAVING FIND_IN_SET('".$search['branch_id']."', branchIdList) ";
			if(!empty($search['inLocationStatus'])){
				if($search['inLocationStatus']==1){
					$strHaving = " HAVING FIND_IN_SET('".$search['branch_id']."', branchIdList) ";
				}elseif($search['inLocationStatus']==2){
					$strHaving = " HAVING NOT FIND_IN_SET('".$search['branch_id']."', branchIdList) ";
				}
			}
			$orderby.=$strHaving;
		...
		//echo $sql.$where.$location.$orderby;
	...
	function getProductInfoByLocationItem($_data = null){
		...
		$sql = "SELECT td.id, $title AS product_name,
			pl.costing,pl.price, pl.price_set, pl.stock_alert, pl.note
	...
	function getAllProductSet($search = '',$items_type=null){
		...
		$base_url = Zend_Controller_Front::getInstance()->getBaseUrl();
		$urlEdit = $base_url."/stock/productset/edit/id/";
		...
		$sql = "SELECT 
			ide.id,
			(SELECT b.$branch FROM `rms_branch` AS b  WHERE b.br_id = ide.branch_id LIMIT 1) AS branch_name,
			ide.code,
			$grade,
			$degree as degree,
			ide.price,
			...
			(SELECT CONCAT(first_name) FROM rms_users WHERE ide.user_id=id LIMIT 1 ) AS user_name
			  ";
		$sql.=$dbgb->caseStatusShowImage("ide.status");
		$sql.=$dbgb->caseEdit("ide.id",$urlEdit);
		...
		if(!empty($search['branch_id'])){
			$where.= " AND ide.branch_id = ".$db->quote($search['branch_id']);
		}
		if(!empty($search['items_search'])){
		...
		$where.=$dbgb->getAccessPermission('ide.branch_id');
		return $db->fetchAll($sql.$where.$location.$orderby);
	}
	...
	*/
	public function addProductSet($_data){
		...
		try{
			...
			$this->_name = "rms_product_location";
			$proLocationId =  $this->insert($_arr);
	...
	public function updateProductSet($_data){
		...
		try{
			...
			$this->_name = "rms_product_location";
			$where = $_db->quoteInto("id=?", $proLocationId);
			$this->update($_arr, $where);
			...
			$this->_name='rms_product_setdetail';
			$where="id_pro_location = ".$proLocationId;
			if (!empty($detailId)){
				$where.=" AND id NOT IN ($detailId) ";
			}
			$this->delete($where);
	...
	function getAllProductSet($search = []){
		$db = $this->getAdapter();
		$tr = Application_Form_FrmLanguages::getCurrentlanguage();
		...
		$branch = $dbgb->getBranchDisplay();
		$strCount="(SELECT COUNT(DISTINCT psd.`subpro_id`)  FROM `rms_product_setdetail` AS psd WHERE psd.`id_pro_location` = pl.`id` LIMIT 1)";
		$sql = "
		SELECT 
			pl.`id`
			,b.$branch AS branch_name
			,itd.`code`
			,itd.$grade AS titleRecord
			,it.$degree AS category
			,pl.`price`
			,CASE    
				WHEN  itd.`is_onepayment` = 0 THEN '".$tr->translate("IS_VALIDATE")."'
				WHEN  itd.`is_onepayment` = 1 THEN '".$tr->translate("ONE_PAYMENTONLY")."'
			END AS is_onepayment
			,pl.`date`
			,(SELECT u.first_name FROM rms_users AS u WHERE pl.`user_id`=u.id LIMIT 1 ) AS user_name
		";
		$sql.=$dbgb->caseStatusShowImage("pl.`status`");
		$sql.=$dbgb->caseEdit("pl.`id`",$urlEdit);
		$sql.="
			,CONCAT('".$tr->translate("AMOUNT").":',' ',$strCount,' ','".$tr->translate("ITEMS")."') AS subTitleRecord
		";
		
		$sql.=" 
		FROM 
			`rms_product_location` AS pl 
			JOIN `rms_itemsdetail` AS itd ON itd.`id` = pl.`pro_id` AND itd.`is_productseat` =1 
			JOIN `rms_branch` AS b ON b.`br_id` = pl.`branch_id`
			LEFT JOIN `rms_items` AS it ON it.`id` = itd.`items_id` AND it.type=3 
		WHERE pl.`isProductSet` =1
		";
		$orderby = " ORDER BY itd.items_id ASC,itd.ordering ASC, pl.id DESC ";
		$where = ' ';
		
		$from_date =(empty($search['start_date']))? '1': " pl.`date` >= '".$search['start_date']." 00:00:00'";
		$to_date = (empty($search['end_date']))? '1': " pl.`date` <= '".$search['end_date']." 23:59:59'";
		$where = " AND ".$from_date." AND ".$to_date;
		...
		$s_search = addslashes(trim($search['adv_search']));
			...
			$s_where[] = " itd.code LIKE '%{$s_search}%'";
			$s_where[] = " pl.`price` LIKE '%{$s_search}%'";
			$sql .=' AND ( '.implode(' OR ',$s_where).')';
		}
		if(!empty($search['branch_id'])){
			$where.= " AND pl.branch_id = ".$db->quote($search['branch_id']);
		}
		if(!empty($search['items_search'])){
			$arrCon = array(
				"categoryId" => $search['items_search'],
			...
			if (!empty($condiction)){
				...
				$where.=" AND itd.`items_id`=".$search['items_search'];
			}
		}
		if($search['is_onepayment']>-1 AND $search['is_onepayment']!=''){
			$where.= " AND itd.is_onepayment = ".$db->quote($search['is_onepayment']);
		}
		if($search['status']>-1 AND $search['status']!=''){
			$where.= " AND pl.`status` = ".$db->quote($search['status']);
		}
		$where.=$dbgb->getAccessPermission('pl.branch_id');
		return $db->fetchAll($sql.$where.$location.$orderby);
	}
	function getProductSetLocationById($id){
		$db=$this->getAdapter();
		$sql="
			SELECT 
				pl.*
				,(SELECT p.items_id FROM `rms_itemsdetail` AS p WHERE p.id = pl.pro_id LIMIT 1) AS items_id
			FROM rms_product_location AS pl 
			WHERE pl.id = $id
		";
		$dbgb = new Application_Model_DbTable_DbGlobal();
		$sql.=$dbgb->getAccessPermission('pl.branch_id');
		return $db->fetchRow($sql);
	}
	function getProductSetDetailById($data){
		$db=$this->getAdapter();
		$id = empty($data["id"]) ? 0 : $data["id"];
		if(empty($data["forCopyBranchId"])){
			$sql="
				SELECT 
					psd.id
					,psd.`branch_id`
					...
					,psd.`price`
					,psd.`id_pro_location`
					,(SELECT p.title FROM `rms_itemsdetail` AS p WHERE p.id = psd.subpro_id LIMIT 1) AS title
					,(SELECT p.cost FROM `rms_itemsdetail` AS p WHERE p.id = psd.subpro_id LIMIT 1) AS pro_price
				FROM rms_product_setdetail AS psd
					JOIN rms_product_location AS pl ON pl.id = psd.id_pro_location
				WHERE psd.id_pro_location=$id";
		}else{
			$sql="
				SELECT 
					psd.id
					...
					,psd.`remark`
					,psd.`id_pro_location`
					,COALESCE(nPl.`costing`,0) AS costing
					,COALESCE(nPl.`price_set`,0) AS price_set
					,(SELECT p.title FROM `rms_itemsdetail` AS p WHERE p.id = psd.subpro_id LIMIT 1) AS title
					,(SELECT p.cost FROM `rms_itemsdetail` AS p WHERE p.id = psd.subpro_id LIMIT 1) AS pro_price
				 ";
				...
				if(!empty($data["productId"])){
					$sql.=" 
					FROM rms_product_setdetail AS psd 
						LEFT JOIN `rms_product_location` AS nPl ON nPl.`branch_id` = ".$data["forCopyBranchId"]." AND nPl.`pro_id` = psd.`subpro_id`
					WHERE 1 ";
					$sql.=" AND psd.pro_id= ".$data["productId"];
					$sql.=" AND psd.branch_id= ".$data["baseOnLocation"];
				}else{
					$sql.=" 
					FROM rms_product_setdetail AS psd
						LEFT JOIN `rms_product_location` AS nPl ON nPl.`branch_id` = ".$data["forCopyBranchId"]." AND nPl.`pro_id` = psd.`subpro_id`
					WHERE 1 ";
					$sql.=" AND psd.id_pro_location=$id";
				}
		}
	...
	function checkProductSetHasExitInLocation($data){
		$db = $this->getAdapter();
		$branchId = empty($data['branch_id']) ? 0 : $data['branch_id'];
		$productId = empty($data['product_id']) ? 0 : $data['product_id'];
		$sql="
		SELECT 
			pl.id 
		FROM 
		...
		WHERE 
			pl.`isProductSet`=1 
			AND pl.`branch_id`=$branchId 
			AND pl.`pro_id` = $productId 
		 ";
		if (!empty($data['id'])){
			$sql.=" AND pl.id != ".$data['id'];
		}
		$sql.=" LIMIT 1";
		$row = $db->fetchOne($sql);
		if (empty($row)) {
