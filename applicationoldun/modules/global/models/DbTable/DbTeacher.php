...
class Global_Model_DbTable_DbTeacher extends Zend_Db_Table_Abstract
{
	...
	function getAllTeacher($search){
		...
		$sql = "SELECT t.id, 
				...
				t.teacher_name_en,
				v1.$label AS sex,
				v2.$label AS staff_type,
				v3.$label AS nationality, 
				(SELECT $label FROM rms_view WHERE rms_view.type=3 AND rms_view.key_code=t.degree LIMIT 1) AS degree,
				(SELECT $label FROM rms_view WHERE rms_view.type=24 AND rms_view.key_code=t.teacher_type LIMIT 1) AS teacher_type,
				t.position_add,
				t.tel,
				t.email,
				t.note,
				(SELECT so.title FROM `rms_schooloption` AS so WHERE so.id = t.schoolOption LIMIT 1) AS schoolOption,
				CASE
				   	WHEN  t.active_type = 1 THEN '".$tr->translate("STOP")."'
				   	WHEN  t.active_type = 0 THEN '".$tr->translate("ACTIVING")."'
			   	END AS active_type 
			";
		$sql.=$dbp->caseStatusShowImage("t.status");
		$sql.=" FROM rms_teacher AS t
			LEFT JOIN rms_branch AS b ON b.br_id=t.branch_id
			LEFT JOIN rms_view v1 ON v1.type=2 AND v1.key_code=t.sex
			LEFT JOIN rms_view v2 ON v2.type=26 AND v2.key_code=t.staff_type
			LEFT JOIN rms_view v3 ON v3.type=21 AND v3.key_code=t.nationality
		 WHERE 1 ";
		...
		$where.= $dbp->getSchoolOptionAccess('t.schoolOption');
		$where.= $dbp->getDegreePermission('t.degreeList');
