...
class Issue_StudentattendanceController extends Zend_Controller_Action {
	...
	const AUTO_LIST_STU_ATT_FROM_SCAN = 0; // 0 = off, 1 = turn on "ទាញសិស្សិត្រូវបញ្ចូលអវត្តមានពីការ Scan Attendance"
	...
	public function indexAction(){
		try{
			...
			$collumns = array("BRANCH","GROUP","SEMESTER","ATTENDANCE_DATE","SESSION_TYPE","amtStuAtt","USER","STATUS");
			$link=array(
	...
	public	function addAction(){
		...
		$this->view->initialStudent = self::AUTO_LIST_STU_ATT_FROM_SCAN;
	...
	public	function editAction(){
		...
		//if ($result['is_pass'] > 0) {
		if ($result['is_pass'] != 0 && $result['is_pass'] !=2 ) {
	...
	function getpreattAction(){
		if($this->getRequest()->isPost()){
			$data = $this->getRequest()->getPost();
			$db = new Issue_Model_DbTable_DbStudentAttendance();
			$data=$db->getInitialStudentAtt($data);
			print_r(Zend_Json::encode($data));
			exit();
		}
	}
