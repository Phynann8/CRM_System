...
class Issuesetting_Model_DbTable_DbScheduleSetting extends Zend_Db_Table_Abstract
{
    ...
    function getAllScheuleSetting($search = ''){
    	...
    	$id = "s.id";
		$viewing = ', CONCAT("<a class=\"btn btn-primary btn-xs\" href=\"", "javascript:void(0);", "\" onClick=\"", "getScheduleSettingInfo(",'.$id.',",2);", "\" >",
		"<i class=\"fa fa-eye\"></i> ", "'.$tr->translate("DETAIL").'", "</a>") AS viewing';
		$sql = " 
			SELECT 
				s.id
	...
	$branch = $dbp->getBranchDisplay();
   	...
   	$sql="
		SELECT 
			...
			,s.title AS settingTitle 
			,(SELECT t.title FROM rms_timeseting As t WHERE t.value =sd.from_hour LIMIT 1) AS fromHourTitle
			,(SELECT t.title FROM rms_timeseting As t WHERE t.value =sd.to_hour LIMIT 1) AS toHourTitle
		FROM 
			`rms_schedulesetting_detail` AS sd 
			JOIN rms_schedulesetting AS s ON sd.setting_id= s.id
		WHERE sd.setting_id=$id
   ...
   function getScheduleSettingInfo($_data){
   	 
		$setting_id = empty($_data['scheduleSetting'])?0:$_data['scheduleSetting'];
		$getHtml = empty($_data['getHtml'])?0:$_data['getHtml'];
		$scheduleSetting = $this->getScheduleSettingDetail($setting_id);
		...
		if(!empty($getHtml)){
			...
			if($getHtml==2){
				$string='<div>';
					$title=0;
					$titles=0;
					if(!empty($scheduleSetting)) foreach($scheduleSetting AS $key => $row){
						if($key==0){
							$branchName = $row["branchName"];
							$settingTitle = $row["settingTitle"];
						...
						if($row['from_hour']<=12){
							if($title==0){
								$string.='
									<div>
										<h3 class="control-label text-center ">
											<strong class="text-primary">Morning</strong>
									   </h3>
								   </div>
								';
								$title=1;
							...
							if($titles==0){
								$string.='
									<div>
										<h3 class="control-label text-center  ">
											<strong class="text-primary">Afternoon</strong>
									   </h3>
									</div>
								';
								$titles=1;
						...
						$string.='
								<div>
									<small class="control-label bold ">
										<i class="fa fa-clock-o"></i> '.$row["fromHourTitle"].'-'.$row["toHourTitle"].'
									</small>
							   </div>
							';
					}
				...
				$string='
							...
							<div class="col-md-12 col-sm-12 col-xs-12"> 
								<div class="d-flex"> 
									...
									<div class="col-md-10 col-sm-10 col-xs-12"> 
										<p class="tx-20 font-weight-semibold d-flex ">Schedule Setting Info</p>
									</div> 
								</div>
					...
					if(!empty($scheduleSetting)) foreach($scheduleSetting AS $key => $row){
						if($key==0){
							...
							$settingTitle = $row["settingTitle"];
							$string.='
								<h5 class="control-label bold col-md-12 col-sm-12 col-xs-12 text-center">
									'.$row["settingTitle"].'
							   </h5>
							';
						}
