...
class Mobileapp_Model_DbTable_DbDashboard extends Zend_Db_Table_Abstract
{
	...
	function getAccountInfo($search){
		$db=$this->getAdapter();
		...
		CONCAT(
			'[',
			GROUP_CONCAT('{',
				'".'"device"'.":','".'"'."',mtk.`device_model_name`,'".'"'."',
				'".',"os"'.":','".'"'."',mtk.`device_os`,'".'"'."',
				'".',"lastAct"'.":','".'"'."',mtk.`last_active`,'".'"'."',
				'".',"timeZone"'.":','".'"'."',mtk.`timezone`,'".'"'."',
				
			'}' ORDER BY mtk.`last_active` DESC )
			,']'
			)
		";
		...
		$order=" GROUP BY mtk.stu_id 
		ORDER BY mtk.stu_id DESC ";
		...
		if(!empty($search['tokenType'])){
			if($search['tokenType']==1){
				$querySelect="
					...
					,s.stu_khname AS userNameInKh
					,CASE 
						WHEN s.password = '".$defaultPass."' THEN 1 
						ELSE 2
					END isDefaultPass
					,CONCAT(COALESCE(s.last_name,''),' ',COALESCE(s.stu_enname,'')) AS userNameInEn
					,(SELECT CONCAT(COALESCE(ac.`fromYear`,''),' ',COALESCE(ac.`toYear`,'')) FROM rms_academicyear AS ac WHERE ac.id = gds.academic_year LIMIT 1) AS academicYear
				...
				$queryJoinTable="
				FROM 
					(rms_student AS s JOIN rms_group_detail_student AS gds ON gds.stu_id = s.stu_id AND gds.itemType=1 AND gds.is_current=1 AND gds.is_maingrade=1) 
				";
				$queryJoinTable.=" LEFT JOIN  mobile_mobile_token AS mtk  ON s.stu_id = mtk.stu_id AND mtk.tokenType = 1 ";
				$queryWhere="";
				if(!empty($search['adv_search'])){
					$s_where = array();
					$s_search = str_replace(' ', '', addslashes(trim($search['adv_search'])));
				...
				if(!empty($search['academic_year'])){
					$queryWhere.=" AND gds.academic_year = ".$search['academic_year'];
				}
				if(!empty($search['degree'])){
					$queryWhere.=" AND gds.degree = ".$search['degree'];
				}
				...
				if(!empty($search['passwordStatus'])){
					if($search['passwordStatus']==1){
						$queryWhere.=" AND s.password = '".$defaultPass."'";
					}else{
						$queryWhere.=" AND s.password != '".$defaultPass."'";
					}
				}
				...
				$querySelect="
					...
					,t.teacher_name_en AS userNameInEn
					,CASE 
						WHEN t.password = '".$defaultPass."' THEN 1 
						ELSE 2
					END isDefaultPass
				";
				$queryJoinTable="
				...
				";
				$queryJoinTable.=" LEFT JOIN  mobile_mobile_token AS mtk  ON t.id = mtk.stu_id AND mtk.tokenType = 3 ";
				$queryWhere="";
				if(!empty($search['adv_search'])){
					$s_where = array();
					$s_search = str_replace(' ', '', addslashes(trim($search['adv_search'])));
				...
				if(!empty($search['passwordStatus'])){
					if($search['passwordStatus']==1){
						$queryWhere.=" AND t.password = '".$defaultPass."'";
					}else{
						$queryWhere.=" AND t.password != '".$defaultPass."'";
					}
				}
		...
		";
		$sql.=	$queryWhere;
		$tokenType= empty($search['tokenType']) ? 0 : $search['tokenType'];
		if(!empty($search["usedAppStatus"])){
			if($search["usedAppStatus"]==1){
				$sql.=" AND mtk.tokenType = ".$tokenType;
				$sql.=' AND mtk.stu_id IS NOT NULL ';
			}else if($search["usedAppStatus"]==2){
				$sql.=' AND mtk.stu_id IS NULL ';
			}
		}else{
			
			
			$sql.=" AND mtk.stu_id > 0 ";
		}
		$sql.=$order;
