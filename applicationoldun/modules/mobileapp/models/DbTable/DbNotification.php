...
class Mobileapp_Model_DbTable_DbNotification extends Zend_Db_Table_Abstract
{
	...
	function getAllNotification($search){
		...
		$sql="
			SELECT 
				...
				,CASE 
					WHEN mba.opt_notification = 2 THEN '".$tr->translate("GROUP")."' 
					WHEN mba.opt_notification = 3 THEN '".$tr->translate("STUDENT")."' 
					WHEN mba.opt_notification = 4 THEN '".$tr->translate("DEGREE")."' 
					WHEN mba.opt_notification = 5 THEN '".$tr->translate("TEACHER")."' 
					ELSE '".$tr->translate("FOR_ALL")."'
				END AS option_type
	...
	function add($data){
        ...
        try{
    		...
    		if (!empty($name)){
    			$tem =explode(".", $name);
    			$imageName = "an-".date("Y").date("m").date("d").time().".".end($tem);
    			$tmp = $_FILES['photo']['tmp_name'];
    			if(move_uploaded_file($tmp, $part.$imageName)){
					$arr['imageFeature']= $imageName;
    			}
    		}
