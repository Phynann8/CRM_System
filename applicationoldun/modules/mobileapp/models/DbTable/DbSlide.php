...
// 	}
	...
	function getImageSlideForRemove($remainImgStr){
		$db= $this->getAdapter();
    	$sql="
			SELECT 
				gx.* 
			FROM 
				mobile_slideshow AS gx 
				WHERE 1 ";
		$sql.= " AND gx.id NOT IN (".$remainImgStr.")";
    	return $db->fetchAll($sql);
	}
			...
			foreach ($ids as $i){
				if (!empty($_data['detail_id'.$i])){
					if (empty($detail_list)){
						...
						$detail_list=$detail_list.",".$_data['detail_id'.$i];
					}
				}
			}
			if (!empty($detail_list)){
				
				$removeRs = $this->getImageSlideForRemove($detail_list);
				if(!empty($removeRs)) foreach($removeRs AS $remove){
					if( $remove["images"] !=""){
						$thisRvFile = $part.$remove["images"];
						if (file_exists($thisRvFile)){
							if (is_file($thisRvFile)){
								unlink($thisRvFile);
							}
						}
					}
			...
			foreach ($ids as $i){
				if (!empty($_data['detail_id'.$i])){
					if (!empty($_FILES['photo'.$i]['name'])){
						...
						$oldPhotoRow = empty($_data['old_photo'.$i]) ? "" : $_data['old_photo'.$i];
						if( $oldPhotoRow!=""){
							$thisFileRow = $part.$oldPhotoRow;
							if (file_exists($thisFileRow)){
								if (is_file($thisFileRow)){
									unlink($thisFileRow);
								}
							}
						}
