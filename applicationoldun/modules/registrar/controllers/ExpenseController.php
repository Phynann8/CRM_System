...
class Registrar_ExpenseController extends Zend_Controller_Action
{
    ...
    public function addAction()
    {
		...
		$dbexp = new Registrar_Model_DbTable_DbExpenseCategory();
		$this->view->rsmaincategory = $dbexp->getParentCategoryExpense(0,null,null,0,1,1);
    ...
    public function editAction()
    {
		...
		$this->view->row = $row;
		if(empty($row)){
			Application_Form_FrmMessage::Sucessfull("NO_DATA",'/registrar/expense');
		}else{
	...
	function getExpenseInfoAction(){
    	if($this->getRequest()->isPost()){
    		$data=$this->getRequest()->getPost();
    		$db = new Registrar_Model_DbTable_DbExpense();
    		$row = $db->getExpenseInfo($data);
    		print_r(Zend_Json::encode($row));
    		exit();
    	}
    }
	
	public function addexpenseajAction(){
    	if($this->getRequest()->isPost()){
    		$_data = $this->getRequest()->getPost();
    		try {
    			$db = new Registrar_Model_DbTable_DbExpense();
    			print_r(Zend_Json::encode($db->addExpense($_data)));
    			exit();
    		} catch (Exception $e) {
    			Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
