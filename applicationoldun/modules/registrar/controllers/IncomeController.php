...
class Registrar_IncomeController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
    	try{
    		...
    		$collumns = array("BRANCH","STUDENT_ID","STUDENT_NAME","GROUP_CODE","INCOME_OPTION","RECEIPT_NO","TOTAL_INCOME","PAID_DATE","INCOME_CATEGORY","INCOME_TITLE","PAYMENT_METHOD","BANK_NAME","BANK_NOTE","NOTE","STATUS");
    		$link=array(
    				'module'=>'registrar','controller'=>'income','action'=>'edit',
    		);
    		$this->view->list=$list->getCheckList(10, $collumns,$rs_rows,array('branch_name'=>$link,'studentName'=>$link,'studentCode'=>$link,'group_name'=>$link,'cate_name'=>$link,'total_amount'=>$link,'invoice'=>$link,'optionType'=>$link));
    	}catch (Exception $e){
    ...
    public function editAction()
    {
		...
		if(empty($row)){
			Application_Form_FrmMessage::Sucessfull("NO_DATA",'/registrar/income');
		}else{
			$session_user=new Zend_Session_Namespace(SYSTEM_SES);
			$issupper_user = $session_user->issupper_user;
			$payment_date = date("Y-m-d",strtotime($row['date']));
			$current_date = date("Y-m-d");
			if($issupper_user!=1 AND $current_date>$payment_date){
			...
			$rowCredit  = $dbm->getCreditmemoByIncomeId($id);
			$this->view->rowCredit = $rowCredit;
	...
	public function addincomeajAction(){
    	if($this->getRequest()->isPost()){
    		$_data = $this->getRequest()->getPost();
    		try {
    			$db = new Registrar_Model_DbTable_DbIncome();
    			print_r(Zend_Json::encode($db->addIncome($_data)));
    			exit();
    		} catch (Exception $e) {
    			Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
