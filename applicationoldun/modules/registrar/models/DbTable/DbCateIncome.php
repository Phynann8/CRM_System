...
class Registrar_Model_DbTable_DbCateIncome extends Zend_Db_Table_Abstract
{
	...
	function getAllCateIncome($search=null,$parent = 0, $spacing = '', $cate_tree_array = ''){
		...
		if(!empty($search['adv_search'])){
			...
			$s_search = str_replace(' ', '', addslashes(trim($advSearch )));
			$s_where[]=" REPLACE(category_name,' ','') LIKE '%{$s_search}%'";
			$s_where[]=" REPLACE(account_code,' ','') LIKE '%{$s_search}%'";
	...
	function addCateIncome($data){
		...
		try{
			...
			$sql="SELECT id FROM rms_cate_income_expense WHERE category_name ='".$titleCate."'";
			$rs = $_db->fetchOne($sql);
			if(!empty($rs)){
				return -1;
			}
