...
//     }
	...
	function getDailyReport($search=null){//using
		try{
			...
			$sql=" SELECT
						...
						s.stu_enname,
						s.last_name,
						(SELECT it.$colTitle FROM `rms_items` AS it WHERE it.id=sp.degree LIMIT 1) AS degree,
						(SELECT itd.$colTitle FROM `rms_itemsdetail` AS itd WHERE itd.id=sp.grade LIMIT 1) AS grade,
						(SELECT pt.$sessionTitle FROM rms_parttime_list AS pt WHERE pt.id=sp.session LIMIT 1) AS session,
						...
						(SELECT v.$label FROM rms_view AS v WHERE v.type=10 AND v.key_code=sp.is_void LIMIT 1) AS void_status,
	...
	function getOtherIncomeById($id){//using
		...
		$sql="SELECT
					inc.*,
					(SELECT branch_namekh from rms_branch where br_id = inc.branch_id LIMIT 1) as branch_name,
					...
					(SELECT group_code FROM `rms_group` WHERE rms_group.id=inc.groupId LIMIT 1) AS group_name,
					...
					(SELECT bank_name FROM `rms_bank` b WHERE inc.bank_id = b.id LIMIT 1) AS bank_name,
					(SELECT CONCAT(last_name,' ',first_name) FROM rms_users AS u WHERE u.id = inc.user_id LIMIT 1) AS userName
				FROM
					ln_income  AS inc 
		...
		$sql.= $_dbGb->getAccessPermission("inc.branch_id");
		$sql.=" LIMIT 1 ";
