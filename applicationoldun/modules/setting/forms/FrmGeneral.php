...
Class Setting_Form_FrmGeneral extends Zend_Dojo_Form {
	...
	public function FrmGeneral($data=null){
		...
		$telegramBotName = new Zend_Dojo_Form_Element_TextBox('telegramBotName');
		$telegramBotName->setAttribs(array(
				'dojoType'=>'dijit.form.TextBox',
				'class'=>'fullside',
				'placeholder'=>$this->tr->translate("botName")
		));
		
		$telegramBotToken = new Zend_Dojo_Form_Element_TextBox('telegramBotToken');
		$telegramBotToken->setAttribs(array(
				'dojoType'=>'dijit.form.TextBox',
				'class'=>'fullside',
				'placeholder'=>$this->tr->translate("botToken")
		));
		
		$receiptDescTitle = new Zend_Dojo_Form_Element_TextBox('receiptDescTitle');
		$receiptDescTitle->setAttribs(array(
				'dojoType'=>'dijit.form.TextBox',
				'class'=>'fullside',
				'placeholder'=>$this->tr->translate("TITLE_KHMER")
		));
		$receiptDescTitleEn = new Zend_Dojo_Form_Element_TextBox('receiptDescTitleEn');
		$receiptDescTitleEn->setAttribs(array(
				'dojoType'=>'dijit.form.TextBox',
				'class'=>'fullside',
				'placeholder'=>$this->tr->translate("TITLE_ENGLISH")
		));
		
		$receiptDesc = new Zend_Dojo_Form_Element_Textarea('receiptDesc');
		$receiptDesc->setAttribs(
			array('dojoType'=>'dijit.form.Textarea',
			'class'=>'fullside',
			'style'=>'min-height:160px; font-size:10px; line-height:14px; ',
			)
		);
		$receiptDescEn = new Zend_Dojo_Form_Element_Textarea('receiptDescEn');
		$receiptDescEn->setAttribs(
			array('dojoType'=>'dijit.form.Textarea',
			'class'=>'fullside',
			'style'=>'min-height:160px; font-size:10px; line-height:14px; ',
			)
		...
		if($data!=null){
			...
			$telegramBotName->setValue($data['telegramBotName']['keyValue']);
			$telegramBotToken->setValue($data['telegramBotToken']['keyValue']);
			
			$receiptDescTitle->setValue($data['receiptDescTitle']['keyValue']);
			$receiptDescTitleEn->setValue($data['receiptDescTitle']['keyValueEn']);
			$receiptDesc->setValue($data['receiptDesc']['keyValue']);
			$receiptDescEn->setValue($data['receiptDesc']['keyValueEn']);
