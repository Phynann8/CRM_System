...
class Setting_Model_DbTable_DbGeneral extends Zend_Db_Table_Abstract
{
	...
	public function updateWebsitesetting($data){
		try{
			...
			$rows = $this->geLabelByKeyName('scoreSystemStatus');
			if (empty($rows)){
				$value = empty($data['scoreSystemStatus']) ? 0 : 1;
				$arr = array('keyValue'=>$value,'keyName'=>'scoreSystemStatus','note'=>"0=មិនប្រើប្រាស់  , 1=ប្រើប្រាស់ប្រព័ន្ធពិន្ទុ ",'user_id'=>$dbg->getUserId());
				$this->insert($arr);
			}else{
				$value = empty($data['scoreSystemStatus']) ? 0 : 1;
				$arr = array('keyValue'=>$value);
				$where=" keyName= 'scoreSystemStatus'";
				$this->update($arr, $where);
			}

			$rows = $this->geLabelByKeyName('disableChAvg');
			if (empty($rows)){
				$value = empty($data['disableChAvg']) ? 0 : 1;
				$arr = array('keyValue'=>$value,'keyName'=>'disableChAvg','note'=>"0=បូកពិន្ទិភាសាចិន  , 1=មិនបូកគិតពិន្ទិភាសាចិន ",'user_id'=>$dbg->getUserId());
				$this->insert($arr);
			}else{
				$value = empty($data['disableChAvg']) ? 0 : 1;
				$arr = array('keyValue'=>$value);
				$where=" keyName= 'disableChAvg'";
				$this->update($arr, $where);
			}
			...
			$rows = $this->geLabelByKeyName('telegramBotName');
			if (empty($rows)){
				$arr = array(
					'keyValue'=>$data['telegramBotName']
					,'keyName'=>'telegramBotName'
					,'note'=>"Get From Bot Telegram"
					,'user_id'=>$dbg->getUserId()
				);
				$this->insert($arr);
			}else{
				$arr = array('keyValue'=>$data['telegramBotName']);
				$where=" keyName= 'telegramBotName'";
				$this->update($arr, $where);
			}
			
			$rows = $this->geLabelByKeyName('telegramBotToken');
			if (empty($rows)){
				$arr = array(
					'keyValue'=>$data['telegramBotToken']
					,'keyName'=>'telegramBotToken'
					,'note'=>"Token Of Bot Telegram"
					,'user_id'=>$dbg->getUserId()
				);
				$this->insert($arr);
			}else{
				$arr = array('keyValue'=>$data['telegramBotToken']);
				$where=" keyName= 'telegramBotToken'";
				$this->update($arr, $where);
			}
			...
			$rows = $this->geLabelByKeyName('receiptDescTitle');
			if (empty($rows)){
				$arr = array(
						'keyValue'=>$data['receiptDescTitle']
						,'keyValueEn'=>empty($data['receiptDescTitleEn']) ? $data['receiptDescTitle'] : $data['receiptDescTitleEn']
						,'keyName'=>'receiptDescTitle'
						,'note'=>"ចំណងជើងពណ៌នាលើបង្កាន់ដៃបង់ប្រាក់សិស្ស"
						,'user_id'=>$dbg->getUserId()
					);
				$this->_name='rms_setting';
				$this->insert($arr);
				...
				$where=" keyName= 'receiptDescTitle'";
				$this->_name='rms_setting';
				$this->update($arr, $where);
			}
			
			$rows = $this->geLabelByKeyName('receiptDesc');
			if (empty($rows)){
				$arr = array(
						'keyValue'=>$data['receiptDesc']
						,'keyValueEn'=>empty($data['receiptDescEn']) ? $data['receiptDesc'] : $data['receiptDescEn']
						,'keyName'=>'receiptDesc'
						,'note'=>"ពណ៌នាលើបង្កាន់ដៃបង់ប្រាក់សិស្ស"
						,'user_id'=>$dbg->getUserId()
					);
				$this->_name='rms_setting';
				$this->insert($arr);
				...
				$where=" keyName= 'receiptDesc'";
				$this->_name='rms_setting';
				$this->update($arr, $where);
			}
			
			if(!empty($data['branchIdetity'])){
				$ids = explode(',', $data['branchIdetity']);
				foreach ($ids as $i){
					$rowStatus = empty($data['brId'.$i]) ? 0 : 1;
					$_arr = array(
						'showReceiptDesc'		=>$rowStatus,	
					);
					$whereFt = ' br_id = '.$i;
	...
	function getAllBranchForSetting(){
		$db = $this->getAdapter();
		$sql="
			SELECT 
				b.`br_id` AS id
				,b.`branch_code`
				,b.`abbreviations` AS shortcut
				,b.`branch_nameen` AS branchName
				,b.`branch_namekh` AS branchNameKh
				,b.`showReceiptDesc`
				,b.`br_address` as brAddress
			FROM 
				`rms_branch` AS b 
			WHERE b.status=1 
		";
