    	...
    	try{
			...
			if (!empty($name)){
				...
				if(move_uploaded_file($tmp, $part.$image_name)){
					...
					$oldBackBg = empty($_data['oldBackBg']) ? "" : $_data['oldBackBg'];
					if( $oldBackBg!=""){
						$thisFileBackBg = $part.$oldBackBg;
						if (file_exists($thisFileBackBg)){
							if (is_file($thisFileBackBg)){
								unlink($thisFileBackBg);
							}
						}
					}
				}
