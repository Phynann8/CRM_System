...
class Stock_CutstockController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$collumns = array("BRANCH","RECEIPT_NO","REF_RECEIPT_NUM","STUDENT","TYPE","NOTE","DATE","STATUS","ACTION");
			$link=array(
					'module'=>'stock','controller'=>'cutstock','action'=>'edit',
			);
			$this->view->list=$list->getCheckList(10, $collumns, $rows,array('branch_name'=>$link,'serailno'=>$link,'refReceiptNum'=>$link,));
			}catch (Exception $e){
...
// 		print_r($row);exit();
	...
	function receiptAction(){
		...
		if(!empty($paymentId)){
			...
			if(empty($rowcutstock)){
				Application_Form_FrmMessage::Sucessfull("No Cutstock Receipt ! please check again.","/registrar/register");
				exit();
			}
		...
		if(!empty($cutstockid)){
			...
			Application_Form_FrmMessage::Sucessfull("No Payment Receipt ! please check again.","/stock/cutstock");
			exit();
	...
	function getProdctInfoAction(){
		if($this->getRequest()->isPost()){
			$data = $this->getRequest()->getPost();
			$proId = $data['proId'];
			$_dbcht = new Stock_Model_DbTable_DbCutStock();
			$itemsCode = $_dbcht->getProductInfoById($data);
			print_r(Zend_Json::encode($itemsCode));
			exit();
		}
	}
