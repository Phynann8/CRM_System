...
class Stock_PurchaseController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$this->view->list=$list->getCheckList(0, $collumns, $rows,array( 'branch_name'=>$link,'purchaseNo'=>$link,
			...
			// $form=$frm->FormSearchProductPurchase();
			// Application_Model_Decorator::removeAllDecorator($form);
			// $this->view->form_search=$form;
	}
	...
	public function editAction(){
		...
		$_pur = new Stock_Model_DbTable_DbPurchase();
		$row = $_pur->getPurchaeSupplierById($id);
		if (empty($row)){
