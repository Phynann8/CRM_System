...
class Stock_RequestproductController extends Zend_Controller_Action {
	...
	public function indexAction(){
		try{
			...
			$collumns = array("BRANCH","REQUEST_NO","REQUEST_FOR","FOR_SECTION","PURPOSE","REQUEST_DATE","TOTAL","USER","ACTION","REQUEST_STATUS","STATUS",);
			$link=array(
					'module'=>'stock','controller'=>'requestproduct','action'=>'edit',
			);
			$this->view->list=$list->getCheckList(10, $collumns, $rs_rows,array('request_no'=>$link,'request_for'=>$link,'purpose'=>$link,));
		}catch (Exception $e){
	...
	public function editAction(){
		...
		if (empty($row)){
			...
			exit();
		}elseif($row['isReturn']==1){
			Application_Form_FrmMessage::Sucessfull("Alreay Returned!","/stock/requestproduct");
			exit();
		}
		$this->view->row= $row;
	...
	public function returnAction(){
		...
		if($this->getRequest()->isPost()){
			...
			try{
				$db = new Accounting_Model_DbTable_DbRequestProduct();
				 $db->returnStock($_data);
				if(isset($_data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("SUCCESS","/stock/requestproduct");
				}else{
					Application_Form_FrmMessage::Sucessfull("SUCCESS","/stock/requestproduct");
				}
				Application_Form_FrmMessage::message("INSERT_SUCCESS");
		...
		if (empty($row)){
			...
			Application_Form_FrmMessage::Sucessfull("Disactive","/stock/requestproduct");
			exit();
		}elseif($row['isReturn']==1){
			Application_Form_FrmMessage::Sucessfull("Alreay Returned!","/stock/requestproduct");
			exit();
