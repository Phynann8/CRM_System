...
class Stock_Model_DbTable_DbAdjustStock extends Zend_Db_Table_Abstract
{
	...
	function getAllProductLocattion($search=null){
		...
		$urlView = $base_url."/stock/initizeqty/view/id/";
    	...
    	$sql="SELECT pl.id, 
			...
			pl.price_set,
			CASE    
				WHEN  t.product_type  = 1 THEN '".$tr->translate("PRODUCT_FOR_SELL")."'
				WHEN  t.product_type  = 2 THEN '".$tr->translate("OFFICE_MATERIAL")."'
			END AS product_type,
    	   pl.`date`,
    	   u.first_name AS byuser,
    	   pl.status ";
    	$sql.=$dbp->caseStatusShowImage("pl.status");
    	$sql.=$dbp->caseView("pl.id", $urlView);
    	$sql.=" FROM `rms_product_location` AS pl 
				LEFT JOIN `rms_itemsdetail` AS t ON t.id = pl.pro_id
				LEFT JOIN `rms_users` AS u ON pl.user_id = u.id 
			  WHERE pl.pro_id IS NOT NULL 
		...
		if(!empty($search['category_id'])){
			...
			if (!empty($condiction)){
				$where.=" AND t.items_id IN ($condiction)";
			}else{
				$where.=" AND t.items_id=".$search['category_id'];
			}
		}
		if($search['product_type']>0 and !empty($search['product_type'])){
    		$where.=" AND t.product_type =".$search['product_type'];
    	}
			...
			if($data['identity']!=""){
				...
				foreach ($ids as $i)
				{
					...
					$param = [
						...
						'movement_type' => 8, // 8= Adjust Stock,
						'quantity'      => $data['spacing_qty'.$i],
						'reference'     => $adjuststock_id,
						'note'          => "Adjust Stock",
					];
 	...
 	function updateAdjustStock($data){
		$dbmv = new Stock_Model_DbTable_DbMovement();
 		...
 		try{
			...
			$dbmv->deleteMovementStock($data['id'], 8); // delete movement stock
