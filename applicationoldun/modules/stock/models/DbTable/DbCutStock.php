...
class Stock_Model_DbTable_DbCutStock extends Zend_Db_Table_Abstract
{
    ...
    public function getStudentProductPaymentDetail($data){
    	...
    	$sql=" SELECT 
					...
					s.stu_khname AS student_name, 
					s.stu_enname, 
					s.last_name, 
					s.stu_code, 
					s.tel, 
					rv.name_kh AS gender, 
					ie.code AS productCode, 
					ie.title AS items_name, 
					ie.items_type ,
					pl.pro_qty
				FROM 
					rms_student_payment AS sp
				JOIN 
					rms_saledetail AS spd ON spd.payment_id = sp.id
				LEFT JOIN 
					rms_student AS s ON s.stu_id = sp.student_id
				LEFT JOIN 
					rms_itemsdetail AS ie ON (ie.id = spd.pro_id AND ie.items_type = 3)
				LEFT JOIN
					rms_product_location AS pl ON ( pl.branch_id = sp.branch_id AND pl.pro_id = spd.pro_id )
				LEFT JOIN 
					rms_view AS rv ON rv.type = 2 AND rv.key_code = s.sex
				WHERE 
					sp.is_void = 0
			...
			if(!empty($type )){
                if($type ==1){
                    $sql.= " AND spd.qty_after >0 ";
					 ...
					 $sql .= " AND sp.id = (
                        SELECT sp2.id 
                        FROM rms_student_payment AS sp2 
                        WHERE sp2.student_id = $student_id 
                          AND sp2.branch_id = $branch_id 
                          AND sp2.is_void = 0 
                        ORDER BY sp2.create_date DESC 
                        LIMIT 1
                    )";
				}
			}
    	...
    	if(!empty($rs)){
    		foreach ($rs as $key => $row){
    			...
    			&nbsp;</td>
    			<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 100px;">
    				<select queryExpr="*${0}*" autoComplete="false" dojoType="dijit.form.FilteringSelect" onchange="resetProductCode('.$no.')" class="fullside" name="itemdetail_id'.$no.'" id="itemdetail_id'.$no.'" >';
    	...
    	$string.='</select>
				...
				<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 70px;">&nbsp;
    				<label id="currentqtylabel'.$no.'">'.number_format($row['pro_qty'],0).'</label>
    				<input type="hidden" dojoType="dijit.form.TextBox" name="current_qty'.$no.'" id="current_qty'.$no.'" value="'.$row['pro_qty'].'" >
    			</td> 
				<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 70px;">&nbsp;
...
if(!empty($type)){
	if($type==1){
		...
		if($row['pro_qty'] < $qty_after){
			$qty_after=$row['pro_qty'];
			$remain = $row['qty_after']-$qty_after;
		}
		$string.='<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc;  min-width: 70px; ">&nbsp;
					...
					<td style="width: 70px;"><input type="text" class="fullside" dojoType="dijit.form.NumberTextBox" required="required" onKeyup="calculateqty('.$no.', '.$type.');" name="qty_receive'.$no.'" id="qty_receive'.$no.'" value="'.$qty_after.'" style="text-align: center;" ></td>
					<td style="width: 70px;"><input type="text" class="fullside" readonly="readonly" dojoType="dijit.form.NumberTextBox" required="required" name="remain'.$no.'" id="remain'.$no.'" value="'.$remain.'" style="text-align: center;" ></td>
					';
	}elseif($type==2){
		$string.='
...
//     	AND inv.branch_id =".$data['branch_id'];
    ...
    function addCutStock($_data){
    	try{
    		...
    		foreach ($ids as $i){
				...
				if($type==1){
					$dbpu->updateStock($_data['itemdetail_id'.$i],$_data['branch_id'],-$_data['qty_receive'.$i],0,$_data['payment_id'.$i],5);
				}elseif($type==2){
					$dbpu->updateStock($_data['itemdetail_id'.$i],$_data['branch_id'],+$_data['qty_balance'.$i],0,$_data['payment_id'.$i],5);
				}
    		}
    		return $cut_id;
    	}catch(Exception $e){
    ...
    function getAllCutStock($search){
		...
		$urlEdit = $base_url."/stock/cutstock/edit/id/";
    ...
    function getCutStockDetailBYId($cutstockid){
    	...
    	$sql="SELECT ct.*,
				...
				LEFT JOIN rms_saledetail AS sd ON sd.`id` = ct.`student_paymentdetail_id`
				LEFT JOIN rms_student_payment AS sp ON sp.id = ct.`paymentId`
    ...
    function getCutstockDetailByCutstockIdAndStuDetailId($cutstockid,$stu_paymetdetail_id){
    	...
    	$sql="SELECT 
				...
				p.qty,
				p.qty_after,
				pl.`pro_qty` AS currentQty
			FROM rms_cutstock_detail AS ct
			LEFT JOIN `rms_cutstock` AS c ON c.`id` = ct.`cutstock_id`
			LEFT JOIN rms_itemsdetail AS ie ON ct.product_id = ie.id
			LEFT JOIN rms_saledetail AS p ON ct.student_paymentdetail_id = p.id
			LEFT JOIN `rms_product_location` AS pl ON (pl.`pro_id` = ct.`product_id` AND pl.`branch_id` = c.`branch_id`)
			WHERE 
				ct.cutstock_id = $cutstockid 
				AND ct.student_paymentdetail_id = $stu_paymetdetail_id;
			";
    	return $db->fetchRow($sql);
    }
    public function getStudentProductPaymentDetailEdit($data){
    	...
    	$sql="SELECT 
				...
				ie.title AS items_name,
				ie.items_type
			FROM rms_saledetail AS spd
			LEFT JOIN rms_student_payment AS sp ON spd.payment_id = sp.id
			LEFT JOIN rms_student AS s ON sp.student_id = s.stu_id
			LEFT JOIN rms_itemsdetail AS ie ON spd.pro_id = ie.id
			WHERE 
				ie.items_type = 3
				AND sp.is_void = 0
				AND spd.qty_after > 0
    	...
    	if(!empty($rs)){
	    	...
	    	foreach ($rs as $key => $row){
			    ...
			    if (!empty($rowpaymentdetail)){
					...
					$current_qty = $rowpaymentdetail['currentQty']+$rowpaymentdetail['qty_receive'];
			    	...
			    	<tr id="row'.$no.'" style="background: #fff; border: solid 1px #bac;">
				    	...
				    	<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 100px;">
				    		<select queryExpr="*${0}*" autoComplete="false" dojoType="dijit.form.FilteringSelect" class="fullside" onchange="resetProductCode('.$no.')"  name="itemdetail_id'.$no.'" id="itemdetail_id'.$no.'" >';
						    	if (!empty($allproduct)) foreach ($allproduct as $pro){
						    		$selected="";
				   ...
				   $string.='</select></td>
				   		<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 70px;">&nbsp;
				    		<label id="currentqtylabel'.$no.'">'.number_format($current_qty,0).'</label>
				    		<input type="hidden" dojoType="dijit.form.TextBox" name="current_qty'.$no.'" id="current_qty'.$no.'" value="'.$current_qty.'" >
				    	</td>
				    	<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 70px;">&nbsp;
...
//     			print_r($rowpaymentdetail);exit();
    			if (!empty($rowpaymentdetail)){
    				...
    				$dbpu = new Stock_Model_DbTable_DbPurchase();
    				$dbpu->updateStock($pay_detail['product_id'],$_data['branch_id'],$qtyreceive,0,0,5);
    			}
    		}
    	...
    	try{
    		...
    		if (!empty($row)) foreach ($row as $pay_detail){
    			...
    			//	$paymenttailbysale = $this->getSumCutStockDetailByStuPayDetId($pay_detail['student_paymentdetail_id'], $pay_detail['id']);// get other pay amount on this Purchase id on other payment receipt number
					...
					if($type==1){
						$dbpu = new Stock_Model_DbTable_DbPurchase();
						$dbpu->updateStock($pay_detail['product_id'],$branch_id,$qtyreceive,0,0,5);
	...
	function getProductInfoById($data){
    	$db=$this->getAdapter();
    	$sql="SELECT 
				pl.*,
				p.`code`,
			...
			LEFT JOIN `rms_itemsdetail` AS p ON p.`id` = pl.`pro_id`
			WHERE pl.`pro_id` = ".$data['proId']." AND pl.`branch_id` =".$data['branch_id'];
		$sql.=" LIMIT 1 ";
    	return $db->fetchRow($sql);
    }
}
