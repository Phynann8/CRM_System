...
class Stock_Model_DbTable_DbPurchase extends Zend_Db_Table_Abstract
{
    ...
    function getAllSupPurchase($search=null){
    	...
    	if(!empty($search['title'])){
    		...
    		$s_where[]= " sp.purchaseNo LIKE '%{$s_search}%'";
	...
	*/
	function getAllProductsetbyId($data=[]){
    	$db=$this->getAdapter();
		$proId = empty($data["pro_id"]) ? 0 : $data["pro_id"];
		$branchId = empty($data["branch_id"]) ? 0 : $data["branch_id"];
    	$sql="
			SELECT 
				pset.*
			FROM  
				rms_product_location AS setlc  JOIN `rms_product_setdetail` AS `pset` ON pset.`id_pro_location` = setlc.`id` 
			WHERE 
			setlc.`branch_id`=$branchId
			AND pset.`pro_id` = $proId
		";
		$sql.=" ORDER BY pset.id ASC ";
    	return $db->fetchAll($sql);
    }
    function updateStock($pro_id,$location_id,$qty_order, $cost=0,$reference,$move_type=0){
		// move_type 1 for add stock, 2 for purchase order
		if(empty($reference)){
			$reference = 0;
		}
		if(empty($cost)){
    	...
    	if($rs_pro['is_productseat']==1){//for set
    		...
    		$rs_set = $this->getAllProductsetbyId($arr);
    		...
    		if(!empty($rs_set)){
    			foreach($rs_set AS $rs){
					...
					$param = array(
						'branch_id'=>$location_id,
						'product_id'=>$rs['subpro_id'],
						'movement_type'=>$move_type, // 2 Po
						'quantity'=>$qty_order*$rs['qty'],
						'reference'=>$reference,
			...
			$param = array(
				'branch_id'=>$location_id,
				'product_id'=>$pro_id,
				'movement_type'=>$move_type, // 1 for add stock
				'quantity'=>$qty_order,
				'reference'=>$reference,
    	...
    	try{
			...
			$reference = $sup_proid;
	    		...
	    		foreach ($ids as $i){
    				...
    				$this->updateStock($_data['product_name_'.$i],$_data['branch'],$_data['qty_'.$i], $_data['cost_'.$i],$reference ,2);
	    		}
    			$db->commit();
		   	}catch (Exception $e){
    ...
    function updateStockBack($id){
 	...
 	function updatePurchase($_data,$id){
    	...
    	$db->beginTransaction();
    	try{ 
    		$oldrs = $this->getPurchaeSupplierById($_data['id']);
    		$old_status = $oldrs['status'];
    		if ($old_status == 1) { // If old PO was active
				$this->updateStockBack($_data['id']); // Revert stock changes
			}
			...
			$dbmv ->deleteMovementStock($reference, 2);
			...
			foreach ($ids as $i){
				...
				if($_data['status']==1){
					...
					$this->updateStock($_data['product_name_'.$i],$_data['branch_id'],$_data['qty_'.$i],$_data['cost_'.$i],$reference,2);
				}
			}
			$db->commit();
		}catch (Exception $e){
    ...
    function getPurchaeSupplierById($id){
    	$db=$this->getAdapter();
    	$sql="SELECT sp.*,s.sup_name,s.purchase_no,s.sex,s.tel,s.email,s.address,sp.amount_due,sp.branch_id,sp.status
		       ...
		       WHERE s.id=sp.supplierId AND sp.id=$id ";
    	$dbp = new Application_Model_DbTable_DbGlobal();
