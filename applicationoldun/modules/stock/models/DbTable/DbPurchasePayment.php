...
class Stock_Model_DbTable_DbPurchasePayment extends Zend_Db_Table_Abstract
{
    ...
    function getPurchaseBySupplier($data){
    	...
    	$sql="SELECT * FROM `rms_purchase` AS p  WHERE p.supplierId =$supplier_id AND p.status=1 AND p.is_paid = 0 AND p.branch_id =$branch_id ";
    	...
    	if (!empty($data['bypuchase_no'])){
    		$sql.=" AND p.purchaseNo ='".addslashes(trim($data['bypuchase_no']))."'";
    	}
    	$rs = $db->fetchAll($sql);
    	$string='';
    	$no = $data['keyindex'];
    	...
    	if(!empty($rs)){
    		foreach ($rs as $key => $row){
    			...
    			<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 100px;">&nbsp;
    			<label id="invoicelabel'.$no.'">'.$row['purchaseNo'].'</label>
    			<input type="hidden" dojoType="dijit.form.TextBox" name="invoice_hidden'.$no.'" id="invoice_hidden'.$no.'" value="'.$row['purchaseNo'].'" >
    			</td>
    			<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 100px;">&nbsp;
    ...
    function getPurchasePaymentDetail($payment_id){
    	$db = $this->getAdapter();
    	$sql="SELECT pd.*,
			(SELECT p.purchaseNo FROM `rms_purchase` AS p WHERE p.id = pd.purchase_id LIMIT 1) AS purchaseNo
    		...
    		foreach ($rs as $key => $row){
    			...
    			if (!empty($rowpaymentdetail)){
    				...
    				<tr id="row'.$no.'" style="background: #fff; border: solid 1px #bac;">
	    				...
	    				<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 100px;">&nbsp;
		    				<label id="invoicelabel'.$no.'">'.$rowpaymentdetail['purchaseNo'].'</label>
		    				<input type="hidden" dojoType="dijit.form.TextBox" name="invoice_hidden'.$no.'" id="invoice_hidden'.$no.'" value="'.$rowpaymentdetail['purchaseNo'].'" >
	    				</td>
	    				<td style="vertical-align: middle; text-align: left; border-left:solid 1px #ccc; min-width: 100px;">&nbsp;
    ...
    function getCurrentBalanceBySupplier($data){
    	$db = $this->getAdapter();
    	$sql = "SELECT SUM(inv.`amount_due_after`) AS all_balance FROM `rms_purchase` AS inv WHERE inv.`status`=1 AND inv.`is_paid`=0 AND inv.`supplierId`=".$data['supplier_id']." AND inv.branch_id =".$data['branch_id'];
    ...
    function getPruchaseById($id,$branch_id){
    	...
    	WHERE s.id=sp.supplierId AND sp.status=1 AND sp.id=$id AND sp.branch_id =$branch_id";
    	return $db->fetchRow($sql);
    }
