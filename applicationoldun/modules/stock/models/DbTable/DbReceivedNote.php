...
class Stock_Model_DbTable_DbReceivedNote extends Zend_Db_Table_Abstract
{
    	...
    	try{
    		...
    		if (!empty($ids)){
    			foreach ($ids as $i){
					...
					$qty_stock = $this->getProductLocation($_data['pro_id_'.$i],$_data['branch_id']);
					if(!empty($qty_stock)){
						// update cost average
						$total_amount_transfer= $qty_main_stock['costing']*$_data['qty_'.$i];
						$this->updateProductCost( $_data['pro_id_'.$i], $_data['branch_id'], $_data['qty_'.$i],$total_amount_transfer);
					...
					$param = array(
						...
						'movement_type'=>3, // 3=receive
						'quantity'		=>$_data['qty_'.$i],
						'reference'		=>$receive_id,
						'note'			=>'ទទួលទំនិញពីការផ្ទេរ',
					);
    ...
    function getReceiveNoteDetailById($id){
    	...
    	(SELECT $coloum FROM `rms_itemsdetail` AS ide WHERE ide.id = td.pro_id LIMIT 1) AS product_name,
		td.*,
    	tr.`qty` AS transferQty,
		r.status
    	FROM `rms_received_note_detail` AS td
    	LEFT JOIN `rms_received_note` AS r ON r.`id` = td.`receive_id`
    	LEFT JOIN `rms_transferdetail` AS tr ON ( tr.`transferid`= r.`transfer_id` AND tr.`pro_id` = td.`pro_id` )
    	WHERE
    	td.receive_id=$id ";
    	return $db->fetchAll($sql);
    }
    ...
    function updateReceiveNoteTransfer($_data){
    	...
    	try{
			...
			// delete old movement 
			$dbmv ->deleteMovementStock($receive_id, 3);
				...
				if (!empty($ids)){
					foreach ($ids as $i){
						...
						$param = array(
							'branch_id'		=>$_data['branch'],
							'product_id'	=>$_data['pro_id_'.$i],
							'movement_type'=>3, // 3= treeransfer stock in
							'quantity'		=>$_data['qty_'.$i],
							'reference'		=>$receive_id,
							'note'			=>'ទទួលទំនិញពីការផ្ទេរ',
						);
