...
//     	header('Content-Type: application/json');
    	...
    	if ($_SERVER['REQUEST_METHOD'] == "GET"){
    		if($GetData['url']=="profile"){
    			...
    			$_dbAction->getFormOptionSelectAction($GetData);
			}else if ($GetData['url']=="scoreOption"){
				$GetData['getControlType'] = "scoreOption";
    			$_dbAction->getFormOptionSelectAction($GetData);
			}else if ($GetData['url']=="optionExamType"){
				$GetData['getControlType'] = "examType";
    			$_dbAction->getFormOptionSelectAction($GetData);
			...
			}else if ($GetData['url']=="classForIssueAttendance"){
    			$_dbAction->getClassForIssueAttedanceAction($GetData);
			}else if ($GetData['url']=="summaryAttSubject"){
    			$_dbAction->getSummaryStuAttBySubjectAction($GetData);
			}else if ($GetData['url']=="scoreStatistic"){
    			$_dbAction->getScoreStatisticAction($GetData);
    		}
    		else{
    		...
    		if($this->getRequest()->isPost()){
				...
				}else if ($GetData['url']=="submitStudentAtt"){
    				$_dbAction->submitStudentAttAction($postData);
    			}
    			else{
    				echo Zend_Http_Response::responseCodeAsText(401,true);
    			}
