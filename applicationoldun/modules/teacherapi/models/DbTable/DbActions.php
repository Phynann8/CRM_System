...
class Teacherapi_Model_DbTable_DbActions extends Zend_Db_Table_Abstract
{
	...
	public function submitCriteriaScoreAction($search){
		try{
			...
			if($submitRequest["status"]){
				...
				$isPushNotify = empty($search['isPushNotify']) ? 0 :$search['isPushNotify'];
				if($isPushNotify=="1"){
	...
	public function submitEditCriteriaScoreAction($search){
		try{
			...
			if($submitRequest){
				...
				$isPushNotify = empty($search['isPushNotify']) ? 0 :$search['isPushNotify'];
				if($isPushNotify=="1"){
					$gradingTmpId = empty($search['gradingTmpId']) ? 0 :$search['gradingTmpId'];
					$notify = array(
						"typeNotify" => "criteriaStudentScore",
						"optNotification" => 3,
						"notificationId" => $gradingTmpId,
	...
	public function getClassForIssueAttedanceAction($search){
		try{
			$search['userId'] = empty($search['userId'])?0:$search['userId'];
			$search['currentLang'] = empty($search['currentLang'])?1:$search['currentLang'];
			$db = new Teacherapi_Model_DbTable_DbApi();
			$row = $db->getClassForIssueAttedance($search);
	...
	public function submitStudentAttAction($search){
		try{
			$search['userId'] = empty($search['userId'])?0:$search['userId'];
			$search['currentLang'] = empty($search['currentLang'])?1:$search['currentLang'];
			
			$db = new Teacherapi_Model_DbTable_DbApi();
			$submitRequest = $db->submitStudentAttendance($search);
	...
	public function getSummaryStuAttBySubjectAction($search){
		try{
			$search['userId'] = empty($search['userId'])?0:$search['userId'];
			$search['currentLang'] = empty($search['currentLang'])?1:$search['currentLang'];
			$db = new Teacherapi_Model_DbTable_DbApi();
			$row = $db->getSummaryStuAttBySubject($search);
	...
	public function getScoreStatisticAction($search){
		try{
			$search['userId'] = empty($search['userId'])?0:$search['userId'];
			$search['currentLang'] = empty($search['currentLang'])?1:$search['currentLang'];
			$db = new Teacherapi_Model_DbTable_DbApi();
			$row = $db->getScoreStatistic($search);
