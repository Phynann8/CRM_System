...
class Test_IndexController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
    	try{
    		...
    		$collumns = array("SERIAL","STUDENT_NAMEKHMER","Last Name","Name","SEX","PHONE","DOB","GENERAL_KNOWLEDGE_RESULT","FOREIGN_LANGUAGE_RESULT","BY_USER","PRINT_PROFILE");
    		
    		if ($testCondiction!==2){
    			$collumns = array("BRANCH","SERIAL","STUDENT_NAMEKHMER","Last Name","Name","SEX","NATIONALITY","PHONE","DOB","FROM_SCHOOL","GENERAL_KNOWLEDGE_RESULT","FOREIGN_LANGUAGE_RESULT","BY_USER","PRINT_PROFILE");
    ...
    function createtestexamAction(){
    	...
    	if($this->getRequest()->isPost()){
    		...
    		try {
				...
				$inFrame = empty($data['inFrame']) ? "" : "true";
    			if(!empty($data['saveclose'])){
    				Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS",self::REDIRECT_URL."?inFrame=".$inFrame);
    			}
    		}catch (Exception $e){
    			Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
    			Application_Form_FrmMessage::message("INSERT_FAIL");
    	...
    	if (!empty($test)){
			...
			$substudy= $_dbgb->getCurentStudentStudy($id);
			$this->view->currentStudy = $substudy;
    	}else{
