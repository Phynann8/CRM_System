...
class Test_Model_DbTable_DbStudentTest extends Zend_Db_Table_Abstract
{
	...
	function updateStudentTest($data){
		...
		try{
			...
			$row  = $this->getStudentTestById($id);
			...
			if($row["is_studenttest"]==1 && $row["customer_type"]==4){
				$array["status"] = $status;
			}
			$photo = "";
			$name = $_FILES['photo']['name'];
	...
	function getAllStudentTest($search=null){
		...
		$sql.="
				...
				s.dob,";
		if ($testCondiction!=2){
			$sql.="pre.schoolName AS from_school,";
		}
		$sql.="
...
// 					'note'		=>$data['note'],
	...
	function insertTestExam($data,$type=null,$test=null){
		...
		try{
			$_dbgb = new Application_Model_DbTable_DbGlobal();
		//	$schoolOption = empty($data['schoolOption'])?1:$data['schoolOption'];
			$array = array(
			...
			$schoolOption = $_dbgb->getSchoolOptionbyDegree($degree);
			
			$data['programType'] = empty($data['programType']) ? 1 : $data['programType'];
			...
			if (!empty($data['id'])){
				if($test!=null){
					...
					if (!empty($check)){
						...
						$_arr['programType']=$data['programType'];
				...
				// update sub class
				$this->_name = 'rms_group_detail_student';
				$where=" itemType = 1 AND is_maingrade =0 AND stu_id = ".$data['stu_test_id'];
				$this->delete($where);
				if(!empty($data['identity_study'])){
					$ids = explode(',', $data['identity_study']);
					foreach ($ids as $i){
						$schoolOption = $_dbgb->getSchoolOptionbyDegree($data['degree_'.$i]);
						$_arr = array(
							'stu_id'			=>$data['stu_test_id'],
							'branch_id'			=>$data['branch_id'],
							'academic_year'		=>$data['academic_year'],
				...
				if (!empty($check)){
					...
					$where.=" AND itemType = 1 AND is_current =1 ";
					$this->_name="rms_group_detail_student";
					$this->update($_arr, $where);
				}else{
					//$schoolOption = empty($data['schoolOption'])?1:$data['schoolOption'];
	...
	function checkStudentInGroupDetail($_data){
		...
		$sql.=" AND sd.itemType=1 AND is_current =1 ";
		$sql.=" LIMIT 1";
		return $db->fetchRow($sql);
	}
	...
	function getAllStudentTestResult($search = null){
		...
		$sql="
			SELECT 
				...
				, (SELECT sOpt.title FROM rms_schooloption AS sOpt WHERE sOpt.id = str.test_type LIMIT 1 ) AS testDateSmallInfo
