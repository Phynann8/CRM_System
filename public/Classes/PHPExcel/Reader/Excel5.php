...
//
			...
			if ($cellAddress == '$B$65536') {
				...
				$extension = true;
				$keys = array_keys($this->_phpSheet->getComments());
...
//		echo '<br />';
	 ...
	 * @param string &$valContext     The current MD5 context state of the value
	 * 
	 * @return bool Success
	 */
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		if ($isActive) {
			$this->_phpExcel->setActiveSheetIndex((int) $this->_phpExcel->getIndex($this->_phpSheet));
		}
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			try {
				$cellRange = $this->_readBIFF8CellRangeAddressFixed(substr($recordData, 0, 8));
			} catch (PHPExcel_Exception $e) {
				return;
			}
...
//			var_dump($bcBitCount);
			...
			$filename = tempnam(PHPExcel_Shared_File::sys_get_temp_dir(), 'phpshift');
			imagepng($ih, $filename);
	 ...
	 * @return string Human readable formula
	 */
	private function _createFormulaFromTokens($tokens, $additionalData, $baseCell = 'A1')
	{
		// empty formula?
		if (empty($tokens)) {
			return '';
	 ...
	 * @param int $version BIFF version (XLS_BIFF8 or XLS_BIFF7)
